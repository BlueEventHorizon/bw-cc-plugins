...
切り替えの詳細 — `.claude/.forge.yaml` の書き方、索引更新スキル（`/forge:update-db-rules` / `update-db-specs`）だけが持つ `--backend` 強制指定、および別軸であるレビュー実行主体（`agent-review`）の指定 — は [switch-forge-backend_ja.md](docs/readme/switch-forge-backend_ja.md) を参照。
...
| スキル                                                                                    | 説明                                                                                                                                                                                                                                                                                                                                                                                                                        | トリガー                            |
| ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| [**review**](docs/readme/forge/guide_review_ja.md)                                        | レビュー依頼の組み立てと所見の評価・修正・再依頼・完了判定をスキルの実行側が駆動し、レビュアーとの往復はレビューバックエンド SKILL へ委譲。`--backend` で指定、未指定なら候補を順に可用性検査（現在の候補は外部依存を持たない agent-review のみ）。依頼/再開の2モード（再開は履歴復元に対応するバックエンドのみで、現在そのバックエンドは無い）。`--secrets` で機密情報スキャン、`--scope` で到達目標と意図的な未実装を明示 | `"レビューして"`                    |
| _consult_                                                                                 | レビュー所見を 1 件ずつ提示し、採否の進行を担う                                                                                                                                                                                                                                                                                                                                                                             | ※ review が呼び出し                 |
...
| [**start-plan**](docs/readme/forge/guide_create_docs_ja.md#start-plan)                    | 設計書からタスクを抽出し JSON 計画書を作成                                                                                                                                                                                                                                                                                                                                                                                  | `"計画書作成"`                      |
...
| **create-feature-from-markdown-plan**                                                     | Claude plan mode の Markdown plan から要件定義書 → 設計書を一気通貫で作成（forge 実装計画書 `{feature}_plan.json` は対象外）                                                                                                                                                                                                                                                                                                | `"markdown plan から feature 作成"` |
...
| _prepare-figma_                                          | Figma デザインからデザイン仕様書を作成。nodeId 検証とプレビュー突合まで                                                                               | ※ impl-ui が呼び出し                |
