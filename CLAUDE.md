...
- **doc-advisor** (v0.1.5) — AI 検索可能なドキュメントインデックス（ToC）生成・検索ツール。文書から AI がメタデータを抽出し、タスクに関連する rules/specs 文書を自動発見する
...
### その他の user-invocable スキル

- **`/forge:start-uxui-design`** — 要件定義書の ASCII アートからデザイントークンと UI コンポーネント仕様を生成（iOS/macOS 対応）
- **`/forge:clean-rules`** — ルール文書の重複・散在を解消して整理する
- **`/forge:query-forge-rules`** (AI専用) — forge 内蔵ドキュメントの ToC を検索する。review スキルが内部で使用する。`/query-rules` はプロジェクト文書を対象とするが、こちらは forge プラグイン自身の知識ベースが対象
