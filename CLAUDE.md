...
- **タグ（`[MANDATORY]` / `[CRITICAL]` / `[IMPORTANT]`）は見出し行の末尾に付け、1 文書 0〜3 箇所に抑える**
- **見出しは 4 階層（`####`）まで**
...
なぜそうするかを置く節。規約そのものは上節が持つ。

- **不具合を最優先するのは**、AI の責任がこのプロジェクトのすべての要素が正しく動くことにあるから。「自分の変更とは無関係」「担当外」は放置の理由にならない
- **原本を直すのは**、キャッシュが次回 install で再生成され、原本の修正だけが下流すべてへ伝播する経路だから。目的は「刹那的に今のセッションで動くようにする」ことではなく、プロジェクトを正しいものに完遂してすべての他のプロジェクトを正しく導くこと
- **黙って進めたものは、報告漏れではなく隠蔽である**
- **available-skills を照合するのは**、素通りすると skill の契約が欠落し、後で気づいて修正・やり直しになるから（実績: PR #31、`anvil:create-pr` 素通りで Issue クローズキーワードが欠落）
