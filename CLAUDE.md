...
forge が保持する「仕様文書（ID体系・フォーマット・原則・レビュー基準等）」は `/forge:query-forge-rules` で検索できる。forgeがターゲットプロジェクトに対してどう作用するかを理解できる
`/update-forge-toc` SKILL で、query-forge-rules のデータベースとも言えるToCを更新することができる。
