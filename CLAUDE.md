...
- **doc-advisor** (v0.2.1) — セマンティック検索対応のドキュメントインデックス生成・検索ツール。OpenAI Embedding API で文書本文をベクトル化し、タスクに関連する rules/specs 文書を自動発見する
...
2. `/create-specs-index` — index 更新（利用可能な場合）
3. `/anvil:commit` — commit/push 確認

start-implement は index 更新を含まず、review → commit の2ステップで完了する。
...
完了後は共通完了処理フロー（review → index → commit）を実行する。
