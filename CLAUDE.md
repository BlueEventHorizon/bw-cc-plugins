...
本リポジトリを扱う前に、**プラグインという仕組みそのもの**を知っている必要がある。仕組みが担う範囲を知らなければ、機構が済ませていることを自分で実装し、機構が置換する記述をシェルの記法と取り違える。
...
- **配布物に、配布物に含まれない具体パスと本リポジトリ固有の事情を書かない**: 固有の事情とは、内部文書の ID・仕様名（`DES-025_issue_driven_flow_design` 等）、`meta/`、worktree 配置、原本 SoT である立場等。**配布物の公開名（`bw-cc-plugins` / `forge` / `anvil`）は固有の事情ではない**（利用者が install 時に指定し、現に目にする名前）
...
- 修正の検証は変更を反映した実体で行う。変更前の状態で得た結果を根拠に「直った」と判断しない。改名・語彙改訂・大規模な書き換えの後は、改名した識別子だけでなくその別名・形式の呼び名も検索し、検索語と範囲を残してから完了とする
...
- **forge plugin の script は、対応する SKILL.md が明示的に「直接呼び出す」と指示しているもの以外は直接実行しない**
