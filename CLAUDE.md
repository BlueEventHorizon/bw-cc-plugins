...
- **`docs/` 配下では必要でない参照リンクを書かない**: 参照先の発見は検索に委ねる。文書には「何に依存するか（概念・ID）」だけ残す。**書かなければ本文の意味が通じない場合に限り**マークダウンリンク方式（`[表示名](相対パス)`）で書く。地の文でのパス直書き・リンクなしのファイル名言及は使わない。記法は `docs/` 側・配布物側とも forge の `document_style_guide.md` §5.1 が定める（`docs/` 側は系統 (3)）。**必要性判定は配布物には適用しない**（決定論的に閉じた木として配布されるため開発者責任で内部リンクを書く）
