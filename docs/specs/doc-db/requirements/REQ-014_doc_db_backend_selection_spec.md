...
doc-advisor は導入有無と鮮度確認機能の有無（前提条件・NFR-002）で判定する。
選択後の索引整備（doc-db の KEY / series 作成、doc-advisor ToC の鮮度確認と更新）も
...
- **可用性判定は各 backend が所有する**。選択の仕組みは判定の中身を知らず、判定の結果だけを使う。
  ...
  - doc-advisor の可用性判定: 導入されており、かつ鮮度確認機能を利用できる場合に利用可能と判定する
    （前提条件の最小対応バージョン規定に従う）。
...
| doc-advisor が最小対応バージョン未満                    | 更新が必要である旨を通知したうえで doc-advisor を利用不能として扱い、残る backend の利用可否を確認する         |
