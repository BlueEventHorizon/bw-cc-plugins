...
この図は既定（優先 backend の指定なし）の順序である。設定で優先 backend が指定されている場合、
確認の順序は BL-001 に従って指定 backend を先頭に入れ替わる。判定の内容は変わらない。
...
- 指定された backend を利用できない場合、wrapper は処理を失敗させず、残る backend の利用可否を確認して利用する。
  指定は優先順位の変更であり、他方の backend の利用を禁止するものではない。
- ここでの「利用できない」は、**選択前の可用性判定**（接続確認、起動試行、利用可否確認）の失敗に限る。
...
- wrapper は、設定で指定された優先 backend を利用できず残る backend を利用した場合、指定が満たされなかったことと
  その理由を利用者へ通知する。
...
- backend の既定の選択順序は、doc-db、doc-advisor とする。
- 利用者がプロジェクトごとの設定で優先 backend を指定した場合、選択順序は指定された backend を先頭とする順序に置き換わる。
- 優先 backend の指定が変更するのは選択順序だけである。各 backend の利用可否の判定規則（接続確認、起動試行、
  鮮度確認、最小対応バージョン）と、選択後の処理（索引整備、通知、失敗の扱い）は指定の有無で変わらない。
- 指定された backend を利用できない場合に残る backend へ切り替えることは、BL-003 のとおり正常な処理経路である。
  切替の事由になるのは選択前の可用性判定の失敗だけであり、選択後の操作失敗は指定の有無にかかわらず
  障害として扱う（切替の理由にならない）。
- doc-db のインストール有無だけでは利用可能と判定しない。接続または起動後の再接続に成功した場合に利用可能と判定する。
- 選択順序で後位の backend は、先位の backend を利用できない場合にのみ選択する。
...
- 未整備を理由に doc-advisor へ切り替えてはならない。未整備は選択順序（BL-001）を動かす事由ではなく、
...
| 設定で指定された優先 backend が可用性判定で利用できない | 残る backend の利用可否を確認し、指定が満たされなかったことと理由を通知して継続する |
