# DES-028 doc-advisor 検索エンジン昇格 設計書
...
`--toc` モード・`--index` モードは **現行動作のまま維持**する（FNC-008 OP-02 / OP-03）。doc-advisor の Embedding 関連スクリプト（`embed_docs.py` / `search_docs.py` / `embedding_api.py`）も **保持**する。
...
> 本設計の変更は (1) mode=auto を doc-db 化、(2) doc-advisor 側 Embedding 関連スクリプトの API KEY 参照を doc-db と同一規約に統一（§3.4）、(3) `query-specs` / `query-rules` SKILL.md の description / argument-hint / mode=auto 動作の更新、に限定される。`--index` モード関連のスクリプト・ワークフロー文書は保持する（FNC-008 OP-03）。
...
本設計では doc-advisor 側 Embedding 関連スクリプト・関連テスト・`query_index_workflow.md`・`forge:clean-rules` SKILL の **削除は行わない**（FNC-008 §現行維持対象）。
...
>
> **Note (`create_checksums.py`)**: `/doc-advisor:create-specs-toc` / `/doc-advisor:create-rules-toc` SKILL（`toc_orchestrator.md` workflow）の incremental mode 用 checksum 生成・promote・cleanup を担う **ToC 系スクリプト**。本設計とは独立に従来通り動作する。
>
> **Note (forge:clean-rules)**: `plugins/forge/skills/clean-rules/SKILL.md` は本設計の対象外（無効化・リネームを行わない）。`embedding_api.py` を保持するため cross-plugin import 経路は repo 環境では引き続き動作する。install 環境（`~/.claude/plugins/cache/...`）の `parents[4]` 解決不能問題は既存バグであり、本設計とは独立した issue として扱う。
...
`--index` モードを維持するため、引数パースは **現行のまま変更なし** とする:
...
> `--index` モードは Embedding 関連スクリプトの保持と合わせて現行動作を維持する（FNC-008 OP-03 / 本設計 §2.1 KEEP サブグラフ）。
...
> 公式 250 文字制限は **listing 表示時の切り詰めライン**であるため、各 description は先頭 250 文字以内で「What（対象範囲）+ When（使用場面）+ 主要 benefit」を完結させる構造にしている（query-specs / query-rules ともに 1〜3 行目の合計が 250 文字以内）。詳細（フォールバック挙動・差別化・トリガー語）はそれ以降に並べる。`--index` モードは現行維持のため、description には `--toc` と `--index` の両モードを案内文として残す。「Hybrid 検索（auto）」「自動フォールバック」を主要 benefit として明示し、類似スキル（query-rules / forge:query-forge-rules）との差別化を本文中に明記している（review-skill-description 基準 #4）。
...
`--index` フラグ指定時は **現行動作（doc-advisor Embedding 検索）を維持** する。本設計では mode=index のロジックに手を加えない。`search_docs.py` 経由で従来通り動作する（FNC-008 OP-03）。
...
#### 5.5.1 FNC-008 §現行維持対象表との整合

本設計は §5.5 の判断により `query_db_workflow.md` を新規作成せず、mode=auto フローを `query-specs/SKILL.md` および `query-rules/SKILL.md` にインライン記述する。FNC-008 §現行維持対象表との整合点は以下:
...
- `--index` モードも **現行動作で維持** され、本設計 §8 / FNC-008 §現行維持対象表の完了判定では「`--index` 指定時に従来通り Embedding 検索が実行されること」が確認対象となる。
- `forge:clean-rules` SKILL も **保持**。本設計 §3.2 末尾 Note および §8 完了判定の「`forge:clean-rules` 維持」行に整合。
...
本設計では Embedding 関連テストは削除せず保持する（FNC-008 §現行維持対象 / §7.1 で更新）。
...
| `--toc` モード（UC-03）                              | `--toc` 指定時に従来通り doc-advisor の ToC 検索（`query_toc_workflow.md`）が実行され、subagent fork に切り替わらず親 Claude が直接 ToC ワークフローを実行して結果が返ること（OP-02 維持）                                                                         |
...
FNC-008 §現行維持対象セクションの受け入れ基準を補完する:
...
| NFR-01（FNC-002「見落としゼロ」要件）   | `meta/test_docs/` のゴールデンセットで本設計に基づく auto モード（doc-db Hybrid 検索）が現行 auto/ToC 方式と同等以上の recall（false negative 0）を満たすこと。基準を満たせない場合は実装前に設計判断（フォールバック継続・閾値緩和等）を本設計書に明記すること                                                                                                                                                                   |
