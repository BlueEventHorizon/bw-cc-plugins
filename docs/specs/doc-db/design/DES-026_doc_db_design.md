...
| `lexical_search.py` | クエリの語彙一致スコア算出、ID/固有名詞の完全一致を高スコア化 | `_utils` |
...
| `/doc-db:query` | `search_index.py` を起動して検索を実行 | `--category {rules\|specs}` `--query "..."` `[--top-n N]` `[--mode {emb\|lex\|hybrid\|rerank}]` `[--doc-type ...]`（`--category specs` で `--doc-type` 未指定時のデフォルトは `requirement,design`。`plan` は明示指定時のみ対象。`--category rules` は単一 Index のため `--doc-type` 無視）|
| `/doc-db:evaluate` | `evaluate.py` を起動してゴールデンセット評価を実行 | `[--method {toc\|advisor\|doc-db\|all}]` `[--output {path}]` |

SKILL.md にロジックは書かず、`${CLAUDE_PLUGIN_ROOT}/scripts/` のスクリプトを呼ぶ薄いラッパーに留める（doc-advisor の SKILL 構造を踏襲）。
...
**前提**: `.doc_structure.yaml` が存在し、OPENAI_API_KEY が設定されている。
...
1. 指定 doc_type の Index を load（rules は単一 Index）。`--category specs` で `--doc-type` 未指定時はデフォルト `requirement, design` をロードし、`plan` は明示指定時のみ対象（FNC-006 OP-04）
...
### 6.4 Lexical 検索の単語分割（FNC-006 LEX-01）

**採用**: 空白＋記号分割 + ID パターン（`[A-Z]+-\d+`）優先処理。

| 代替案 | 採否 | 理由 |
| ------ | ---- | ---- |
| 空白＋記号分割 + ID パターン | ✅ 採用 | 標準ライブラリのみで動作（NFR-005）。ID 検索の高スコア化要件（LEX-01）を満たす |
| 日本語形態素解析（MeCab 等） | 不採用 | 外部 pip パッケージが必要（NFR-005 違反） |
| TF-IDF / BM25 を完全実装 | 不採用 | 評価結果次第で v0.2 以降に検討。初期実装の複雑度を抑える |

スコア式: 各クエリ語の chunk 内出現頻度 × ID/固有名詞ボーナス。具体的な係数は実装で調整、ゴールデンセット評価で決定する。
