...
| `plugins/forge/scripts/doc_reference/check_refs.py`    | 索引と抽出結果を突き合わせ、参照切れと判定不能を報告する（FNC-006・NFR-001・NFR-004）      | 上記 3 モジュール + `doc_structure`（glob 展開と除外判定。§2） |
| `plugins/forge/scripts/doc_reference/fix_refs.py`      | 報告から索引で導ける置換先を決め、**その位置を差し替えて原本へ書き戻す**（FNC-013）（CLI） | `ref_normalize`                                                |
...
### 3.1a 前処理は 1 段で行い、位置を保存する [MANDATORY]
...
sequenceDiagram
    ...
    Check ->> Norm: 対象ファイルごとに正規化を要求する
    Norm -->> Check: 正規化テキストと位置写像
    Check ->> Extract: 正規化テキストから参照の抽出を要求する
    Extract -->> Check: 参照の列（正規化テキスト上の位置つき）と、判定不能の列
    Check ->> Check: 参照の列から索引に無いものを選ぶ
    Check ->> Check: 位置写像で原本の行・桁へ戻す
...
sequenceDiagram
    ...
    loop 索引で導ける所見だけ
        ...
        Fix ->> Norm: 位置写像で原本の範囲を得る
        Norm -->> Fix: 原本の行・桁
        Fix ->> Fix: その範囲だけを差し替える
    end
    Fix ->> Fix: 変更のあったファイルを書き戻す
    Fix -->> Caller: 置換した件数と位置 / 置換しなかった所見
```

**正常フロー**: 検査をやり直したうえで、索引から一意に導ける所見だけを、記録された範囲で差し替える。**同じ文字列を探し直さない。**

#### 4.3.1 座標系は 2 つあり、境界は 1 か所に置く

| 座標系   | 使う場所                                   |
| -------- | ------------------------------------------ |
| 正規化上 | 抽出が返す範囲、突合が参照を引くまでの内部 |
| 原本上   | 報告に載る位置、置換が書き戻す範囲         |

**変換は突合が報告を組み立てるときに 1 度だけ行う。** 抽出は正規化上の範囲だけを返し、原本の桁を知らない。置換は報告に載った原本上の範囲だけを使い、正規化テキストを再び作らない。**2 か所で変換すると、二重変換か変換漏れのどちらかが必ず起きる。**

#### 4.3.2 置換は後ろから当てる [MANDATORY]

1 つのファイルに複数の置換が決まったとき、**位置の大きいものから順に当てる**（行の降順、同一行では桁の降順）。

前から当てると、1 件目の差し替えで長さが変わった分だけ、2 件目以降の記録された桁がずれる。ずれた桁で差し替えれば、参照の途中を切って別の文字列を作る——参照切れよりも有害な誤接続になる（FNC-013）。
...
**置換しないもの**: 候補が 1 件に絞れないもの、位置指定の置換先（§3.3.1）、判定不能。これらは書き換えず、置換しなかった所見として返す。呼び出し側はそれを FNC-013 の 2 段目・3 段目として扱う。
...
- **`fix_refs.py` の適用順序**: **同一行に 2 件以上の置換が決まったとき、両方が正しく当たること**（素材として、置換後に長さが変わる 2 件を同じ行に置く。前から当てる実装では 2 件目がずれて落ちる。§4.3.2 の回帰）、同一ファイルの複数行にまたがる置換でも同じことを固定する
