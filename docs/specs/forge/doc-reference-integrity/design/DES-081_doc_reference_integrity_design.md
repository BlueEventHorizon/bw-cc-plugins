...
- **`fix_refs.py`**: 同名の実在パスが 1 件のとき参照元からの相対パスへの置換先を返すこと、**2 件以上・0 件では返さないこと**、位置指定を保存すること、**位置指定（節番号）の移動先は現在の節が何であれ返さないこと**（FNC-013 の 2 段目・3 段目の境界。番号の近さを対応の根拠にした退行を止める）
