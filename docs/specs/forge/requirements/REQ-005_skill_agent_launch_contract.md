...
> **重要 (2026-06-22 改訂)**: 本要件 §5 で第 1 推奨として採択した **方針 B-2 (fork 型 SKILL)** は §11 で撤回された。Claude Code の `context: fork` 機構に 9 件の構造的不具合が公式に報告されたためである。reviewer / evaluator / fixer は現在 `plugins/forge/agents/<name>.md` の **カスタム Agent** として実装されている。§1〜§10 は当時 (2026-05) の方針評価の歴史的記録であり、現行設計と矛盾する箇所は §11 / DES-029 を正とする。
...
| ADR    | `doc-advisor:ADR-002_query_skill_subagent_isolation`（外部リポジトリ BlueEventHorizon/DocAdvisor） | カスタム Agent 採用根拠 (COMMON-DES-001 §6 経由)                                                    |

---

## 11. 方針 B-2 撤回と Agent 採用 (REQ-006 fold)

本要件 §5 で第 1 推奨として採択した **方針 B-2 (fork 型 SKILL)** は、2026-06 に **撤回した**。Claude Code の `context: fork` 機構に 9 件の構造的不具合が公式に報告されており、本要件の前提 (fork 機構が信頼できる) が成立しないことが運用で判明したためである。代替として **カスタム Agent** (`plugins/forge/agents/<name>.md`) を採用し、reviewer / evaluator / fixer を再実装した。詳細根拠は本節を参照。

### 11.1 fork 機構の構造的不具合 (公式 Issue 9 件)

`anthropics/claude-code` リポジトリで以下が報告されている。特に **#18394 / #34164 / #60720 / #55592** は `/forge:review` 経路で観測された「何もせずに終了する」現象と症状が一致する:

| #                                                                | 状態   | バージョン        | 内容（一行要約）                                                                                                           |
| ---------------------------------------------------------------- | ------ | ----------------- | -------------------------------------------------------------------------------------------------------------------------- |
| [#18394](https://github.com/anthropics/claude-code/issues/18394) | CLOSED | 2.1.1             | `context: fork` が 95%+ の確率で効かず、既存 context でそのまま実行される                                                  |
| [#17283](https://github.com/anthropics/claude-code/issues/17283) | CLOSED | -                 | Skill ツール経由の起動で `context: fork` / `agent:` が honor されない                                                      |
...
| [#60720](https://github.com/anthropics/claude-code/issues/60720) | CLOSED | -                 | fork 型 SKILL の出力が Desktop UI に届かず無音終了                                                                         |
| [#55592](https://github.com/anthropics/claude-code/issues/55592) | CLOSED | Sonnet 4.6 で再現 | fork 型 SKILL が無限再帰し、fork された SKILL が自分自身を再 dispatch する (102+ fork / 5 分で手動停止)                    |
| [#19751](https://github.com/anthropics/claude-code/issues/19751) | CLOSED | -                 | fork 型 SKILL 内で AskUserQuestion が機能しない                                                                            |
| [#17351](https://github.com/anthropics/claude-code/issues/17351) | OPEN   | -                 | nested skill が呼び出し元 skill context に戻らず main context に戻ってしまう                                               |
...
A 層 (fork 境界) 自体が信頼できないため、SKILL.md 側の改訂では治せない。

### 11.2 カスタム Agent 採用方針 (FNC-N01 〜 FNC-N10)

新たな要件として以下を確定する (詳細記述・受け入れ基準は DES-029 §3 / §5 に集約):

- **FNC-N01 fork 型 SKILL の新規採用禁止 [MANDATORY]**: bw-cc-plugins 配下で `context: fork` を frontmatter に持つ SKILL を新規に作成しない。既存 fork 型 SKILL は FNC-N02 の置換方針に従って Agent 化する
- **FNC-N02 既存 fork 型 SKILL の置換 [MANDATORY]**: reviewer / evaluator / fixer を `plugins/forge/agents/<name>.md` のカスタム Agent に置換する
- **FNC-N03 session_dir ファイル契約の維持 [MANDATORY]**: refs.yaml / plan.yaml / `review_<種別>.md` / patch_result.json の構造を維持する
- **FNC-N04 起動契約の単一箇所集約 [MANDATORY]**: reviewer / evaluator / fixer の起動契約 (`subagent_type` / prompt / return スキーマ) は orchestrator (`review/SKILL.md`) の 1 箇所に集約する
- **FNC-N05 fixer の安全境界 [MANDATORY]**: 単一 finding 起動 / 編集対象 allowlist / 無関係 refactor 禁止 / 構文検証 の 4 制約を Role 制約として常時適用する (DES-029 §3.5)
- **FNC-N06 汎用 Agent と カスタム Agent の選定基準 [MANDATORY]**: 6 種別 × P1/P2/P3 / 5 観点精査 / 4 安全境界の playbook を持つ reviewer / evaluator / fixer はカスタム Agent。一回性が高く finding ID のみで完結する短ジョブは汎用 Agent
- **FNC-N07 起動経路定義の改訂 [MANDATORY]**: `docs/rules/skill_launch_paths_definitions.md` § 1 の fork 型 SKILL 項を「採用しない (廃止)」へ書き換える
- **FNC-N08 設計書の改訂 [MANDATORY]**: COMMON-DES-001 §6 / DES-015 / DES-028 / DES-029 を本要件に従い改訂する (実施済み、本書 fold 完了時点で全て反映済)
- **FNC-N09 静的検証テストの更新 [MANDATORY]**: `tests/common/test_no_fork_skill.py` + `tests/forge/agents/test_agent_frontmatter.py` + `tests/forge/agents/test_fixer_safety_prompt.py` を新設し、旧 fork 型関連テストを削除する
- **FNC-N10 段階移行 [推奨]**: F-1 (REQ/DES) → F-2 (reviewer) → F-3 (evaluator) → F-4 (fixer) → F-5 (クリーンアップ) の 5 段階で実施する

### 11.3 fold 元の追加 feature

本節は **REQ-006_fork_skill_deprecation** (`docs/specs/forge/no-fork-skill/requirements/`) と **DES-032_no_fork_skill_migration_design** (`docs/specs/forge/no-fork-skill/design/`) を fold した結果である。fold 元の文書は本書および COMMON-DES-001 / DES-015 / DES-028 / DES-029 への反映を完了したため削除した (`/forge:merge-specs` 実行)。
...
| 2026-06-22 | k2moons | **方針 B-2 (fork 型 SKILL) を撤回**し、REQ-006 / DES-032 (no-fork-skill feature) を fold。§11 「方針 B-2 撤回と Agent 採用」を新設し、Claude Code `context: fork` の構造的不具合 9 件 (公式 Issue #18394 / #34164 / #60720 / #55592 等) を根拠として記録。FNC-N01 〜 FNC-N10 を新規機能要件として確定 (詳細は DES-029 §3 / §5 に集約)。reviewer / evaluator / fixer は `plugins/forge/agents/<name>.md` のカスタム Agent として再実装済                                                                                                                                                                                                                                                                                                                                                                                                             |
