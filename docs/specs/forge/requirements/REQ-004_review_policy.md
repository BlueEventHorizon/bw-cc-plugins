...
fixer は汎用 Agent (general-purpose) を起動して Read/Edit を行う設計 (汎用 Agent 委譲による親コンテキスト消費抑制)。修正規模が非常に小さい場合、汎用 Agent 起動オーバーヘッドが修正コストを上回り、orchestrator (親 Claude) が直接修正した方が速い。Issue #86 が要求するパフォーマンス改善はここに該当する。

#### 要件

`/forge:review --auto-critical` / `--auto` の自動修正フロー、および `/forge:present-findings` の対話修正フローにおいて、**修正規模が軽量と判定できる場合は fixer (汎用 Agent) を起動せず、orchestrator が直接 `Edit` で修正する** 軽量修正経路を提供する。
...
`review_<種別>.md` の Read は軽量経路でのみ発生し、しかも該当 finding の抜粋のみを参照する。fixer 経路に進む場合は orchestrator は `review_<種別>.md` を読まず、fixer (汎用 Agent) が自前で Read する従来動作を維持する。
