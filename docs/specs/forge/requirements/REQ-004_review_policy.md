...
| `--auto`          | (値なし) | critical + major を自動修正 (🟢 minor は対象外・高リスク・明示警告を表示) |

#### 制約

- `--interactive` / `--auto-critical` / `--auto` は **すべて相互排他**。複数指定はエラー (early validation で拒否)
- 介入フィルタ (`--auto-critical` は critical のみ / `--auto` は critical + major) は介入軸でのみ機能する。検出段階では全 severity・全件を出力する
- **`--auto` は 🟢 minor を対象外とする** — minor は「改善提案」であり不具合ではないため、人間の確認なしに自動修正しない。minor を反映したい場合は `--interactive` で個別判断する
- **件数指定 (`--auto N`) は採用しない** — severity 順で N 件取るのは「重大度ベース」と「件数ベース」を混在させる軸であり、AI が誤生成しやすい。介入は「対話 / 🔴 のみ / 🔴🟡」の 3 モードに限定する
...
fixer はカスタム Agent として session_dir から plan.yaml / refs.yaml / `review_<種別>.md` を読み、Agent 自身が Read/Edit を行う設計である。修正規模が非常に小さい場合、Agent 起動オーバーヘッドが修正コストを上回り、orchestrator (親 Claude) が直接修正した方が速い。Issue #86 が要求するパフォーマンス改善はここに該当する。

#### 要件

`/forge:review --auto-critical` / `--auto` の自動修正フロー、および `/forge:present-findings` の対話修正フローにおいて、**修正規模が軽量と判定できる場合は Agent 経由 fixer を起動せず、orchestrator が直接 `Edit` で修正する** 軽量修正経路を提供する。
...
| fixer 経路 | 上記の 1 つでも欠ける     | forge:fixer Agent を Agent ツール (`subagent_type: "forge:fixer"`) で finding 単位ループ起動し、prompt に `session_dir / kind / id={id} / allowed_files=[...]` を渡す (DES-029 §3.5 / DES-028 §4.5)              |

`review_<種別>.md` の Read は軽量経路でのみ発生し、しかも該当 finding の抜粋のみを参照する。fixer 経路に進む場合は orchestrator は `review_<種別>.md` を読まず、fixer Agent が `session_dir` 経由で自前で Read する。
...
| `plugins/forge/agents/reviewer.md` (カスタム Agent)  | 優先度 1〜3 に基づく検出制御 (固有 perspective ロジック削除)。**reviewer は 1 起動とし、P1/P2/P3 を順次評価する** (FNC-412)                                                                                      |
| `plugins/forge/agents/evaluator.md` (カスタム Agent) | 5 観点精査と優先度 1〜3 の関係明示                                                                                                                                                                               |
...
| `plugins/forge/agents/fixer.md` (カスタム Agent)     | --auto-critical / --auto への対応 (FNC-413 軽量経路では呼び出されない)。**単一 finding 起動 / allowed_files allowlist / 無関係 refactor 禁止 / 構文検証** の 4 安全境界を Role 制約として常時適用 (DES-029 §3.5) |
...
| ID      | 内容                                                                                                                                                                                                                                                                                                               | 期限             |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| TBD-407 | `--auto` (critical + major を件数無制限で自動修正) を最終的に残すか撤去するか (現方針: 残すが明示警告。AI agent の誤用リスクを運用で再評価)                                                                                                                                                                        | 運用評価フェーズ |
