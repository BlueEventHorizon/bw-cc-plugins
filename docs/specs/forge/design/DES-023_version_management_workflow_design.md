...
**`tag_format` は宣言であって実在タグの保証ではない**。タグは利用者が手で付けることが多く、宣言と実際に打たれた形式（`v` の有無・target prefix の有無）は乖離しうる。この乖離は設定ミスではなく通常の状態として扱う。乖離を前提に、`tag_format` の用途を検索と作成で分ける:

| 用途                     | `tag_format` の扱い                                                        |
| ------------------------ | -------------------------------------------------------------------------- |
| 履歴取得のためのタグ検索 | 候補生成のテンプレートとして使い、下記 4 通りを検索する                    |
| AI によるタグ作成        | 既存タグの前例が優先。前例がなければ `tag_format`、それも無ければ `v` 付き |

検索候補は **prefix の有無 × `v` の有無の 4 通り**とする（prefix は `tag_format` の `{version}` より前の部分から `v` を除いた文字列）。宣言どおりの名前だけを見ると、タグが実在しても「タグ不在」と誤判定し、常に CHANGELOG 日付フォールバックへ落ちる。**`v` 軸だけでは足りない**: リリースタグを `{version}` だけで打ち、target prefix を使わない運用は珍しくないため、prefix 軸も振る。

**検索順は `{version}` → `v{version}` → `{prefix}{version}` → `{prefix}v{version}`** とし、最初に実在したものを採る。単純な形式を先に見るのは、リリースタグを `{version}` だけで打つ運用が最も広く、宣言側の複雑な形式より実在率が高いためである。`tag_format` の宣言と一致する候補を優先する規則は置かない（宣言は実在の根拠にならないため、優先しても外れを先に引くだけである）。

prefix を持たない候補は target を区別しないため、マルチ target で別 target が同じバージョン番号を持つと他 target のタグを拾いうる。採用前に当該タグの内容を確認する。範囲が多少ずれても CHANGELOG の材料としては成立するため、確認を条件に採用を許す。

作成側で前例を優先するのは、AI が新しい形式のタグを増やすと以降の履歴取得とリリース運用が分断されるためである。
...
| **changelog_header モード** | `--version-path changelog_header` | `_update_changelog_header()` | CHANGELOG を canonical version source とする場合  |
| **path モード**             | `--version-path` あり（上記以外） | `_update_with_path()`        | version_file（plugin.json 等）のフィールド更新    |
...
`--optional` 指定時、両エラーは区別して stderr の JSON `status` に反映される:

| 例外                  | `--optional` 時の `status` | 意味                                                          |
| --------------------- | -------------------------- | ------------------------------------------------------------- |
| `FilterNotFoundError` | `"skipped"`                | 対象外（正常系）                                              |
| `VersionDriftError`   | `"drift"`                  | ドリフト検出。呼び出し元 SKILL は完了メッセージで必ず警告する |

`--optional` 未指定時はいずれも exit 1 のエラー終了（従来通り）。
...
**責務**: base ブランチ（既定: main）と現在ブランチのバージョンを target ごとに比較し、二重バンプ防止・marketplace バンプ要否を判定する。
...
- **version 抽出**: 以下の順で試行する。
  1. `version_path` が `changelog_header` → 最初の version 見出しから抽出
  2. JSON パース → `version_path` のキーパスで取得
  3. JSON 失敗時 → テキスト正規表現で `version_path` 最終キー名に紐づく `v?X.Y.Z` を抽出（Swift 定数・TOML 等の非 JSON ファイル対応）
- **note の出し分け**: ファイルが base に**存在しない**場合のみ「新規追加」、存在するが version を抽出できない場合は別 note（形式未対応 / version_path 不一致）を付与する。これにより base に存在する非 JSON ファイルを「新規」と誤判定しない。
- semver 比較（`classify_bump`）も先頭 `v` を許容する。
...
1. **タグ候補の検索**: `tag_format` から前バージョンのタグ名を生成し、§2.4 の 4 形式を `git tag --list` へ渡す。複数が実在する場合は §2.4 の優先順で採る
...
| 対象                                   | ラッパー                       | 低レベルへの追加引数             |
| -------------------------------------- | ------------------------------ | -------------------------------- |
| version_file                           | `update_main_version.py`       | `--version-path {version_path}`  |
| sync_file (filter なし・optional 不可) | `update_required_dependent.py` | （追加引数なし）                 |
| sync_file (filter なし・optional 可)   | `update_optional_dependent.py` | `--optional`                     |
| sync_file (filter あり・optional 不可) | `update_required_filtered.py`  | `--filter "{filter}"`            |
| sync_file (filter あり・optional 可)   | `update_optional_filtered.py`  | `--filter "{filter}" --optional` |

ラッパー実行後の挙動:

- stderr の JSON で `status: "ok"` → 対象ファイルが更新済み
...
| `simple`           | 最初の「`##` + 数字または `v` + 数字で始まる version 見出し」行（メタ見出しはスキップ） |
