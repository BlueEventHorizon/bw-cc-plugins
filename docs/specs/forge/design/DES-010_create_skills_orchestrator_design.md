...
```yaml
source: forge:query-db-specs # forge:query-db-specs | forge:query-db-rules | doc_structure_fallback | code-exploration
...
sequenceDiagram
    ...
    par コンテキスト収集（並列）
        O->>S: specs 収集を委譲
        S->>S: /forge:query-db-specs or Glob
        S-->>O: refs/specs.yaml
    and
        O->>R: rules 収集を委譲
        R->>R: /forge:query-db-rules or Glob
        R-->>O: refs/rules.yaml
    and
        O->>C: code 探索を委譲
        C->>C: Grep/Glob で探索
