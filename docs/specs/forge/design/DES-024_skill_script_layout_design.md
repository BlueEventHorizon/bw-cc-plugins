...
1 subprocess を透過的に呼ぶラッパー。本設計のデフォルト分類で、ほぼ全てのラッパー (`resolve_*` / `scan_*` / `update_*` 等) がこれに該当する。
...
### 7.1 単一ラッパー (代表例: `clean-rules/scripts/resolve_rules.py`)
...
def main() -> int:
    r = subprocess.run(
        [sys.executable, str(LOW_LEVEL), "--{flag}", FIXED_ARG],
        check=False,
    )
    return r.returncode
...
固定 flag は配置 SKILL ごとにラッパー内へ hardcode する。`parents[3]` はラッパーが `plugins/{plugin}/skills/{skill}/scripts/` に置かれることを前提とした相対解決である（`${CLAUDE_PLUGIN_ROOT}` はデータ本文では展開されないため、スクリプト内では使わない）。

### 7.2 複合ラッパー (例外層。現存しない)

複合ラッパーは複数段の subprocess を連鎖させ、失敗時に stage 識別子で障害切り分けを可能にする。§2.2 の制約を全て満たす場合にのみ追加できる。
