...
全 reviewer 完了後、`extract_review_findings.py` が session_dir 内の `review_*.md` を glob で収集し、`review.md` と `plan.yaml` を生成する。plan.yaml には perspective が含まれる。実装は CLI facade、`scripts/review/findings_parser.py`、`scripts/review/findings_renderer.py` に分かれ、parser / renderer は file write を持たない。
