...
flowchart TD
    ...
    EVALUATORS["③ /forge:evaluator<br>finding 単位で 5 観点精査<br>→ plan.yaml 更新<br>(recommendation: fix / skip / create_issue)"] --> FIX_CHECK
...
extract 完了後、orchestrator が `/forge:evaluator` を起動する。evaluator は `review_{種別}.md` 内の各 finding を 5 観点 (ルール照合 / 設計意図 / 副作用リスク / false positive / 対象ファイル確認) で精査し、`recommendation` (`fix` / `skip` / `create_issue`) ・ `auto_fixable` ・ `reason` を plan.yaml に付与する。5 観点と P1/P2/P3 軸は直交関係であり、すべての finding に 5 観点を適用する (DES-028 §4.3)。
...
| `plan.yaml`        | extract_review_findings.py（初期）→ evaluator / present-findings / fixer（更新） | 全スキル                                        | 修正プランと進捗状態。`recommendation` ∈ {`fix`, `skip`, `create_issue`} / `auto_fixable` / `reason` / `priority` を含む |
...
orchestrator (review SKILL.md)
  ...
  │     ├─ 入力: session_dir, review_{種別}.md
  │     └─ 出力: plan.yaml 更新（recommendation: fix / skip / create_issue / auto_fixable / reason を付与）
  │
  ├─ Phase 6 (--interactive)
...
| 出力 | review_{種別}.md 更新 | 各指摘に `recommendation` (`fix` / `skip` / `create_issue`) + `auto_fixable` + `reason` を付与 (DES-028 §4.3) |
...
| 出力 | plan.yaml            | 全指摘の統合管理（`recommendation` ∈ {`fix`, `skip`, `create_issue`} / `auto_fixable` / `reason` / `priority` (P1/P2/P3) を含む） |
...
recommendation 決定フロー (5 観点の結果から `fix` / `skip` / `create_issue` を分岐) の詳細は DES-028 §4.3 を参照。
...
| recommendation 値域        | `fix` / `skip` / `create_issue` の 3 値 (`create_issue` は `should_continue` 判定から除外)                    |
