...
実際のレビュー・吟味・修正は専用の worker に委譲する。reviewer / evaluator / fixer は **カスタム Agent** (`plugins/forge/agents/<name>.md`) として実装され、Agent ツール (`subagent_type: "forge:<name>"`) 経由で起動する。present-findings はユーザー対話 (AskUserQuestion) を伴うため継承型 SKILL のまま:

| worker                    | 種別           | 起動方法                                  | 役割                           |
| ------------------------- | -------------- | ----------------------------------------- | ------------------------------ |
| `forge:reviewer`          | カスタム Agent | `Agent(subagent_type: "forge:reviewer")`  | レビュー実行（指摘事項の作成） |
| `forge:evaluator`         | カスタム Agent | `Agent(subagent_type: "forge:evaluator")` | 指摘事項の吟味・修正判定       |
| `/forge:present-findings` | 継承型 SKILL   | Skill ツール                              | 対話モードでの段階的提示       |
| `forge:fixer`             | カスタム Agent | `Agent(subagent_type: "forge:fixer")`     | 単一 finding 単位の修正実行    |

> reviewer / evaluator / fixer は旧 fork 型 SKILL から Agent 化済み (DES-029 / COMMON-DES-001 §6 参照)。fork 機構の構造的不具合 (Claude Code Issue #18394 / #34164 / #60720 / #55592 等) を回避するため、カスタム Agent へ移行した。
...
flowchart TD
    ...
    REVIEWERS["① forge:reviewer Agent × 1 起動<br>P1→P2→P3 を順次評価<br>→ review_種別.md"] --> EXTRACT

    EXTRACT["② extract_review_findings.py<br>priority/severity 抽出<br>→ review.md + plan.yaml"] --> EVALUATORS

    EVALUATORS["③ forge:evaluator Agent<br>finding 単位で 5 観点精査<br>→ plan.yaml 更新<br>(recommendation: fix / skip / create_issue / needs_review)"] --> FIX_CHECK
    ...
    FIXER["④ forge:fixer Agent<br>単一 finding 単位の修正実行"] --> RE_REVIEW

    RE_REVIEW["⑤ forge:reviewer Agent<br>単独修正のレビュー<br>（fixer の変更差分のみ）"] --> FIX_ISSUE_CHECK
...
orchestrator が refs.yaml の `review_packet` を読み、Agent ツール (`subagent_type: "forge:reviewer"`) で reviewer Agent を **1 体だけ** 起動する (FNC-412)。reviewer は P1 → P2 → P3 を `check_order` に従い順次評価し、`review_{種別}.md` (例: `review_design.md`) を出力する。観点ごとの分離は finding の `priority: P1|P2|P3` ラベルで表現し、agent 分離では行わない。

engine (`codex` / `claude`) は reviewer Agent に prompt 経由で渡し、Codex 実行 (`run_review_engine.sh`) / Claude self-review / Codex 不在時の fallback の差分はすべて **reviewer Agent 内部で完結** する。orchestrator は engine を問わず reviewer を 1 体起動するのみで、`run_review_engine.sh` を直接起動しない (DES-029 §4.2)。
...
extract 完了後、orchestrator が Agent ツール (`subagent_type: "forge:evaluator"`) で evaluator Agent を起動する。evaluator は `review_{種別}.md` 内の各 finding を 5 観点 (ルール照合 / 設計意図 / 副作用リスク / false positive / 対象ファイル確認) で精査し、`recommendation` (`fix` / `skip` / `create_issue` / `needs_review` の 4 値) ・ `auto_fixable` ・ `reason` を plan.yaml に付与する。5 観点と P1/P2/P3 軸は直交関係であり、すべての finding に 5 観点を適用する (DES-028 §4.3)。
...
| 2    | 修正実行 (軽量経路 or fixer 経路、FNC-413 で判定)                     | orchestrator (軽量) / forge:fixer Agent (重量、id 単位ループ起動) |
...
| 2b   | fixer 経路: 単一 finding 単位ループ起動                                                                                     | Agent ツール (`subagent_type: "forge:fixer"`、id 単位ループ。prompt: `session_dir / kind / id={id} / allowed_files=[...]`) |
| 3    | 再レビュー                                                                                                                  | Agent ツール (`subagent_type: "forge:reviewer"`、prompt: `session_dir / kind / engine / --diff-only / files_modified`)     |
...
旧経路 (汎用 Agent 起動による fixer / Skill ツール fork 型 fixer) は DES-029 で廃止。REQ-005 §1.1 の「経路 3 種混在」問題は経路 2 種に縮約される。fork 型 fixer は Claude Code Issue #34164 / #60720 等の構造的不具合により採用しない (REQ-005 §11.1)。
...
orchestrator (review SKILL.md)
  ...
  │
  ├─ Phase 5 → forge:evaluator Agent
  │     ├─ 起動: Agent ツール (subagent_type: "forge:evaluator")
  │     ├─ 入力: session_dir, review_{種別}.md
  │     └─ 出力: plan.yaml 更新（recommendation: fix / skip / create_issue / needs_review / auto_fixable / reason を付与）
  │
  ├─ Phase 6 (--interactive)
  │     └─ /forge:present-findings (継承型 SKILL) → plan.yaml 更新, 修正実行 (Issue 化選択肢を含む)
  │
  └─ Phase 6 (--auto / --auto-critical)
        ├─ 軽量経路 (FNC-413): orchestrator が Edit で直接修正
        ├─ fixer 経路: forge:fixer Agent (subagent_type: "forge:fixer"、id 単位ループ起動) → コード・文書修正
        └─ forge:reviewer Agent (--diff-only) → 単独修正レビュー
```
...
| 方向 | 項目                             | 内容                                                                                                                                                                                            |
| ---- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 入力 | 起動経路                         | Agent ツール (`subagent_type: "forge:fixer"`) で forge:fixer Agent を **finding 単位ループ起動** する (一括修正は呼び出し元が id 単位でループする)                                              |
| 入力 | prompt 構造                      | `session_dir / kind={review_type} / id={id} / allowed_files=[<finding.target_file>, ...]` (単一 finding 起動)。`--diff-only` の場合は `files_modified=[...]` を追加し、修正差分のみを再レビュー |
| 入力 | session_dir                      | fixer Agent が plan.yaml / refs.yaml / `review_<種別>.md` を自力 Read するための正本ディレクトリ                                                                                                |
| 入力 | レビュー種別                     | `code` / `design` / `requirement` / `plan` / `uxui` / `generic`                                                                                                                                 |
| 入力 | 4 安全境界 (DES-029 §3.5)        | 単一 finding 起動 / allowed_files allowlist 内のみ Edit / 無関係 refactor 禁止 / 修正後の構文検証                                                                                               |
| 入力 | 介入軸フラグ                     | `--auto-critical` / `--auto`。review からのループ起動では必ず透過する                                                                                                                           |
| 制約 | 指摘詳細・対象ファイル・参考文書 | **直接渡さない**。fixer Agent が `session_dir` から plan.yaml / refs.yaml / `review_<種別>.md` を Read して取得する                                                                             |
| 出力 | patch_result.json                | `patched_ids` / `failed_ids` / `files_modified` / `syntax_check` / `allowlist_violations` を記録する                                                                                            |
...
以降のカスタム Agent (reviewer・evaluator・fixer) に session_dir 経由で渡す。各 Agent は session_dir/refs.yaml から渡されたパスを Read するだけで、独自収集は行わない。

### 関連コードを探索してカスタム Agent に渡す

orchestrator が target_files を起点に汎用 Agent (general-purpose) で関連コードを探索し、
session_dir/refs.yaml 経由で reviewer・evaluator・fixer 全員に渡す。
