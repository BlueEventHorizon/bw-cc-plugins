...
Step 4: calculate_version.py で新バージョンを算出
    ↓
Step 5: ファイル更新
    5-1. version_file を update_version_files.py --version-path で更新
    5-2. sync_files を順次 update_version_files.py [--filter] で更新
    ↓
Step 6: CHANGELOG 更新（changelog セクションが存在する場合）
    ↓
Step 7: テスト実行（tests/ が存在する場合）
    ↓
Step 8: git 操作（auto_commit/auto_tag が有効な場合）
```
...
**`git_log_auto: false`**: CHANGELOG の最初の `## [` 行の直前に空テンプレートを挿入する。
