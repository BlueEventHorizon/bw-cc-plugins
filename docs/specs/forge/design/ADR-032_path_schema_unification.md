...
| 作成日     | 2026-06-29                                                           |
| ステータス | accepted (2026-06-30 実装完了)                                       |
...
### SoT 委譲注記 (2026-06-30 実装完了後)

実装完了後、refs.yaml / session.yaml の **操作的仕様 SoT は `plugins/forge/docs/session_format.md`** に委譲する。本 ADR は設計判断の履歴・代替案検討の記録として扱い、schema の正規定義としては参照しない。

- refs.yaml / session.yaml の field 値域・必須性・validation ルールの最新仕様は `session_format.md` §3 (session.yaml) / §7 (refs.yaml) / §9 (write_refs.py CLI) を参照
- 本 ADR が定義した「全 11 形式の適用方針」「設計原則」は実装完了時点で `session_format.md` に反映済み
- 本 ADR の grep 受け入れ条件・代替案検討は将来の参考のため保持するが、新規開発で参照する必要はない
...
| 2026-06-30 | 実装完了 (TASK-001〜008 / 計画書 `path_schema_unification_plan.yaml`)、ステータスを accepted に更新、SoT を `plugins/forge/docs/session_format.md` に委譲 |
