...
`/forge:query-db-`* は継承型のため親 context をそのまま保持する。*呼び出し側（forge:review / forge:start- / forge:create-feature-from-plan 等）は、`Skill` ツール経由で本 SKILL を起動する際、`args` に親タスクの context を貼り付けてはならない**（COMMON-DES-001 §3.4 / §4.2 に整合）。
