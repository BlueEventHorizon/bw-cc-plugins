...
`/forge:query-db-`* は継承型のため親 context をそのまま保持する。_呼び出し側（forge:review / forge:start- / forge:create-feature-from-plan 等）は、`Skill` ツール経由で本 SKILL を起動する際、`args` に親タスクの context を貼り付けてはならない_*（COMMON-DES-001 §4 / §4.2 に整合）。
