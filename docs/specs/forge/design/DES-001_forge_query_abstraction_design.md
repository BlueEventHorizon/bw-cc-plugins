...
- doc-advisor 側の内部仕様（auto モードの実行ロジック等）の規定。本書は forge 側のバックエンド選択を扱い、doc-advisor の挙動は doc-advisor 側 SoT（doc-advisor:DES-006_semantic_search_design および `plugins/doc-advisor/skills/query-{rules,specs}/SKILL.md`）を **引用** する。両者に矛盾が生じた場合は doc-advisor 側 SoT に従う。

### 1.4 API キーの前提（doc-advisor:DES-007_unified_api_key_reference_design 統一仕様）
...
forge は doc-advisor を呼ぶときに `--toc` / `--index` を渡さない（**フラグなし = auto**）。auto モード起動後の実行ロジックは doc-advisor 側 SoT（doc-advisor:DES-006_semantic_search_design および `plugins/doc-advisor/skills/query-{rules,specs}/SKILL.md`）を参照する。本書は forge 側のバックエンド選択を扱い、doc-advisor 内部の auto モード仕様は規定しない（§5.3 引用）。両者の記述に矛盾が生じた場合は doc-advisor 側 SoT に従う。
...
採用後の各バックエンドの内部挙動（doc-advisor auto モードのモード判定 / Index スキップ条件 / マージ仕様 等、doc-db Hybrid 検索の詳細 等）は本書の対象外であり、それぞれのプラグイン側 SoT を参照する（§5.3 引用 / doc-db:DES-026_doc_db_design）。
...
> - 後段セクションの 2 軸規定（forge 主処理は不参照 / doc-db 単独契約としては doc-db:FNC-006_doc_db_search_spec OUT-01/OUT-02 / doc-db:DES-026_doc_db_design 準拠で必須）は上記出力契約本文に昇格済み。doc-db 単独利用時の詳細出力契約は本設計で変更しない。
...
- **Role 章に read-only 制約 [MANDATORY] を明記**（doc-advisor:ADR-002_query_skill_subagent_isolation §B / COMMON-DES-001 §6 B 層）。`/forge:query-db-*` は read-only であり、以下を保証する:
...
- doc-advisor:ADR-002_query_skill_subagent_isolation で発生した暴走事象の根因は「親 context + args の命令調」の組み合わせで subagent が「実装指示」と推論したこと。継承型では fork 境界が無い分、`args` を検索語に限定することが C 層引数解釈ガードと並んで重要
...
- **変更する**: `plugins/doc-db/skills/query/SKILL.md` の **Output Format 記述** を変更する。具体的には、出力の先頭に `Required documents:` セクション（プロジェクトルート相対パスのリスト）を追加し、既存の chunk 見出し・chunk テキスト・スコア内訳・ヒット理由（doc-db:DES-026_doc_db_design / doc-db:FNC-006_doc_db_search_spec OUT-01/OUT-02 で規定される詳細情報）は後段セクションとして引き続き出力する形に再構成する。これは `/forge:query-db-*` が両バックエンド共通の出力契約に依拠するために必要な記述変更であり、既存の詳細情報を削除・任意化するものではない。
- **変更しない**:
  - doc-db の検索スクリプト本体（`plugins/doc-db/scripts/search_index.py` / `grep_docs.py` 等）の内部実装
  - `/doc-db:build-index` skill の仕様・スクリプト・引数契約（doc-db:FNC-006_doc_db_search_spec / doc-db:DES-026_doc_db_design に準拠したまま）
  - `/doc-db:query` の引数契約（`--category` / `--query` / `--mode` 等は既存仕様のまま）
  - chunk 見出し・chunk テキスト・スコア内訳・ヒット理由の出力（doc-db:FNC-006_doc_db_search_spec OUT-01/OUT-02 の契約は後段セクションとして維持）

`/forge:query-db-*` から `/doc-db:query` を呼ぶ際の引数は doc-db の既存仕様（doc-db:FNC-006_doc_db_search_spec、doc-db:DES-026_doc_db_design）にそのまま準拠する。
...
| SW-10  | 両方インストール + `OPENAI_API_KEY` のみ設定（フォールバック）                       | doc-db が採用される（doc-advisor:DES-007_unified_api_key_reference_design のフォールバック経路）                                                             |
...
- **新規 forge query 系 SKILL の継承型 + 多重防御契約の機械検証**: `tests/common/test_query_skill_isolation.py` の `CONSTRAINT_TARGET_SKILLS` に新規 2 SKILL（`plugins/forge/skills/query-db-rules/SKILL.md` / `plugins/forge/skills/query-db-specs/SKILL.md`）を追加し、§3.1「subagent 契約 [MANDATORY]」/ §8.1 が必須化した以下の項目を機械的に検証する。新規 2 SKILL は COMMON-DES-001 §4 規定リスト外（継承型）のため、`FORK_TARGET_SKILLS`（fork 検証）には追加しない:
  ...
  - Role 章に「バックエンド検索 SKILL（`/doc-db:query` / `/doc-advisor:query-*`）以外の `Skill` ツール呼び出し禁止」および「`/doc-db:build-index` 等の書き込み系 SKILL 起動禁止」が明記されている（doc-advisor:ADR-002_query_skill_subagent_isolation §B / §3.1「subagent 契約 [MANDATORY]」の例外条項）
...
- **`plugins/forge/scripts/backend_selection/select_backend.py` のユニットテスト**: 新規 `tests/forge/scripts/test_backend_selection.py` を追加し、§2.3 の **分岐テーブル A（採用バックエンド決定 5 行）と分岐テーブル B（採用バックエンド × category × operation → skill 名 8 行）の両方をゴールデン**として網羅する。各ケースで以下を検証する:
  ...
  - §1.5.1 の API キー判定式の正常 / 異常パスを網羅する: `OPENAI_API_DOCDB_KEY` のみ設定 / `OPENAI_API_KEY` のみ設定 / 両方未設定 / 両方空文字列 / 片方が空文字列でもう片方が有効値、の各ケースで API キーの有無判定が doc-advisor:DES-007_unified_api_key_reference_design のフォールバック順序に従う
...
本設計書は forge 改修のスコープ内で「`/forge:query-db-*` / `/forge:update-db-*` 経由のアクセス導線を用意する」ことだけを確定し、doc-advisor 側の user-invocable 維持／格下げや description 変更の方針は doc-advisor 側 SoT（doc-advisor:DES-006_semantic_search_design / doc-advisor:ADR-002_query_skill_subagent_isolation 等の doc-advisor 設計書群）に委ねる。doc-advisor 側 SoT の結論が出たら本書の関連記述（§4.3 ガイド文書等）を更新する。
...
### 12.4 doc-advisor:DES-007_unified_api_key_reference_design 反映漏れ（独立 issue）
