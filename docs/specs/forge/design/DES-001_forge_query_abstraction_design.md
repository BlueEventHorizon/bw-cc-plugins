...
`/forge:query-db-`* は継承型のため親 context をそのまま保持する。_呼び出し側（forge:review / forge:start- / forge:create-feature-from-plan 等）は、`Skill` ツール経由で本 SKILL を起動する際、`args` に親タスクの context を貼り付けてはならない_*（COMMON-DES-001 §3.4 / §4.2 に整合）。
...
4 抽象 skill の SKILL.md frontmatter は以下のテンプレートで固定する。`description` 本文（何をするか／いつ使うか／トリガー句）はユーザー対話で確定済みの内容を SoT とする。query 系の継承型方針・`allowed-tools`・read-only 制約は §3.1「subagent 契約 [MANDATORY]」で定めた契約に従う（本節では重複明記しない）。
...
- **マニフェスト整合性テスト** — `tests/common/` に抽象 skill 4 件の plugin.json 登録チェックと出力契約・subagent 隔離契約の機械検証を含める（§7.2）
...
- **forge query 系 SKILL の継承型 + 多重防御契約の機械検証**: `tests/common/test_query_skill_isolation.py` の `CONSTRAINT_TARGET_SKILLS` に 2 SKILL（`plugins/forge/skills/query-db-rules/SKILL.md` / `plugins/forge/skills/query-db-specs/SKILL.md`）を含め、§3.1「subagent 契約 [MANDATORY]」/ §6.1 が必須化する以下の項目を機械的に検証する。これら 2 SKILL は COMMON-DES-001 §4 規定リスト外（継承型）のため、`FORK_TARGET_SKILLS`（fork 検証）には含めない:
