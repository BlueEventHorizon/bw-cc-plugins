...
> review スキルとの違い: review.md / plan.yaml / evaluation.yaml は不要。refs/ はサブディレクトリとして分離（orchestrator_session_protocol.md（廃止済み・設計経緯参照のみ）に準拠）。
...
orchestrator_session_protocol.md（廃止済み・設計経緯参照のみ）の統一スキーマに準拠:
...
> orchestrator_session_protocol.md（廃止済み・設計経緯参照のみ）の方針に準拠: 「refs/ がない場合、後続の agent は参照文書なしで動作する（最低限の品質でも実行可能）」
...
| コンポーネント | ファイルパス | 用途 |
|---------------|-------------|------|
| セッションプロトコル | `docs/specs/forge/design/orchestrator_session_protocol.md`（廃止済み・設計経緯参照のみ） | refs/ スキーマ、セッションライフサイクル |
