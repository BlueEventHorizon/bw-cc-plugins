...
| 関連設計 | orchestrator_session_protocol.md, forge_review_pipeline.md      |
| 作成日   | 2026-03-13                                                      |
...
create-design / create-plan / create-requirements の 3 スキルを、review スキルと同じオーケストレータパターンに移行する。
...
| AIレビュー | `/forge:review` に委譲 | レビューパイプライン全体を担当 |
...
> review スキルとの違い: review.md / plan.yaml / evaluation.yaml は不要。refs/ はサブディレクトリとして分離（orchestrator_session_protocol.md に準拠）。

### 3.3 session.yaml スキーマ（create-* 用）

```yaml
skill: create-design         # create-design | create-plan | create-requirements
feature: login               # 対象 Feature 名
mode: new                    # new | update（create-requirements は interactive | reverse-engineering | from-figma）
...
orchestrator_session_protocol.md の統一スキーマに準拠:
...
#### ガイド文書による自己完結性の確保（FNC-003 準拠）

コンテキスト収集 agent には `plugins/forge/docs/context_gathering_guide.md`（自己完結型ガイド）のパスと `session_dir` のみを渡す。agent はガイド文書を読むだけで収集作業を自律実行できる。

```
session_dir: {session_dir}
guide: plugins/forge/docs/context_gathering_guide.md
steps: [1, 2, 3]    # 適用マトリクスに基づく実行 Step
```

> **Note**: ガイド文書 `context_gathering_guide.md` は本設計書とは別タスクで作成する。以下のプロンプトテンプレートはガイド文書移行前の暫定仕様として記載する。
...
> orchestrator_session_protocol.md の方針に準拠: 「refs/ がない場合、後続の agent は参照文書なしで動作する（最低限の品質でも実行可能）」
...
> **設計判断**: create-requirements の interactive モードではコンテキスト収集は最小限（rules のみ）。
...
| コンポーネント | ファイルパス | 用途 |
|---------------|-------------|------|
| セッションプロトコル | `docs/specs/forge/design/orchestrator_session_protocol.md` | refs/ スキーマ、セッションライフサイクル |
...
- **正常完了時**: 全フェーズ完了後に `rm -rf {session_dir}` で削除
- **中断時**: 残存させる（再開の可能性があるため）

### 6.3 session.yaml の更新タイミング

| タイミング | 操作 |
|------------|------|
| セッション作成時 | session.yaml 初期化（status: in_progress） |
| コンテキスト収集完了時 | last_updated 更新 |
| 文書作成完了時 | last_updated 更新 |
| 正常完了時 | status: completed, last_updated 更新 |

### 6.4 残存セッションの処理（resume_policy: none）

create-* スキルはセッション再開をサポートしない（`resume_policy: none`）。
ただし共通プロトコル（orchestrator_session_protocol.md）との整合性を保つため、次回起動時に残存セッションを検出した場合は以下の最小フローを実行する:

1. `.claude/.temp/` 内に自スキル名（`skill` フィールド）が一致する `session.yaml` を持つディレクトリを検索
2. 見つかった場合: AskUserQuestion で「前回の未完了セッションがあります。削除しますか？」と確認
3. 削除を選択 → `rm -rf {session_dir}` で削除して新規セッションを開始
4. 削除しない → 残存ディレクトリを無視して新規セッションを開始

> **設計判断**: review はサイクル実行（reviewer → evaluator → fixer）があり中間状態の価値が高いが、
> create-* は直線的なワークフローであり、中断時は最初からやり直す方が効率的。
> 再開ではなく「検出+削除確認」の最小対応でプロトコルの契約を満たす。
