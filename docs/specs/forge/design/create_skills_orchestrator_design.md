...
| 後処理 | メインコンテキスト | `/create-specs-index` による index 更新、`/anvil:commit` による commit/push 確認、完了案内 |
...
Phase 3: 品質保証
├── 3.1: 完全性チェック
└── 3.2: /create-specs-index [MANDATORY]
...
Phase 3: AIレビュー                ← 新規（FNC-006 準拠）
└── /forge:review design {差分ファイル} --auto

Phase 4: 品質保証（変更なし）

完了処理                           ← 更新（FNC-006 準拠）
├── /create-specs-index（利用可能な場合）
├── /anvil:commit
└── セッション削除 + 完了案内
```
...
Phase 3: AIレビュー [MANDATORY]    ← 更新（FNC-006 準拠: --auto モード）
