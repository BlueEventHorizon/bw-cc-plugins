...
**並列実行される agent は共有リソースに直接書き込まない。結果は Agent ツールの return value として orchestrator へ返す。**

1. 各 agent は**結果そのものを return value で返す**（markdown / 構造化テキスト）
2. orchestrator は return value を **main context に保持**し、必要なら統合する
3. 成果物（文書・設定ファイル等）への書き込みは **orchestrator が main context で行う**
...
| 責務                       | 説明                                         |
| -------------------------- | -------------------------------------------- |
| 結果を return value で返す | 共有リソースにも中間ファイルにも書き込まない |
...
Claude Code の Agent 環境ではファイルロック（`flock` 等）の信頼性が保証されない。ロックに依存するのではなく、書き込みを orchestrator へ集約する本契約を使用する。
...
| 2 | 共有リソース・成果物への Write は orchestrator のみが行うか          |
