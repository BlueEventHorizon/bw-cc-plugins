...
| 作成日   | 2026-03-14                             |
| 対象     | create-requirements, create-design, create-plan, start-implement |
...
|--------|-----------|--------------|--------------|---------------|-----------|
| create-requirements | `--add` モード時のみ | 常時 | `reverse-engineering` モード時のみ | 不要 | 不要 |
| create-design | 常時 | 常時 | 常時 | 不要 | 不要 |
| create-plan | 常時 | 常時 | 不要 | 不要 | 不要 |
| start-implement | 不要（直接特定） | 常時 | 常時 | 不要（直接特定） | 不要 |

> **create-requirements の設計判断**: interactive モード（新規）ではコンテキスト収集は最小限（rules のみ）。
> 要件定義は「何を実現するか」を定義する工程であり、既存実装への過度な依存は避ける。
>
> **create-plan の設計判断**: 計画書は実装ファイルを参照しないため、既存コード調査は不要。
>
