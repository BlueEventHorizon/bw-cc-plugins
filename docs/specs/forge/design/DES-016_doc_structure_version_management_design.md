...
方法: `sys.path` にスクリプトのディレクトリを追加して import する。`resolve_doc_structure.py` は `plugins/forge/scripts/doc_structure/`（`migrate_doc_structure.py` と同一ディレクトリ）に配置されているため:

```python
import sys
from pathlib import Path
SCRIPT_DIR = Path(__file__).resolve().parent  # plugins/forge/scripts/doc_structure/
...
| ファイル                                                               | 役割                                                 |
| ---------------------------------------------------------------------- | ---------------------------------------------------- |
| `plugins/forge/scripts/doc_structure/migrate_doc_structure.py`         | マイグレーションスクリプト（COMMON-REQ-001 準拠）    |
| `plugins/forge/scripts/doc_structure/resolve_doc_structure.py`         | バージョン検出（`get_version`, `get_major_version`） |
...
| `docs/specs/common/requirements/COMMON-REQ-001_versioned_migration.md` | マイグレーション要件定義                             |
