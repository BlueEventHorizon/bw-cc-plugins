...
出力: review_packet = {
  ...
  ssot_refs: [                          # 1 reviewer がまとめて参照する SSOT 文書群 (ADR-032: path に統一)
    # P1: criteria の「SSOT参照」表から選定
    { path: <delegated_rules_or_principles>, priority: "P1",
      doc_type: "rules" | "principles" | "format" },
    ...
    # P2: 種別共通 (criteria 不要、固定パス)
    { path: "plugins/forge/docs/spec_priorities_spec.md",  # §1 境界設定
      priority: "P2", doc_type: "principles" },
    # P3: 種別共通 (criteria 不要、固定パス)
    { path: "plugins/forge/docs/spec_priorities_spec.md",  # §3.4 / §4
...
- **criteria は判断を持たない** (FNC-402)。reviewer / evaluator は重大度カタログ・グレーゾーン許容範囲を必ず `ssot_refs[].path` (ADR-032 で `doc_path` から改名) 側の principles から読み取る
- **対象ファイルも軸分割しない** (FNC-412): target_files は 1 つの reviewer にまとめて渡す。対象ファイル数が実用上限 (3〜5) を超える場合は **起動分割せず**、Phase 1 の AskUserQuestion でユーザに `--files` の絞り込みを促す (起動数増加による複雑性再発を防ぐ)。finding ごとの対象ファイル分離は `target` フィールドで表現する
...
review_packet:
  ...
  ssot_refs:
    - path: <path>                    # ADR-032 で doc_path から改名
      priority: "P1" | "P2" | "P3"
      doc_type: "rules" | "principles" | "format"
  check_order: ["P1", "P2", "P3"]
  severity_source: "principles"
  output_filename: review_<種別>.md   # ADR-032 で output_path から改名 (sandbox 内ファイル名)
related_code:
  - path: <path>
    reason: <text>
    lines: <range>
...
- **検証**: `review_packet.criteria_path` / `output_filename` 必須、`ssot_refs[].path` 必須 (ADR-032 で `doc_path` から改名)、`ssot_refs[].priority` は P1/P2/P3 のいずれか、`doc_type` は rules/principles/format のいずれか、`output_filename` は `review_<種別>.md` 形式 (種別は CLI 引数の値域に一致)、`target_files[]` は dict 配列 (`path` フィールド必須)
...
| UC-5 | `/forge:review <種別> --files a.md,b.md --auto`                        | 指定ファイル群を critical + major 自動修正 (🟢 minor 除外・高リスク・明示警告) |
...
| `--auto N` (件数指定)                                 | severity 順 × 件数の混合軸は AI 誤生成リスクが高い。「対話 / 🔴 のみ / 🔴🟡」の 3 モードに限定 (FNC-410) |
...
| Phase 5 介入                   | 介入軸フラグ (`--interactive` / `--auto-critical` / `--auto`) で分岐。`--interactive` (=未指定時のデフォルト) は present-findings、`--auto-critical` は 🔴 のみ、`--auto` は critical + major (🟢 minor 除外) を fixer に渡す。severity フィルタは priority と独立に動作                                                                                                                                                                                                                                                                                   |
| Phase 5 軽量経路判定 (FNC-413) | `--auto-critical` / `--auto` で fixer を呼び出す直前に、plan.yaml の `recommendation: fix` AND `status ∈ {pending, in_progress}` の件数を集計 (`--auto-critical` は severity=critical でさらに絞る)。**5 件以下 + 全 `auto_fixable: true`** なら軽量経路に分岐し、orchestrator が `review_<種別>.md` から該当 finding を Read して `Edit` で直接修正する (`mark_in_progress.py` → `Edit` → `mark_fixed.py`)。条件不成立なら Agent ツール (`subagent_type: "forge:fixer"`) で fixer Agent を finding 単位ループ起動する。詳細は §4.5 軽量修正経路 (FNC-413) |
...
| `skills/review/scripts/review_session.py`         | review SKILL のセッション動詞 wrapper (probe / start / resume / finish / status)。session 作成 + refs.yaml 書き込みを 1 動詞 `start` に統合し、内部で `session_manager.py init` と `write_refs.py` を順次呼ぶ。`--files` は `start` の引数として透過 (`session.yaml` への `files` 保存責務は `session_manager.py` 側、#100)。旧 `init_session.py` / `find_session.py` ラッパーは本 wrapper 導入により撤廃済 |
| `scripts/session/write_refs.py`                   | refs.yaml のスキーマ (§2.3「refs.yaml のスキーマ契約」) に対応。`review_packet` セクション (criteria_path / ssot_refs[]{path, priority, doc_type} / check_order / severity_source / output_filename) の必須検証を行う (ADR-032 で `doc_path`→`path`、`output_path`→`output_filename`)。`output_filename` は `review_<種別>.md` 形式チェックを行う                                                           |
| `skills/review/scripts/skip_all_unprocessed.py`   | 変更なし                                                                                                                                                                                                                                                                                                                                                                                                    |
| `skills/review/scripts/run_review_engine.sh`      | 変更なし                                                                                                                                                                                                                                                                                                                                                                                                    |

### 4.2 `plugins/forge/agents/reviewer.md`
...
| severity 取得経路    | criteria は判断を持たないため、重大度は必ず `ssot_refs[].path` (ADR-032 で `doc_path` から改名) の principles 側重大度カタログ (FNC-411) から取得すること                                                                                                 |
...
| ファイル                                          | 規定内容                                                                            |
| ------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `plugins/forge/agents/templates/review_result.md` | 重大度セクション見出し (🔴/🟡/🟢) は表示用に温存。各 finding に `priority` 行を追加 |

### 4.3 `plugins/forge/agents/evaluator.md`
...
| --auto / --auto-critical 連携 | severity フィルタを priority と独立に評価。`--auto-critical` 対象 = severity=critical のみ、`--auto` 対象 = severity=critical + major (🟢 minor 除外) (priority 不問)                                                  |
...
| 軽量経路判定 (FNC-413) | 「段階的に解決」「✅を一括修正」で fixer を呼び出す直前に判定を行う。「段階的に解決」(件数=1) は `auto_fixable: true` のみ軽量経路に入り、ユーザー修正方針 (A 案 / B 案 / Other) が提示された場合は fixer 経路に進む。「✅を一括修正」は ✅付き項目 (= `auto_fixable: true`) が 5 件以下なら軽量経路に入る。軽量経路では orchestrator が `review_<種別>.md` から該当 finding を Read して `Edit` で直接修正する。詳細は §4.5 軽量修正経路 (FNC-413) |
...
### 4.5 `plugins/forge/agents/fixer.md`
...
| スクリプト                                  | 規定内容 |
| ------------------------------------------- | -------- |
| `plugins/forge/scripts/fixer/mark_fixed.py` | 変更なし |
...
4. 抽出件数が **5 以下** AND 全項目が `auto_fixable: true` → **軽量経路**
5. それ以外 → **fixer 経路**
...
##### fixer 経路の手順 (Agent 経由)

fixer は `plugins/forge/agents/fixer.md` のカスタム Agent として実装されており、Agent ツール (`subagent_type: "forge:fixer"`) で起動する。**単一 finding 単位起動 + 編集対象 allowlist + 無関係 refactor 禁止 + 構文検証** の 4 安全境界を Role 制約として常時適用する (DES-029 §3.5)。一括修正 (`--batch`) は呼び出し元が id 単位ループで複数回起動する。

| 手順 | 操作                                                                                                                                                                                                                                       |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | 呼び出し元 (review / present-findings) が Agent ツールで forge:fixer Agent を起動する (`subagent_type: "forge:fixer"`)                                                                                                                     |
|      | review (batch): finding ごとにループ起動。prompt: `session_dir / kind={review_type} / id={id} / allowed_files=[<finding.target_file>, ...]`                                                                                                |
|      | present-findings single: prompt: `session_dir / kind={review_type} / id={id} / allowed_files=[...]`                                                                                                                                        |
|      | present-findings batch: id ごとにループ起動 (review batch と同形)                                                                                                                                                                          |
| 2    | fixer Agent は session_dir から refs.yaml / plan.yaml / `review_<種別>.md` を自力 Read し、allowed_files 内に限定して Edit/Write で修正する。allowlist 外への書き込みは Role 制約で禁止される (DES-029 §3.5.2)。修正後に構文検証を実施する |
| 3    | fixer Agent は `patch_result.json` に `patched_ids` / `files_modified` / `syntax_check` / `allowlist_violations` を書き出して return する。plan.yaml の `fixed` 遷移は行わない                                                             |
| 4    | 呼び出し元が単独修正レビュー完了後に `mark_fixed.py` を呼び、plan.yaml を `status: fixed` に更新する                                                                                                                                       |

> 旧 fork 型 fixer 経路 (`Skill ツール (fork)` で `/forge:fixer` 起動) は Claude Code の `context: fork` 機構の構造的不具合 (Issue #34164 `$ARGUMENTS` 不達 / #60720 出力消失 / #18394 fork 効かず / #55592 無限再帰) により採用しない。詳細根拠は REQ-005 §11.1 / DES-029 §5.0。
...
| `plugins/forge/agents/reviewer.md`                                | Agent         | §4.2                                                                                                                                                                                      |
| `plugins/forge/agents/templates/review_result.md`                 | template      | §4.2 templates                                                                                                                                                                            |
| `plugins/forge/agents/evaluator.md`                               | Agent         | §4.3                                                                                                                                                                                      |
| `plugins/forge/skills/present-findings/SKILL.md`                  | SKILL         | §4.4                                                                                                                                                                                      |
| `plugins/forge/agents/fixer.md`                                   | Agent         | §4.5                                                                                                                                                                                      |
| `plugins/forge/skills/review/scripts/resolve_review_context.py`   | script        | §4.1 関連                                                                                                                                                                                 |
| `plugins/forge/skills/review/scripts/review_session.py`           | script        | §4.1 関連 (旧 `init_session.py` / `find_session.py` を統合した動詞 wrapper)                                                                                                               |
...
| review_session.py         | `start --files` が `session_manager.py init --files` に透過される (`--section` 受理経路が無いこと)。`files` の session.yaml 保存自体は `test_session_manager.py` が検証 (#100)。probe / resume / finish / status の各動詞が期待動作で返却 |
