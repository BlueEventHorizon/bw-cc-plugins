...
flowchart TB
    ...
    subgraph Body["plugins/forge/skills/review/"]
        ...
        Triage["scripts/prepare_triage.py<br/>（段階的提示の仕分けファイル）"]
    ...
    User --> Skill
    Skill --> Targets & Branch & Builder & Gate & Triage & Baseline & Verify & Collect & Finalize
    Triage --> Gate
...
| `gate_findings.py`           | 位置による所見の振り分け（auto_fix / excluded）。介入軸も重大度も見ない |
| `prepare_triage.py`          | 段階的提示の仕分けファイルの用意（振り分け・組み立て・書き出し）        |
...
介入軸そのもの（2 値・既定・相互排他・確信度が可否を決めること）は REQ-013 FNC-1304 が定める。本節が定めるのは、その要件を満たすための実装の形である。

**判断は 2 段に分かれる [MANDATORY]**。

| 段 | 何を決めるか               | 誰が決めるか                               |
| -- | -------------------------- | ------------------------------------------ |
| 1  | 修正できるか               | `gate_findings.py`（位置が確定しているか） |
| 2  | 確認を取らずに直してよいか | **本体**（修正内容への確信度）             |

段 1 は所見の性質だけで決まるため決定論的に処理できる。段 2 は所見の中身と決めた修正内容を読んで初めて決まるため、スクリプトでは扱えない。**この境界を越えて段 2 をスクリプトへ持ち込むと、確信度の代理として重大度を使うことになる。** 重大度は「問題の重さ」であって「修正が正しい自信」ではないため、代理として機能しない。実装が一度その形になっており、`--auto` が確信の無い修正を重大度だけを理由に適用しうる状態になっていた。

`gate_findings.py` の出力は 2 群である。
...
**介入軸も重大度も受け取らない [MANDATORY]**。同じ入力には常に同じ出力を返す。`--auto` でも `--interactive` でも、🔴 でも 🟢 でも、修正できるかどうかは変わらない。

**位置が確定していない所見を `excluded` とする理由**: 修正は修正対象が確定していて初めて成立する。位置の無い所見を修正対象に含めると、どこを直すかを推測で決めることになり、修正後の allowlist 検証も「意図した変更か」を判定できない。人間が「修正する」と判断してもこの点は変わらない。位置の特定自体を人間に依頼することはできるが、それは採否の判断ではなく調査の依頼であり、振り分けの外にある。

段 2 は本体が所見ごとに行う。確信度が ☑️ かつ介入軸が `--auto` のときだけ確認を取らずに直し、それ以外は段階的提示（§3.11）へ回す。**`--auto` でも人間の応答を待つことがある**——確信の無い箇所で止まることが `--auto` を信頼できるものにする。

`secrets` パターンは介入軸によらず `confirmed_fix` を空として終端処理へ進む。**確認なしの修正だけでなく段階的提示も行わない**——採否を聞いても採用の行き先が修正であり、その修正を禁じている（REQ-013）ためである。

### 3.11 段階的提示

介入軸によらず、確認を取ると決まった所見をここで提示する（`interactive` は修正できる所見すべて、`auto` は確信度が 🤔・無印の所見だけ）。人間の応答を挟むためターンをまたぐ。所見・AI の評価・利用者の判断をコンテキストだけで保持すると、圧縮で失われたときに再レビューからやり直しになり、**失われたこと自体も検知できない**。したがってこれらを仕分けファイルへ書き出す。

`prepare_triage.py` が振り分け（mode 固定）・Markdown の組み立て・置き場の作成と書き出しを一続きで行う。**この連鎖に AI の判断は挟まらない。** 途中結果の取り出しと置き場の組み立てを本体へ残すと、決定論的な処理を AI が手で行うことになり、件数の取り違えと位置の写し違いが静かに起きる。

書き出す骨格は所見が運んできた事実（識別子・重大度・位置・本文）だけで、背景・本質・推奨・決着の欄は空のまま出す。これらは AI の評価と利用者の判断であり、対話の中で埋まる。識別子は提示順に `01`, `02`, … と振り、**採否が決まっても振り直さない**（利用者が所見を指す手段であるため）。アジェンダ表のセルへ入れる要約は長さを切り詰める（所見本文は数百字になることがあり、そのまま入れると表に背景が流れ込んで読めなくなる。全文は所見ごとの節に残る）。

**ファイルは常に 1 つに保つ [MANDATORY]**。`.claude/.temp/review/triage.md` に固定し、置き場も名前も引数にしない。既存があれば上書きする。
...
上書きしてよいのは、このファイルが**進行中のラウンドを保持する作業ファイル**であって決着の保管庫ではないからである。決着した内容は修正そのものと次ラウンドの対応表に出る。コンテキストが保持している間はそちらが主であり、ファイルは圧縮に備えた控えにすぎない。

**ファイルが持つのは所見と判断だけである**。対象・観点・バックエンドは再開時の起動引数と Step 2 / Step 1.5 で確定するため、ファイルへ写さない。写すと、同じ値が 2 か所に存在して食い違う。

**前回の残りは依頼の開始時に利用者へ確認して片付ける [MANDATORY]**。介入軸によらず行う（`auto` 系で起動しても、前回の `interactive` が中断したまま残っていることがある）。確認は「削除して新しく始める / 続きから進める」の二択であり、**後者が中断からの再開の入口である**。再開のための専用モードも探索の仕組みも設けない。置き場は一時ディレクトリであり成果物ではない——git 管理下へ入れることも、レビューの対象に含めることも想定しない。

**利用者へ示すのは絶対パスである**。相対パスは cwd が異なると開けず、AI の次の Read でも解決できない。

**AI がドロップした所見も行を閉じる [MANDATORY]**。`interactive` でも所見評価のドロップ権限は AI に残る——不要と判断済みの所見を 1 件ずつ人間に見せるのは、時間を使うだけで判断の質を上げない。ただしドロップした行を `未着手` のまま残すと、提示も決着もされないまま残り、ファイルが進行を映さなくなる。`状態` を `対象外` にして理由を書き、件数と理由をアジェンダと同時にまとめて示す。人間はそれを見て個別に戻せる。

**段階的提示の中断は終端経路の `interrupted` ではない**。所見を残したままレビューが完了する経路（`findings`）であり、`interrupted` は利用者がレビューそのものの中止を指示した場合に限る。取り違えると未対応所見の一覧が要約報告から落ちる。
...
片付ける確認で、**中断からの再開もここが入口になる**（再開専用のモードを設けない）。振り分けと提示の詳細は §3.10 / §3.11 が定める。
...
| `gate_findings.py`          | 位置による 2 群への振り分け。**介入軸も重大度も受け取らないこと**（受け取れると確信度の代理に重大度が使われる）。位置未確定は `excluded`、それ以外は `auto_fix`（§3.10）                                    |
| `prepare_triage.py`         | 置き場もファイル名も固定であること。**ラウンドが違っても `review_id` が違ってもファイルが 1 つに保たれること**（最新が残る）。探索・列挙の口を持たないこと。依頼の文脈を受け取る引数を持たないこと（§3.11） |
| 同上                        | 提示順（重大度順）と識別子の対応。位置未確定・位置欠落の表示。表を壊す文字（改行・パイプ）の畳み込み。長い所見の要約が切り詰められ、全文は節に残ること                                                      |
| 同上                        | 絶対パスを返すこと。`--project-root` を指定すれば cwd によらずその直下へ書くこと。ラウンド番号の省略と 1 未満の値を拒否すること                                                                             |
