...
本体とバックエンドを分離する以前、本体はレビュアーとの往復をインラインで持っており、バックエンドという概念自体が実装に存在しなかった。本設計はその分離後の本体を定める。
...
flowchart TB
    ...
    subgraph Body["plugins/forge/skills/review/"]
        ...
        Combine["scripts/combine_findings_and_evaluations.py"]
...
本体とバックエンドはいずれも継承型 SKILL であり、本体が `Skill` ツールで起動する。共通書式の解釈（`parse_findings.py`）は全バックエンドが使うため、特定スキル配下ではなく配布物共通の場所に置く。
...
手順 3 の解決は**依頼を 1 通も送る前に完了する**。選ばれた結果は送信前の引数解釈結果表に出る（実行主体が可変である以上、所見の出どころが見えている必要がある）。ラウンド実行が `failure` になったときに代替を選ぶことはしない——それは REQ-013 FNC-1318 が禁じた自動フォールバックである。候補順による解決は依頼を 1 通も送る前に完了するため、これに当たらない。
...
| `combine_findings_and_evaluations.py` | reviewer の所見実体（`parse_findings.py` が生成した配列）と evaluator の判定（`parse_evaluation.py` が検証済みの配列）を、`index` で対応させて 1 件ずつ結合する（§3.10a）                                                                                                                                        |
...
本体はラウンドごとにレビュアーの変更を検出しない。変更集合の比較として一度実装したが、すでに集合へ入っているファイルを書き換えても集合が変わらず、最も起きやすい経路だけが漏れるため廃止した。
...
**終端かどうかをそこで再計算しない [MANDATORY]**。判定点が 2 つあれば食い違い、中断の指示に反して次のラウンドが走る。
...
**経路ごとに散文で発行を指示しない理由**: 終了通知の発行漏れは、資源を持たないバックエンドでは何も起きず、資源を自前で持つバックエンドを接続して初めてリークとして現れる（現在唯一の `agent-review` は資源を持たないため、発行漏れがあっても露見しない）。露見が遅れる失敗であり、同じ SKILL が送信・起床・待機で同型の失敗を 2 度起こしている。集約先がスクリプトから SKILL.md へ移っても、**単一の出口である**ことは変えない。

**終端なら経路を問わず通知する**。資源を持たないバックエンドは無視してよい契約であるため、発行側で「通知が要る経路か」を選別しない。選別を入れると、選別条件の誤りが再び発行漏れになる。
...
`combine_findings_and_evaluations.py`は、この結合を機械的に行う。

- **検証**: ①両配列が同じ長さであること　②evaluatorが返す`index`の集合が`{1, ..., len}`と完全一致すること（欠落・重複の検出）。いずれか不成立なら結合せずエラーを返し、本体はそのラウンドを`halted_with_open_findings`として終端処理へ回す
- **結合**: `index`が一致する所見と判定を1件ずつ組にする。この結合結果が、§3.10の振り分け（`located`/`unlocated`）の入力になり、また§3.11でconsultへ渡す「提示へ回るすべての所見」の実体になる

`secrets`パターンは所見配列の評価そのものを持たないため、本節は対象としない（§3.10の`secrets`除外と同じ理由）。

`combine_findings_and_evaluations.py`は実装済みであり、`plugins/forge/skills/review/SKILL.md`のStep 7手順1がこれを呼び出す（本節が危険視するAIが記憶でindex結合する動作は行わない）。

### 3.11 段階的提示

**本節は段階的提示が発生する場合の規定である。** 提示へ回る所見が 1 件も無いとき（ドロップのみ・`auto` で全件 `✅`）は consult への委譲を行わない——提示しないものに提示は要らない。その場合のドロップした所見は、対応表と要約報告に載る（§3.2）。

提示へ回った所見について、本体は consult（[consult:REQ-017](../../consult/requirements/REQ-017_consult_skill.md)）へ**同一コンテキストのまま**委譲する。consult は継承型 SKILL であり、review 本体を実行しているのと同じ会話が振る舞いを consult のものへ切り替えるだけである。したがって、以下は本体と consult の間で構造化データとして受け渡す必要が無い——**本体が既に評価した内容は会話コンテキストにあり、consult による対話進行の結果もそのままコンテキストに残る**:

- 所見の評価内容（重大度・位置・本文・`confidence`・`fix_confident`）
- 対話進行の結果（採否・理由）
...
唯一のユースケース**である。その分の状態は consult（[consult:REQ-017](../../consult/requirements/REQ-017_consult_skill.md) / [agenda:REQ-019](../../consult/agenda/requirements/REQ-019_agenda_record.md)）が持つ。UC-11 は依頼の開始時に前回の残りを
片付ける確認で、**中断からの再開もここが入口になる**（再開専用のモードを設けない）。再開を選んだ場合、
**このユースケースは UC-01（依頼の組み立てと委譲）を通らない**——依頼を送らない経路だからである。振り分けの詳細は §3.10 が、段階的提示の委譲は §3.11 が定める。

```mermaid
sequenceDiagram
    participant U as "利用者"
    participant B as "/forge:review 本体"
    participant R as "Reviewer（解決済みの契約の相手）"
    ...
    loop 往復
        B->>R: "review_id・ラウンド・依頼本文"
        R-->>B: "判定・所見配列"
        alt 所見の修正を終えて再依頼する
            B->>E: "依頼本文・所見配列（Agent ツール）"
            E-->>B: "disposition・severity・confidence・fix_confident（JSON）"
            B->>B: "reviewerの所見とevaluatorの判定を結合する（combine_findings_and_evaluations.py）"
...
**`R`（Reviewer）は「解決済みの契約の相手」を表す**——どのバックエンド（現在は `agent-review`）がこの契約を実現するかは、直前の`B->>B: "引数解釈・バックエンド解決"`が既に決めている（§2.1）。この往復ループが表すのは実行主体を問わない共通の契約（依頼を渡すと判定・所見配列が返る）そのものであり、「どのバックエンドか」という選択の話とは別の関心事である。両者を同じ参加者名（バックエンドSKILL）で表すと、この往復が「バックエンド切替の仕組み」であるかのように読めてしまう。

## 5. 使用する既存コンポーネント

- **依頼テンプレート 8 種**（`skills/review/templates/`）: 全バックエンド共通。バックエンドごとに分岐させない
...
| `combine_findings_and_evaluations.py` | 件数不一致・`index` 集合不一致（欠落・重複）を検出して結合を拒否すること。両配列の長さが一致し `index` が過不足なく揃っている場合のみ 1 件ずつ結合すること（§3.10a）             |
| `parse_evaluation.py`                 | evaluator 応答の件数・`index`・`disposition`/`severity`/`confidence`/`fix_confident` の値域を機械的に検証し、契約違反を `status: "error"` として返すこと（§3.10）                |
...
**テストで担保できない範囲 [MANDATORY]**: **終端経路の網羅は、テストではなく §3.2 の経路表に依存する**。SKILL.md はテスト対象外であるため、4 経路すべてが終端処理 Step へ到達することを機械が確かめる手段は無い。
...
| ID      | 内容                                                                   | 解決方法                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TBD-002 | 2 つ目のバックエンド接続後、本設計の境界が実際に非依存であったかの検証 | **`agent-review` 接続時点（evaluator 導入前）で検証済み**。契約と依頼本文の境界は保たれていた（8 テンプレートに実行主体の言及なし、共通 parser も実行主体に依存しない）。一方、本体 SKILL の起動条件・利用者向けガイド・呼び出し元スキルの記述には実行主体を前提とした説明が残っており、これらは是正した。evaluator・`combine_findings_and_evaluations.py` 導入後も、両者は所見配列・`evaluations` 配列という構造化データのみを扱い実行主体を参照しないため、この結論はそのまま維持される。**未検証のまま残るのは、2 つ目のバックエンドを実際に接続した際の確認そのものである。現時点で候補は `agent-review` 1 つであり、この検証は次のバックエンド接続時まで持ち越す** |
