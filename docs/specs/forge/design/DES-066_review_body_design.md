...
| -------- | ------------------------------------------------------------------------------------- |
| 設計ID   | DES-066                                                                               |
| 関連要件 | REQ-013（FNC-1303 / FNC-1304 / FNC-1310 / FNC-1312 / FNC-1313 / FNC-1318）            |
| 関連設計 | ADR-060（バックエンド軸）、ADR-065（契約の 3 要素）、ADR-066（本体と backend の分離） |
| 作成日   | 2026-08-03                                                                            |
...
解決先が存在しない場合は依頼を送信せずエラー終了する（fail closed。REQ-013 FNC-1318）。既定バックエンドへ暗黙にフォールバックしない——利用者が意図しない実行主体でレビューが走ることを避ける。

`--codex` / `--claude` は警告のうえ無視し、**backend 名としては解釈しない**。既存の呼び出し元がこれらを付けて起動しているため、backend 名として再解釈すると推測で実行主体を選ぶことになる。
