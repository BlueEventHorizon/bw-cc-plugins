...
本体とバックエンドを分離する以前、本体は msg-sys 経由の往復をインラインで持っており、バックエンドという概念自体が実装に存在しなかった。本設計はその分離後の本体を定める。
...
**経路ごとに散文で発行を指示しない理由**: 終了通知の発行漏れは、資源を持たないバックエンド（msg-review）では何も起きず、資源を自前で持つバックエンドを接続して初めてリークとして現れる。露見が遅れる失敗であり、同じ SKILL が送信・起床・待機で同型の失敗を 2 度起こしている。集約先がスクリプトから SKILL.md へ移っても、**単一の出口である**ことは変えない。
...
**`R`（Reviewer）は「解決済みの契約の相手」を表す**——どのバックエンド（`agent-review`/`msg-review`等）がこの契約を実現するかは、直前の`B->>B: "引数解釈・バックエンド解決"`が既に決めている（§2.1）。この往復ループが表すのは実行主体を問わない共通の契約（依頼を渡すと判定・所見配列が返る）そのものであり、「どのバックエンドか」という選択の話とは別の関心事である。両者を同じ参加者名（バックエンドSKILL）で表すと、この往復が「バックエンド切替の仕組み」であるかのように読めてしまう。
...
| ID      | 内容                                                                   | 解決方法                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TBD-002 | 2 つ目のバックエンド接続後、本設計の境界が実際に非依存であったかの検証 | **`agent-review` 接続時点（evaluator 導入前）で検証済み**。契約と依頼本文の境界は保たれていた（8 テンプレートに実行主体の言及なし、共通 parser も実行主体に依存しない）。一方、本体 SKILL の起動条件・利用者向けガイド・呼び出し元スキルの記述には実行主体を前提とした説明が残っており、これらは是正した。evaluator・`combine_findings_and_evaluations.py` 導入後も、両者は所見配列・`evaluations` 配列という構造化データのみを扱い実行主体を参照しないため、この結論はそのまま維持される。**未検証のまま残るのは、`msg-review` 等の 2 つ目のバックエンドを実際に接続した際の確認そのものである** |
