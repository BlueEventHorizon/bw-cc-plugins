...
- **検証**: ①両配列が同じ長さであること　②evaluatorが返す`index`の集合が`{0, ..., len-1}`と完全一致すること（欠落・重複の検出）。いずれか不成立なら結合せずエラーを返し、本体はそのラウンドを`halted_with_open_findings`として終端処理へ回す
...
**現状（未実装）**: `combine_findings_and_evaluations.py`は本節が新設定義したものであり、まだ実装していない。実装が追いつくまでの間、`plugins/forge/skills/review/SKILL.md`のStep 7手順1は旧来通り「`evaluations`の各要素を、対応する所見に結び付けて以後の手順で使う」という記述のままであり、本節が危険視するAIが記憶でindex結合する動作を実際には行っている。実装は本節の確定後、別途行う。
...
| ID      | 内容                                                                   | 解決方法                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TBD-002 | 2 つ目のバックエンド接続後、本設計の境界が実際に非依存であったかの検証 | **`agent-review` 接続時点（evaluator・`combine_findings_and_evaluations.py` 導入前）で検証済み**。契約と依頼本文の境界は保たれていた（8 テンプレートに実行主体の言及なし、共通 parser も実行主体に依存しない）。一方、本体 SKILL の起動条件・利用者向けガイド・呼び出し元スキルの記述には実行主体を前提とした説明が残っており、これらは是正した。**この結論は evaluator 導入後の現状には及ばない**——§3.10a が新設した結合ロジックは未実装のままであり（§3.10a「現状（未実装）」）、この乖離は本 TBD の検証範囲外である |
