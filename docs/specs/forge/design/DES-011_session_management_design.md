...
| 関連設計 | DES-010, DES-012, DES-014, DES-024                                                                  |
| 作成日   | 2026-03-13                                                                                          |
...
| monitor 表示用の正規化データ                     | monitor adapter の派生結果          |

monitor adapter の `derived` は表示用の派生情報であり、永続正規情報ではない。
...
`session.yaml` に追加できるのは、monitor 表示と resume 判断に必要な浅いフィールドに限定する。
...
| `focus`           | string | 1 行文字列                                         | `""`          | monitor 表示用の短い焦点 |
...
標準 phase は、monitor 表示とテスト安定化のために以下を優先して使う。ただし skill 固有 phase
...
- 更新後は `monitor.notify.notify_session_update(session_dir, session_yaml_path)` を呼ぶ
- monitor 不在は成功扱いとする
...
### 6.2 writer の通知と meta 更新

writer script は、成果物ファイルの保存に成功した後で monitor へ通知し、必要に応じて
`session.yaml` の浅い進行状態を更新する。core writer は `plugins/forge/scripts/session/store.py`
の `SessionStore` を通し、artifact write → artifact notify → meta update → `session.yaml`
notify の順序を共有する。

成果物本文の保存に成功し、session meta 更新だけ失敗した場合、writer 本体は成功扱いとする。
`session.yaml` は monitor 表示用の粗い進行状態であり、正規成果物ではないためである。
ただし stderr に警告を出す。
...
`read_session.py` は CLI facade として reader の結果を JSON 出力し、`monitor/session_adapter.py`
は reader の結果に monitor 表示用の `derived` だけを追加する。これにより YAML / Markdown の
