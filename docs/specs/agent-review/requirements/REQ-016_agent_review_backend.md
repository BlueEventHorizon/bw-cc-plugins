...
2. `reviewer` の役割定義が、成果物の変更・他の実行主体の起動・外部への書き込みを禁じており、`reviewer` が利用できるツールに編集専用の手段と他の実行主体を起動する手段が含まれていない。**この 2 つは違反を防ぐ手段であり、担保ではない**。担保は「レビュアーが成果物を変更していないことをラウンドごとに確認する」（REQ-013）が与える
3. 正常応答が `approved` または共通所見配列を伴う `findings` として返る
4. Agent の起動失敗、応答欠落、不正な判定が理由付き `failure` になる
5. 可用性検査が `reviewer` の定義、起動機能、read-only 制約を副作用なく確認する
