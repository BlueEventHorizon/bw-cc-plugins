...
**これらは違反を防ぐ手段であり、read-only の担保ではない [MANDATORY]**。担保は本体側のラウンドごとの変更集合の確認が与える（REQ-013 / DES-066）。

**採用理由**: read-only 性を「変更する能力の不在」で保証する設計は採らない。理由は 2 つある。

1. レビュアーは差分・ブランチ対象を自分で確定する必要があり（REQ-013 FNC-1312）、そのために読み取り用途のコマンド実行を要する。許可するツールの単位は汎用のコマンド実行であり、その内側を読み取りだけに絞って宣言する手段は持たない。能力を奪えば範囲指定のレビューが成立しない
2. 同種のコマンド実行と編集の能力は、所見の評価と修正を行う本体側の実行主体も持っている。レビュアーにのみ能力の不在を要求する根拠はなく、要求しても本体側の能力がそのまま残るため、成果物が変更されうるという性質は変わらない

したがって防ぐ対象を「能力の存在」から「役割の逸脱」へ移し、逸脱は事前の禁止と事後の検出で扱う。役割定義には許可する git 操作の列挙と変更操作の禁止列挙を置くが、これは指示への追従に依存する手段であり、担保として数えない。
...
汎用コマンド実行を許可していること自体は不足として扱わない（§2.2 のとおり対象の確定に必要である）。**条件 3 を「外部通信を許可していない」と書かない**——汎用コマンド実行は外部通信も可能にするため、そう書くと検査条文が事実と食い違う。検査は Agent 定義が意図どおりに書かれていることを確認するだけであり、read-only 性の担保ではない（担保は本体側の検出。§2.2）。
