...
> **本決定は ADR-043 により撤回された。** 撤回の理由は、本 ADR の前提（§1「msg-sys 自体がレビュー対話専用の記述に変質する」）が誤りだったと判明したことにある。msg-review（REQ-011/DES-040）は msg-sys（REQ-006/DES-034）へ正式に merge 済みであり、`docs/specs/msg-review/` は削除されている。以下は撤回された決定の経緯として保持する（削除しない）。
