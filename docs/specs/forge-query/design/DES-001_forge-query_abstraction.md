...
- doc-advisor 側の内部仕様（auto モードの実行ロジック等）の規定。本書は forge 側のバックエンド選択を扱い、doc-advisor の挙動は doc-advisor 側 SoT（`docs/specs/doc-advisor/design/DES-006_semantic_search_design.md` および `plugins/doc-advisor/skills/query-{rules,specs}/SKILL.md`）を **引用** する。両者に矛盾が生じた場合は doc-advisor 側 SoT に従う。
...
forge は doc-advisor を呼ぶときに `--toc` / `--index` を渡さない（**フラグなし = auto**）。auto モード起動後の実行ロジックは doc-advisor 側 SoT（`docs/specs/doc-advisor/design/DES-006_semantic_search_design.md` および `plugins/doc-advisor/skills/query-{rules,specs}/SKILL.md`）を参照する。本書は forge 側のバックエンド選択を扱い、doc-advisor 内部の auto モード仕様は規定しない（§5.3 引用）。両者の記述に矛盾が生じた場合は doc-advisor 側 SoT に従う。
...
- `docs/specs/doc-advisor/requirements/FNC-001_context_external_search_spec.md`（エラーケース表を含む契約）
...
> forge 側の動作: `/forge:query-db-*` は doc-advisor が返す応答をそのまま親に転送する。doc-advisor 側でエラー化されていない応答を forge 側でエラー化することはしない。
>
> なお doc-advisor 側の案内文言に旧スキル名（`/doc-advisor:create-rules-toc` 等）が含まれ、forge 側から見える経路でユーザーに提示される場合、本書 §4.1 のマッピングに従い、forge 側から見える文言を `/forge:update-db-*` に置換する責務を forge 側が負う（doc-advisor 側 SKILL.md / 内部文言の書き換えは doc-advisor 側 SoT の責務であり、本書では行わない）。
...
本設計書は forge 改修のスコープ内で「`/forge:query-db-*` / `/forge:update-db-*` 経由のアクセス導線を用意する」ことだけを確定し、doc-advisor 側の user-invocable 維持／格下げや description 変更の方針は doc-advisor 側 SoT（`docs/specs/doc-advisor/design/` 配下）に委ねる。doc-advisor 側 SoT の結論が出たら本書の関連記述（§4.3 ガイド文書等）を更新する。
