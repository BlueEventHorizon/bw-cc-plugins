...
>
> 注: ここでの『構造変換しない』（§3.1）は出力の形式整形に関する規定。バックエンド SKILL から見える案内文言中の旧スキル名（`/doc-advisor:create-*-toc` 等）を `/forge:update-db-*` 系にマッピング置換する責務は別カテゴリの操作であり、本規定に抵触しない。
...
description: |
  ...
  トリガー: "ルール検索インデックス更新", "ルールインデックス再構築", "/forge:update-db-rules"
user-invocable: true
argument-hint: "[--full]"
...
description: |
  ...
  トリガー: "仕様検索インデックス更新", "仕様検索インデックス再構築", "設計書インデックス更新", "/forge:update-db-specs"
user-invocable: true
argument-hint: "[--full]"
allowed-tools: Read, Bash, Skill
---
