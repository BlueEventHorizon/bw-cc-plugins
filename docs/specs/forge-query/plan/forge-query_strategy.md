...
- **スコープ**:
  ...
  - DES-001 §1.5.1 API キー判定式（`OPENAI_API_DOCDB_KEY` / `OPENAI_API_KEY` のフォールバック、DES-007 統一仕様）を Python で同等実装
...
- **目標**:
  ...
  - query 系 2 SKILL の SKILL.md が B 層・C 層多重防御契約（read-only 制約 / バックエンド検索 SKILL 以外の Skill 起動禁止 / `/doc-db:build-index` 等の書き込み系起動禁止 / 引数解釈 [MANDATORY] / 自己再帰禁止 / 出力契約 `Required documents:`）を ADR-002 / §3.1 subagent 契約に従って明記する
...
- **スコープ**:
  ...
  - ADR-002 §B / §C: read-only 制約と引数解釈ガードの SKILL.md への明記（COMMON-DES-001 §3.1 デフォルト継承型に整合、`context: fork` を付けない）
- **検証ポイント**:
