# SUBAGENT-DES-001 SKILL / Agent 起動契約 設計書

## メタデータ

| 項目       | 値                                                                                                                                                                                                                  |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 設計 ID    | SUBAGENT-DES-001                                                                                                                                                                                                    |
| 関連要件   | SUBAGENT-REQ-001_skill_agent_launch_contract                                                                                                                                                                        |
| 関連設計   | COMMON-DES-001_skill_base_design, forge:DES-015_review_workflow_design, forge:DES-028_review_policy_design, forge:DES-022_parallel_agent_output_contract_design, doc-advisor:ADR-002_query_skill_subagent_isolation |
...
`/forge:review` 配下の 5 SKILL の起動契約を整理し、reviewer / evaluator / fixer を **fork 型 SKILL** として再定義する。present-findings と review (orchestrator) は継承型のまま残す。これにより SUBAGENT-REQ-001 §5 の方針 B-2 を具体化し、Issue #32 の誤読 (`subagent_type: "forge:fixer"` 誤指定) を **構造的に解消** する。

### 1.1 採用したアプローチ

- **fork 型 3 種** (reviewer / evaluator / fixer): COMMON-DES-001 §4 リストに追加。`context: fork` frontmatter + 引数解釈ガード + Role 否定的制約を持つ
...
- **COMMON-DES-001 §4 改訂**: §4.3 のリスト変更手順に従い 3 SKILL を追加。fork 採用根拠を §4 に明記
- **静的検証 5 種追加**: SUBAGENT-REQ-001 TEST-S001〜S005 を `tests/forge/` 配下に実装
...
flowchart TB
    ...
    Review -->|"Skill ツール (fork)<br/>--kind {種別}"| Reviewer
...
| 起動契約の自己完結性               | 呼び出し元 prompt と対象 SKILL.md の整合保証が必要 (SUBAGENT-REQ-001 FNC-S003)        | SKILL.md 単独で完結 (FNC-S003 は不要に)                                                              |
...
sequenceDiagram
    ...
    Review->>Reviewer: Skill ツール (fork): args=session_dir + kind=code
...
sequenceDiagram
    ...
    alt N ≤ 3 AND 全 auto_fixable=true [軽量経路]
        ...
        loop 各 finding
            ...
            Fixer->>FS: mark_fixed.py
        end
        Fixer-->>Review: return: {status: ok, fixed_count, errors}
    end
```
...
| ----- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| UC-S1 | `session_dir`, `kind` (code/design/requirement/plan/uxui/generic) | return: `{status, output_path}` / 副作用: `review_<種別>.md` 書込                             | return に error 詳細。`review_<種別>.md` 不在で停止       |
...
| UC-S3 | (起動なし、orchestrator が直接 Edit)                              | plan.yaml 更新 (mark_in_progress.py / mark_fixed.py)                                          | orchestrator が継続判定                                   |
| UC-S4 | `session_dir`, `kind`, モードフラグ (`--single` / `--batch`)      | return: `{status, fixed_ids, errors}` / 副作用: ファイル修正 + plan.yaml 更新                 | return に error 詳細。修正済み項目は plan.yaml に保存済み |
| UC-S5 | `session_dir` のみ。COMMON-DES-001 §3.4 [MANDATORY] により親タスクの指示文・差分・Issue 本文を args に貼り付けない (継承型でも呼び出し側責務) | plan.yaml 更新 (対話結果反映)                                                                 | AskUserQuestion キャンセル時はサマリ表示で終了            |
| UC-S6 | `session_dir`, `kind`, `--single`                                 | UC-S4 と同じ                                                                                  | UC-S4 と同じ                                              |
| UC-S7 | (reviewer 内部、Bash で Codex CLI 起動)                           | stdout を reviewer が capture → `review_<種別>.md` に書き出す                                 | reviewer がエラーを return                                |

---

## 5. fork 採用根拠 (COMMON-DES-001 §3.2 適合)

本設計は COMMON-DES-001 §4.3 のリスト変更手順に従い、reviewer / evaluator / fixer を §4 リストに追加する。fork 採用根拠を SKILL ごとに以下に示す。

### 5.1 reviewer

| 判断基準 (§3.2)                                              | 適合                                                                                                                                                                                                                                 |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 親 context 漏洩による具体的な実害が記録されている            | **適合**: reviewer/SKILL.md L36 / L155 の「汎用 Agent を起動」記述が誤読を誘発するリスクあり (`reviewer 汎用 Agent がさらに別の汎用 Agent を起動する` と読まれる懸念)。Issue #32 主因は §5.3 fixer に集約。 |
...
| 判断基準 (§3.2)                                              | 適合                                                                                                                 |
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| 親 context 漏洩による具体的な実害が記録されている            | **適合**: evaluator/SKILL.md L23 の「`/forge:review` から汎用 Agent として起動される」記述が誤読を誘発するリスクあり (Issue #32 と同型の誤指定経路を生む懸念)。Issue #32 主因は §5.3 fixer に集約。 |
...
| 判断基準 (§3.2)                                              | 適合                                                                                                                                                                                                                                 |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 親 context 漏洩による具体的な実害が記録されている            | **適合**: Issue #32 の起源は **fixer** の `subagent_type: "forge:fixer"` 誤指定。本要件 (SUBAGENT-REQ-001) の主動機                                                                                                                  |
...
reviewer / evaluator / fixer は ADR-002 §A〜C / COMMON-DES-001 §4.1 に従い、以下を SKILL.md に含める。

### 6.1 frontmatter

```yaml
---
name: reviewer  # or evaluator / fixer
description: |
  ...
user-invocable: false  # AI 専用 SKILL (review orchestrator から呼ばれる)
context: fork
allowed-tools: Read, Write, Bash[, Edit]  # 最小集合 (fixer のみ Edit を含む)
argument-hint: "session_dir kind [flags]"
---
```

`agent:` フィールドは未指定 (`general-purpose` がデフォルト)。
...
| fixer     | `{status: "ok"\|"error", fixed_ids: integer[], failed_ids: integer[], files_modified: string[], error_message?: string}` ※ 修正サマリーは files_modified で把握 |

return 値以外の副作用 (`review_<種別>.md` / `eval_<種別>.json` / plan.yaml / 対象ファイル) は **session_dir 経由** で親が後から Read する。

---

## 7. 修正経路分岐表 (SUBAGENT-REQ-001 FNC-S007)

SUBAGENT-REQ-001 FNC-S007 に基づき、修正経路を **1 箇所の表** に整理する。本表は `forge:DES-015_review_workflow_design` に追記する候補。
...
旧経路 (汎用 Agent 起動による fixer) は本設計で **廃止**。SUBAGENT-REQ-001 §1.1 の「経路 3 種混在」問題は経路 2 種に縮約される。
...
| mark_in_progress.py / mark_fixed.py | `plugins/forge/skills/fixer/scripts/mark_*.py`          | plan.yaml の status 更新。軽量経路では review が直接呼ぶ、fixer 経路では fork 内から呼ぶ。本設計で変更なし |
...
SUBAGENT-REQ-001 §4 TEST-S001 〜 TEST-S005 を `tests/forge/` 配下に実装する。

### 9.1 テストモジュール一覧

| テスト ID | モジュール                                                    | 検査内容                                                                                                                                                                                                           |
| --------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| TEST-S001 | `tests/forge/subagent/test_agent_allowedtools_consistency.py` | SKILL.md 本文に `Agent ツール` / `汎用 Agent を起動` / `カスタム Agent を起動` の語があれば、`allowed-tools` に `Agent` が含まれること                                                                             |
| TEST-S002 | `tests/forge/subagent/test_skill_allowedtools_consistency.py` | SKILL.md 本文に `/forge:<skill>` / `/anvil:<skill>` の Skill 呼出記述があれば、`allowed-tools` に `Skill` が含まれること                                                                                           |
...
## 10. COMMON-DES-001 §4 の改訂案

COMMON-DES-001 §4.3 の手順 (1. PR で判断基準明示 → 2. リスト更新 → 3. SKILL.md 修正 → 4. テスト追加) に従い、§4 リストに以下 3 行を追加する。

```markdown
| `plugins/forge/skills/reviewer/SKILL.md` | forge | `reviewer` | （未指定＝`general-purpose`） | `false` | `/forge:review` のレビュー実行エンジン (種別ごとに 1 起動) | SUBAGENT-DES-001 §5.1 (Issue #32 の親 context 漏洩誤読を構造的に解消、設計原則「1 起動」と整合) |
| `plugins/forge/skills/evaluator/SKILL.md` | forge | `evaluator` | （未指定＝`general-purpose`） | `false` | `/forge:review` の判定エンジン (種別ごとに 1 起動) | SUBAGENT-DES-001 §5.2 (同上 + 5 観点精査の review playbook を独立 context で実行) |
| `plugins/forge/skills/fixer/SKILL.md` | forge | `fixer` | （未指定＝`general-purpose`） | `false` | `/forge:review` の修正実行エンジン | SUBAGENT-DES-001 §5.3 (同上 + 「メインコンテキスト消費を抑える」設計原則と整合 + 二重起動解消) |
```
...
| # | 範囲                                                                                                                              | 依存                  |
| - | --------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| 1 | COMMON-DES-001 §4 リスト改訂 (本設計 §10 の 3 行を追加)                                                                           | なし                  |
| 2 | reviewer/SKILL.md の改訂 (`context: fork` 追加 / Role 否定的制約 / 引数解釈ガード / 自己再帰禁止 / Issue #32 誤読源の文言削除)    | 1 完了後              |
| 3 | evaluator/SKILL.md の改訂 (同上)                                                                                                  | 1 完了後 (2 と並列可) |
| 4 | fixer/SKILL.md の改訂 (同上 + 「汎用 Agent への委譲」記述を「自身で Edit/Write」に変更)                                           | 1 完了後 (2 と並列可) |
| 5 | review/SKILL.md の改訂 (reviewer / evaluator / fixer の起動を Agent ツール → Skill ツール (fork) に変更 + 経路分岐表 (§7) を追記) | 2〜4 完了後           |
| 6 | present-findings/SKILL.md の改訂 (fixer 起動を Skill ツール (fork) に変更 + 経路分岐の文言整理)                                   | 4 完了後              |
| 7 | 静的検証テスト追加 (TEST-S001〜S005 + §9.2 fork 型 frontmatter 検証)                                                              | 2〜6 完了後           |
| 8 | DES-015 への経路分岐表追記 (本設計 §7 の表)                                                                                       | 5 完了後              |
| 9 | SUBAGENT-REQ-001 の TBD-002 〜 TBD-005 解消 (静的テスト判定ロジック・OBSOLETE マーカー運用・検出ヒューリスティクスを本設計に集約) | 7 完了後              |
...
| 日付       | バージョン | 内容                                                                                                                                                                                                                                                                                                                                                                  |
| ---------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-24 | 0.1        | 初版作成。SUBAGENT-REQ-001 §5 方針 B-2 (fork 型 SKILL) を具体化。reviewer / evaluator / fixer の 3 SKILL を fork 型化し、COMMON-DES-001 §4 リストに追加。修正経路を軽量経路 + fork 型 fixer の 2 種に縮約。引数解釈ガード・Role 否定的制約・自己再帰禁止を共通設計として規定。静的検証 5 種 (TEST-S001〜S005) と fork 型 frontmatter 検証を `tests/forge/` 配下に追加 |
