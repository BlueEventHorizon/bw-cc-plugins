...
`agent: general-purpose` を明示する (SUBAGENT-REQ-001 §6.4 受け入れ条件)。

> **reviewer 固有の追加引数**: `engine` (codex/claude) は reviewer のみが受け取る。reviewer の `argument-hint` は `"session_dir kind engine [flags]"` とする。evaluator / fixer は engine を受け取らないため共通雛形どおり `"session_dir kind [flags]"` を使用する。
...
| テスト ID | モジュール                                                    | 検査内容                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TEST-S001 | `tests/forge/subagent/test_agent_allowedtools_consistency.py` | SKILL.md 本文に `Agent ツール` / `汎用 Agent を起動` / `カスタム Agent を起動` の語があれば、`allowed-tools` に `Agent` が含まれること。**除外スコープ**: (1) fenced コードブロック (`` ``` `` で囲まれた範囲全体)、(2) `### 制約` / `## 制約` / `## Role` などの禁止・制約見出し配下のセクション全体（次の同レベル以上の見出しまで）                                                                                           |
| TEST-S002 | `tests/forge/subagent/test_skill_allowedtools_consistency.py` | SKILL.md 本文に `/forge:<skill>` / `/anvil:<skill>` の Skill 呼出記述があれば、`allowed-tools` に `Skill` が含まれること。**除外スコープ**: (1) fenced コードブロック (`` ``` `` で囲まれた範囲全体)、(2) `### 制約` / `## 制約` / `## Role` などの禁止・制約見出し配下のセクション全体（次の同レベル以上の見出しまで）                                                                                                         |
...
```markdown
| `plugins/forge/skills/reviewer/SKILL.md` | forge | `reviewer` | `general-purpose` | `false` | `/forge:review` のレビュー実行エンジン (種別ごとに 1 起動) | SUBAGENT-DES-001 §5.1 (Issue #32 の親 context 漏洩誤読を構造的に解消、設計原則「1 起動」と整合) |
| `plugins/forge/skills/evaluator/SKILL.md` | forge | `evaluator` | `general-purpose` | `false` | `/forge:review` の判定エンジン (種別ごとに 1 起動) | SUBAGENT-DES-001 §5.2 (同上 + 5 観点精査の review playbook を独立 context で実行) |
| `plugins/forge/skills/fixer/SKILL.md` | forge | `fixer` | `general-purpose` | `false` | `/forge:review` の修正実行エンジン | SUBAGENT-DES-001 §5.3 (同上 + 「メインコンテキスト消費を抑える」設計原則と整合 + 二重起動解消) |
```
...
| # | 範囲                                                                                                                                                                                                                                                   | 依存                  |
| - | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------- |
| 1 | COMMON-DES-001 §6 リスト改訂 (本設計 §10 の 3 行を追加) + reviewer/evaluator/fixer/SKILL.md に frontmatter 最小変更 (`context: fork` / `agent: general-purpose` のみ追加)。§6 追加と frontmatter は同一段階で完結させ、§6 SoT 原則との中間不整合を防ぐ | なし                  |
| 2 | reviewer/SKILL.md の残改訂 (Role 否定的制約 / 引数解釈ガード / 自己再帰禁止 / Issue #32 誤読源の文言削除)                                                                                                                                              | 1 完了後              |
| 3 | evaluator/SKILL.md の残改訂 (同上)                                                                                                                                                                                                                     | 1 完了後 (2 と並列可) |
| 4 | fixer/SKILL.md の残改訂 (同上 + 「汎用 Agent への委譲」記述を「自身で Edit/Write」に変更)                                                                                                                                                              | 1 完了後 (2 と並列可) |
