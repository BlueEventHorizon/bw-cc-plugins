...
本要件で扱う混乱の根本原因は、**「subagent」「fork 型 SKILL」「Skill ツール呼び」「Agent ツール起動」「Bash 経由 subprocess」が 1 箇所で図解されていない** こと。各文書に散発的に記述されているが、用語マップとして集約されていない。

> **優先度 [重要・先行可能]**: 用語マップの追加は **方針選定 (A / B-2) と独立** に進めてよい。配置先と用語定義が確定すれば本要件の他項目を待たずに着手・PR 化可能であり、後続作業 (SKILL.md 修正 / 設計書改訂) は **本マップを参照する形で書ける** ようになるため、先行 PR 化が望ましい。`§2.7` の旧 perspective 文書整理と同じく「破壊的変更を含まない独立 PR」に分類する。

### 3.1 新規追加すべき文書 (成果物)

`docs/rules/skill_authoring_notes.md` または `docs/specs/common/design/COMMON-DES-001_skill_base_design.md` に「SKILL / Agent / subagent 用語マップ」セクションを追加する。最低限以下を含める:
...
### 3.2 配置先の選定方針 [先行 PR で決定可能]

配置先は **先行 PR のレビューで決定** してよい (設計フェーズを待たない)。判断材料:

| 候補配置先                                                                | 採用する場合の理由                                               |
| ------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| `docs/rules/skill_authoring_notes.md` への節追加                          | SKILL 作成時の参照導線が既存。執筆者が最初に見る場所             |
| `docs/specs/common/design/COMMON-DES-001_skill_base_design.md` への節追加 | 設計判断の WHY を併記しやすい。fork 型リスト (§4) と隣接配置可能 |
| 独立文書 (`docs/rules/skill_agent_terminology.md` 等)                     | 図解中心で量がある場合。両方の文書からリンクで参照できる         |

> どこに置くかより **「1 箇所で図解されていること」が要件**。配置先の選択は実装フェーズで決めれば足りる。
...
### FNC-S008: 用語マップ文書の追加 [MANDATORY]

`docs/rules/skill_authoring_notes.md` または `docs/specs/common/design/COMMON-DES-001_skill_base_design.md` に「SKILL / Agent / subagent 用語マップ」が追加されていること。「Skill ツール (継承型 / fork 型) / Agent ツール / Bash subprocess」の 4 経路を 1 図に統合する。
...
| #  | 範囲                                                                                        | 依存 |
| -- | ------------------------------------------------------------------------------------------- | ---- |
| P1 | **用語マップ文書の追加** (§3 / FNC-S008)。配置先は PR レビューで決定                        | なし |
...
| ID      | 内容                                                                                                                                 | 期限                                                |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------- |
| TBD-001 | 用語マップ文書の配置先 (`docs/rules/skill_authoring_notes.md` / `COMMON-DES-001` / 独立文書のいずれか)                               | **先行 PR レビューで決定** (設計フェーズを待たない) |
