...
本要件で扱う混乱の根本原因は、**「継承型 SKILL」「fork 型 SKILL」「汎用 Agent」「カスタム Agent」「Bash subprocess」を 1 箇所で定義した文書がない** こと。各文書に散発的に記述されているが、定義として集約されていない。
...
```
起動経路 (短縮名称):
  - 継承型 SKILL     ← Skill ツール (context: 未指定)。親 context を消費。SKILL.md は親が直接 Read
  - fork 型 SKILL    ← Skill ツール (context: fork)。別 context が起動。SKILL.md は fork 先が Read
  - 汎用 Agent       ← Agent ツール (subagent_type: general-purpose / Explore / Plan)
  - カスタム Agent   ← Agent ツール (subagent_type: <カスタム名>)
  - Bash subprocess  ← Bash で外部プロセス起動 (codex exec 等)

「subagent」の用法統一:
  - SKILL.md / 設計書では「subagent」単独使用を避ける
  - fork 型 SKILL の隔離 context を指す場合 → 「fork 型 SKILL」と書く
  - Agent ツールで立てたビルトインを指す場合 → 「汎用 Agent」または具体名 (general-purpose / Explore / Plan)
  - Agent ツールで立てたカスタムを指す場合 → 「カスタム Agent」または具体名 (例: claude-code-guide)
...
- §3.3 の最低限の内容 (起動経路 5 種の短縮名称 / 「subagent」用法統一 / Issue #32 系誤読の説明) を含む
...
| ---- | --------------------------------------------------------- | ------------ | ----------------------- | ---------------------------------------- |
| A    | 汎用 Agent (general-purpose) で SKILL.md を Read して起動 | 消費しない   | 不要                    | **第 1 推奨**                            |
| B-1  | 継承型 SKILL として Skill ツールで呼ぶ                    | **消費する** | 不要                    | 非推奨 (※)                               |
| B-2  | fork 型 SKILL として Skill ツールで呼ぶ                   | 消費しない   | **§4 追加が必要**       | 第 2 候補                                |
| C    | Bash subprocess として起動                                | 消費しない   | 不要                    | reviewer の Codex エンジン以外では非該当 |

※ B-1 は fixer の「メインコンテキスト消費を抑える」設計原則と衝突するため非推奨。

### 方針 A: `review` orchestrator が 汎用 Agent 起動を担当する [第 1 推奨]
...
- COMMON-DES-001 §4 は **fork 型 SKILL** のリストであり、**継承型 SKILL の Skill ツール呼びには §4 更新は不要**
...
### 方針 B-2: fork 型 SKILL として Skill ツールで呼ぶ [第 2 候補。§4 改訂を伴う]
...
reviewer の Codex エンジン (`run_review_engine.sh` 経由) が該当する既存経路。fixer / evaluator / present-findings に同経路を採用する合理性は本要件のスコープでは見当たらないが、定義文書 (FNC-S008) で経路の 1 種として整理対象に含める。
...
修正経路 (orchestrator 直接 / 汎用 Agent 起動 / 継承型 SKILL / fork 型 SKILL / Bash subprocess) の分岐条件・責務境界を **1 箇所に整理された表** で記述する。最有力候補は `docs/specs/forge/design/DES-015_review_workflow_design.md` への追記 (FNC-S007)。方針選定 (A / B-2) によって表に載る経路の組み合わせが変わる。
...
- COMMON-DES-001 §4 のリストに対象 SKILL (fixer / reviewer / evaluator / present-findings のうち fork 型化したもの) が **fork 採用根拠つきで追加** されている
