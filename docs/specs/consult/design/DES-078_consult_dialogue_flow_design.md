...
| 記録の置き場・識別名                            | 固定パス・固定識別名（[DES-075](../agenda/design/DES-075_agenda_mechanism_design.md) §7「review 起点」の行）                                                                                                                    | 固定パス・固定識別名（同 §7「consult 起点」の行）。起点ごとに値が違うだけで、どちらも同時に 1 つである |
...
| ID      | 内容                                         | 状態                                                                                                                                                                                                                                                                      |
| ------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TBD-001 | 既存記録を確認する範囲（どこまで走査するか） | **決着**: 論点が消滅した。記録は起点ごとに固定パスの 1 つだけであり（[DES-075](../agenda/design/DES-075_agenda_mechanism_design.md) §7）、確認は固定パスの存在確認 1 回で足りる。走査する対象が無い。複数同時を導入する場合は同 §7「個数を 1 つに置く理由」から再検討する |
