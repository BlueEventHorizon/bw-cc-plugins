...
本文書の内容は、Artifact での配色実験・`file://` 環境での動作検証を経て確定した。検証で判明した事実と、その結果として採った設計判断を区別して記す。
...
- **検証済み**: 本セッションで実際に `open <path>.html` を実行し、ローカルファイルがブラウザで開くことを確認した
- **理由**: 「AI が必要なタイミングで人間に表示する」という要求は、`Artifact` や `SendUserFile` のような専用の提示手段を要すると当初考えたが、OS 標準の `open`（macOS）1 コマンドで足りる。既存の「サーバー不要・`file://` 前提」の設計方針を変えずに満たせる
...
**検証結果を踏まえた判断**: 当初、変更箇所（FNC-002）・対話中の項目の強調は、カード全体の背景色 + 左ボーダーで表現する案で検討したが、Artifact での配色実験を経て、**カード左側の余白（ガター）に置く小さなドット 1 点に色を集約する**方式へ変更した。
...
**方針転換**: 当初、重大度（`critical`/`major`/`minor` 等）は絵文字（🔴🟡🟢）で表現し、CSS では追加の色分けをしない方針としていたが、これは誤りだった。絵文字は「形が同じ円で色だけが違う」ため、**色以外の手がかりを持たない表現**であり、[WCAG 2.2 達成基準 1.4.1「色の使用」](https://www.w3.org/TR/WCAG22/#use-of-color)が要求する「色を、情報を伝える・操作を示す・応答を促す・視覚要素を区別するための唯一の視覚的手段として使用しない」に反する。
...
### 4.1 検証結果: `<script src>` タグの動的差し替え

`fetch`/`XHR` は `file://` オリジンで CORS 制約によりブロックされるが、**`<script>` タグの `src` 読み込みはこの制約を受けない**。本セッションで以下を実際に検証した。

- `agenda.js`（`window.AGENDA_DATA = {...}` を代入するだけのファイル）を用意し、ページ内 JS が 2 秒ごとに新しい `<script src="agenda.js?t=<timestamp>">` を生成して差し替える
- `agenda.js` の内容をエディタで書き換えて保存すると、**ページをリロードせずに 2 秒以内に新しい値が反映される**ことを実機（`file://` で開いたブラウザ）で確認した

これにより、ページ全体を再読み込みしない部分更新が `file://` 環境でも技術的に可能であることが分かった。

### 4.2 採用方式: 状態フラグは即時反映、本文の変化は全体再読み込みに委ねる

検証した部分更新は、**軽量な状態フラグ（今どの項目が対話中か、どの項目が直前に変わったか）の反映には使えるが、本文（背景・本質・決着等のテキスト）や項目数の変化まで部分更新で扱うのは複雑になる**（新規項目の追加は、あらかじめ HTML 骨格に存在しない要素をどう生成するかという問題を生む）。したがって、次のハイブリッド方式を採る。
...
§4.2 の手順 3（`location.reload()`）が発生したときに限り、ページ全体の再読み込みでスクロール位置が失われる。これを防ぐため、`pagehide` 時にスクロール位置を `sessionStorage` へ保存し、読み込み後に復元する（本セッションで実装・動作確認済み）。
...
- **手動検証観点**（ブラウザの実際の挙動に依存し、単体テストで機械的に保証できない事項）:
  ...
  - `<script src>` 差し替えによる部分更新が主要ブラウザで動作すること（本セッションでは 1 ブラウザでの検証に留まる）
  - スクロール位置保持が実際に機能すること

## 6. 実験の記録

本設計は、Artifact での配色実験（カード全体ハイライト → ガタードット方式への変更）と、ローカル `file://` 環境での動作検証（`open` コマンドによる表示、`<script src>` 差し替えによる部分更新、スクロール位置保持）を踏まえて確定した。これらの実験用ファイルは一時的な検証物であり、本リポジトリには含めない。
