...
```html
<main><!-- 行長を抑えるコンテンツ幅コンテナ -->
<header>
<h1>{identity}</h1><!-- 「アジェンダ」の接頭ラベルは CSS（h1::before）が付ける -->
...
<section id="item-01" data-changed="{is_last_changed}"><!-- FNC-005: アンカーリンク到達点。項目カード -->
  ...
  <dl><!-- ラベル列（チップ）と本文列を分離し、複数項目を縦走査で比較できるようにする -->
    ...
    <dt>背景</dt><dd>...</dd>
    <dt>本質</dt><dd>...</dd>
    <dt class="label-recommend">推奨</dt><dd>...</dd><!-- items[].recommendation。空なら本行を出力しない -->
    <dt class="label-decision">決着</dt><dd>...<!-- 未定は <span class="undecided">(未定)</span> --></dd>
  </dl>
</section>
</main>
```
...
| セレクタ                                         | 適用対象                                              | 備考                                                                                                                                                                                                                |
| ------------------------------------------------ | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `body`, `main`                                   | 全体のベースフォント・行間・背景色・コンテンツ幅      | 落ち着いたトーンとする（きつい原色は避ける）。ワイド画面で行長が伸びすぎないよう本文幅を制限する                                                                                                                    |
| `h1`, `h2`                                       | タイトル階層（アジェンダ全体タイトル / 項目タイトル） | 階層が視覚的に区別できる程度の差（フォントサイズ・太さ）。「アジェンダ」の接頭ラベルは `h1::before` が付ける                                                                                                        |
...
**開いているブラウザタブは、記録の更新を検知して自動的に全体を再読み込みする。**利用者に手動リロードを求めない（`record` は議論の進行中に繰り返し発生する操作であり、その度の手動リロードは agenda:REQ-021 FNC-002 の趣旨「利用者に差分を探させない」を実運用で損なうため）。

### 4.1 技術的根拠: `<script src>` タグの動的差し替え

`fetch`/`XHR` は `file://` オリジンで CORS 制約によりブロックされるが、**`<script>` タグの `src` 読み込みはこの制約を受けない**。この性質を用いて、サーバーを立てずにファイルの内容変化をページへ伝える（実機の `file://` 環境で動作検証済み）。

### 4.2 採用方式: 世代番号の比較による全体再読み込み

`agenda_render.py` は書き込み成功時（[DES-075](DES-075_agenda_mechanism_design.md) §8.1 のトリガー）に、次の 2 ファイルを毎回まるごと再生成する。

| ファイル          | 内容                                                                                                                 |
| ----------------- | -------------------------------------------------------------------------------------------------------------------- |
| `agenda.html`     | 全項目の完全な内容（§3 のテンプレート）+ 自動追従スクリプト（生成時点の `content_version` を世代番号として埋め込む） |
| `agenda_state.js` | `window.AGENDA_STATE = {"contentVersion": <整数>};` のみを持つ軽量ファイル                                           |

`agenda.html` に埋め込む固定スクリプトは次のように振る舞う。

1. 数秒ごとに `agenda_state.js` を §4.1 の手法（`<script src="agenda_state.js?t=<timestamp>">` の差し替え。クエリはブラウザキャッシュ回避のため）で読み込む
2. 読み込んだ `contentVersion` が、自ページに埋め込まれた世代番号と**同じ**なら何もしない。**異なる**なら `location.reload()` でページ全体を再読み込みする
...
§4.2 の `location.reload()` でスクロール位置が失われることを防ぐため、`pagehide` 時にスクロール位置を `sessionStorage` へ保存し、読み込み後に復元する。`sessionStorage`・`window.scrollTo` は**ページ自身の中だけで完結する API** であり、`file://` の CORS 制約に抵触しない。`sessionStorage` へのアクセスが例外を投げる環境でもページ表示自体は成立させる（try/catch で包む）。
...
- **手動検証観点**（ブラウザの実際の挙動に依存し、単体テストで機械的に保証できない事項）:
  ...
  - `record` 実行後、開いているタブが数秒以内に自動で再読み込みされ、スクロール位置が保持されること（§4.2・§4.3）
