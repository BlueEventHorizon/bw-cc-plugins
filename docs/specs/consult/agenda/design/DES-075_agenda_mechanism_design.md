...
保存形式は JSON（標準ライブラリ `json` のみ）を採用する。PyYAML は外部依存のため採らず、TOML は標準ライブラリで書き込みができないため採らない。

既存の `consult` 実装からの移行手順・フェーズ分割は本設計書の範囲外である（時間軸・順序を含む記述は設計書ではなく実装戦略の責務）。実装完了後に削除される ephemeral 文書（実装戦略書）を恒久文書である本設計書から参照しない。
...
| `config.identity`             | 記録の識別名。`agenda_store.py` が `path` 引数の**親ディレクトリ名**から機械的に導出する（`Path(path).parent.name`）。起点の判定を別途行わない——`path` は呼び出し側 script（`agenda_wrapper.py`）が起点から絶対パスとして解決する値であり、ファイル名は起点を問わず常に`agenda.json`（[DES-077](DES-077_agenda_display_design.md)が前提とする固定名）、親ディレクトリ名だけが起点で変わる（review 起点は`.claude/.temp/review/agenda.json`→識別名`"review"`、consult 直接利用は`.claude/.temp/consult/agenda.json`→識別名`"consult"`。§7参照）。呼び出し側は `identity` を組み立てて渡さない           | FNC-009・NFR-003        |
...
| `consult` 起点（review 経由でない直接利用） | 1 回の consult 実行全体（起動から終了処理まで） | consult も同時に 1 つの記録しか持たない（§7）。記録の生存期間（`start`〜`finish` での削除）と一致する範囲を集合の単位とする                       |
...
| `consult` 起点（review 経由でない直接利用） | `.claude/.temp/consult/agenda.json`（固定パス） | 同時に 1 つ（下記「個数を 1 つに置く理由」）                                                           | 親ディレクトリ名 `consult`（固定）。`agenda_store.py` が wrapper から受け取った `path` から機械的に導出する | 全項目で `decision.by`・`decision.outcome`・`decision.reason` がすべて非空になった時点で削除する（§5.1・`finish` が判定）              |
...
**両起点とも固定名・箱なしである**。「同時に 1 つしか成立しないなら識別のための名前も、列挙のための境界も要らない」（NFR-003 導出例）に素直に従う。

### 個数を 1 つに置く理由（`consult` 起点）

NFR-003 のもう一方の導出例（「複数が同時に成立するなら、**名前が主題を持ち**、列挙のための境界が要る」）を `consult` 起点へ適用しない。複数同時を採るには**主題を表す名前**が要るが、それを支える材料も必要も現時点で無い。

- **並行させたい単位は主題であって、実行の単位ではない**。実行の単位で名前を分けても、1 回の consult の中で別主題の議論を並行させることはできない。分かれるのは実行境界だけであり、要求されている「別主題の並行」は満たされない
- **主題名は誰かが作らなければならない**。agenda への入力境界は「AI は置き場も `config` も組み立てない」（§6）であり、主題名を導入するなら wrapper が 1 引数として受け取る形が要る。実運用で複数同時が必要だと分かってから設計する
- **識別名は提示に出る**。`config.identity` は `agenda.html` の見出しとタブ名になる（[DES-077](DES-077_agenda_display_design.md) §3）。主題を持たない機械的な識別子を名前に採ると、読み手にとって意味を持たない見出しが出る
- **探す先が 1 箇所に定まる**。起動時に残った記録を確認する操作（consult SKILL.md）が、固定パスの存在確認 1 回で済む。複数を許すと、どこまで走査するか・他の実行が進行中の記録をどう扱うかという判断が新たに要る

複数同時の必要性が実運用で確認された時点で、主題名の導入とあわせて再検討する。
