...
| 4    | `mark_fixed.py {session_dir} {id} {files_modified}` で plan.yaml を `status: fixed` に更新                |

複数件 (2〜3 件) の場合は finding ごとに上記サイクルを順に回す。全件完了後、呼び出し元 (review / present-findings) は通常の終了処理 (単独修正レビュー / 終了サマリ等) に進む。
...
| 2    | fixer が refs.yaml / plan.yaml / `review_<種別>.md` を Read し、subagent に修正を委譲      |
...
| 2026-05-21 | 3.7        | **TASK-032〜036 完了 (GROUP-001 1/5〜5/5)**: addendum 4 ファイル (spec_priorities / spec_design_boundary / design_principles / plan_principles) を本体 principles に merge 完了し、`docs/specs/forge-review/principles/*_addendum.md` 4 件と principles ディレクトリを削除。これに伴い `plugins/forge/docs/review_priorities_spec.md` L20 / L76 の「addendum merge 完了までの一時参照」記述も削除し、merge 後の正規 SoT 経路 (`plugins/forge/docs/*_spec.md`) を明示。§5.1 / §5 本文 (merge 戦略表) は merge 経路の歴史的記録として温存。本 feature (forge-review v0.2 / Issue #68 系) の主要実装は完了し、残課題は最終 AI レビューのみ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-05-24 | 3.8        | **FNC-413 軽量修正経路の差分設計を追加 (Issue #86 起点)**: (1) §4.5 fixer SKILL 表に「軽量経路除外」行を追加 (軽量経路では fixer が起動されない / 本体不変)。(2) §4.5 末尾に「軽量修正経路 (FNC-413) — fixer 起動前の判定設計」サブセクションを新設し、判定アルゴリズム (plan.yaml 集計のみ、`review_<種別>.md` Read 禁止)、軽量経路の手順 (`mark_in_progress.py` → `Edit` → `mark_fixed.py`)、fixer 経路 (変更なし)、除外規定 (`--diff-only` / 修正方針提示後の `--single` は常に fixer 経路)、単独修正レビューとの関係 (軽量経路 / fixer 経路にかかわらず実行) を明文化。(3) §4.1 review SKILL 表に「Phase 5 軽量経路判定 (FNC-413)」行を追加 (`--auto-critical` / `--auto` で 3 件以下 + 全 `auto_fixable: true` なら軽量経路に分岐)。(4) §4.4 present-findings SKILL 表に「軽量経路判定 (FNC-413)」行を追加 (「段階的に解決」は件数=1 + `auto_fixable: true` で軽量経路 / 「✅を一括修正」は ✅付き項目 3 件以下で軽量経路 / ユーザー修正方針提示後は常に fixer 経路)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
