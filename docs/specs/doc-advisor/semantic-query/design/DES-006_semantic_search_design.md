...
doc-advisor の文書検索（query-specs / query-rules）を、AI が index YAML を全量読み込む方式から、**Embedding ベースのセマンティック検索 + 全文検索スクリプト** に置き換える。
...
| 設定・差分検出 | .doc_structure.yaml の読み込み、チェックサム管理、ファイル列挙 | `index_utils.py`（既存・再利用） |
...
| モジュール | ファイルパス | 責務 | 依存 |
| ---------- | ------------ | ---- | ---- |
| `embed_docs.py` | `plugins/doc-advisor/scripts/embed_docs.py` | Embedding インデックス構築（全体・差分） | `index_utils.py`, `urllib`（標準） |
| `search_docs.py` | `plugins/doc-advisor/scripts/search_docs.py` | セマンティック検索実行 | `index_utils.py`, `urllib`（標準） |
| `grep_docs.py` | `plugins/doc-advisor/scripts/grep_docs.py` | 全文検索（テキストマッチング） | `index_utils.py` |
| `index_utils.py` | `plugins/doc-advisor/scripts/index_utils.py`（既存） | 共通ユーティリティ | 標準ライブラリのみ |
...
sequenceDiagram
    ...
    participant Utils as index_utils.py
...
`index_utils.py` の `calculate_file_hash()` と `load_checksums()` を使用した差分検出を行う。
...
```
.claude/doc-advisor/indexes/{category}/{category}_index.json
```

既存の index YAML と同じディレクトリに配置する。
...
1. `index_utils.init_common_config(category)` で設定を読み込む
2. `index_utils.get_all_md_files()` で対象ファイルを列挙
...
| コンポーネント | ファイルパス | 用途 |
| -------------- | ------------ | ---- |
| `load_config()` | `plugins/doc-advisor/scripts/index_utils.py` | `.doc_structure.yaml` の読み込み |
