...
| index 出力先 | `doc-advisor/rules/`                     | `doc-advisor/indexes/rules/`            |
...
- **識別子ベースの保護**: プラグイン更新時、旧バージョンのファイルはプラグインマネージャーが差し替える。ユーザーのワークスペース内にコピーされた管理ファイル（index、チェックサム等）は識別子で判定する
- **ユーザー資産の保護**: プラグインが管理するディレクトリ外のユーザーファイルには一切触れない。ランタイム出力（index、チェックサム、作業ディレクトリ）はアップグレード時に保持する
...
- `.claude/doc-advisor/indexes/rules/`（ランタイム出力: index、チェックサム、作業ディレクトリ）
- `.claude/doc-advisor/indexes/specs/`（ランタイム出力: index、チェックサム、作業ディレクトリ）
...
### REQ-002-NF-01: index ファイルの保持

**説明**: 既存の index ファイルはアップグレード時に削除しない

**受入条件**:

- [ ] `doc-advisor/indexes/rules/rules_index.yaml` は削除されない
- [ ] `doc-advisor/indexes/specs/specs_index.yaml` は削除されない
- [ ] 差分更新が可能
...
| T-007 | index ファイル保持              | 既存 index がアップグレード後も保持される              |
