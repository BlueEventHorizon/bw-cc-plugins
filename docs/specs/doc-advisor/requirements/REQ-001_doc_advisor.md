...
| FR-01-7 | index の各エントリにはファイルパスから判定した `doc_type` フィールドを自動付与する                               |
...
| FR-02-3 | システムは抽出したメタデータを index ファイルに出力する            |
...
| FR-03-2 | システムはチェックサムファイル (`.index_checksums.yaml`) を管理する |
...
| ID      | 要件                                                                                                           |
| ------- | -------------------------------------------------------------------------------------------------------------- |
| FR-05-1 | `/query-rules` skill は index を検索してタスクに関連する開発ドキュメントを特定する                               |
| FR-05-2 | `/query-specs` skill は index を検索して関連する仕様ドキュメントを特定する                                       |
...
| FR-07-3 | 各スキル（create-*-index, query-*）の先頭で `check_doc_structure.sh` を実行し、`.doc_structure.yaml` の対象カテゴリの `root_dirs` 設定状態を検査する |
...
| ID      | 要件                                                                                                                                                                                     |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FR-08-1 | `.doc_structure.yaml`（文書構造）とコードデフォルト（Doc Advisor 内部設定、`index_utils.py` で定義）がランタイム設定を構成する                                                             |
...
| NFR-02-3 | 並列処理数はコードデフォルトで定義する（`index_utils.py`）                                                                                                                          |
| NFR-02-4 | `.doc_structure.yaml` は `root_dirs`（配列）、`doc_types_map`（辞書）、`patterns`（辞書: target_glob, exclude）の3フィールドで文書構造を定義する                                     |
| NFR-02-5 | Doc Advisor 内部設定（checksums_file, output, common）はコードデフォルト（`index_utils.py` の `_get_default_config()`）で定義し、`.doc_structure.yaml` には含めない |
...
| index               | ドキュメントの検索インデックスファイル（Embedding ベース）                                                                            |
