...
| NFR-02-5 | Doc Advisor 内部設定（toc_file, checksums_file, work_dir, output, common）はコードデフォルト（`index_utils.py` の `_get_default_config()`）で定義し、`.doc_structure.yaml` には含めない |
