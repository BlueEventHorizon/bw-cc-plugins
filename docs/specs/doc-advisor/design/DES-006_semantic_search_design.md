...
**設計判断**: auto モードでは **Index を先に実行** する。`embed_docs.py` の auto-build で最新化を行い、また 100 件超の ToC では Index 候補を縮小 ToC のフィルタとして利用するため。
...
auto（デフォルト）:
  Step 1: Read query_index_workflow.md → Index 候補パス取得（search_docs.py --threshold 0.2、失敗時 = 空）
  Step 2: Read query_toc_workflow.md → ToC 候補パス取得
          - ToC エントリ ≤ 100 件 → 全量 Read（従来動作）
          - ToC エントリ > 100 件 → Step 1 の Index 候補を {filter_paths} として filter_toc.py に渡し、
                                   抽出された縮小 YAML を Read（Filter Procedure）
          - >100 件かつ Index 候補が空 → ToC 全量 Read にフォールバック
          - ToC 不在時 = 空
