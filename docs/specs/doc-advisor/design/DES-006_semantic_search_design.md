...
- auto モードは **doc-advisor 単独完結** とする。`context: fork` の subagent は doc-advisor 内蔵の ToC / Embedding 検索パイプラインのみを fork 内で実行し、**他プラグインの SKILL（特に `/doc-db:query` / `/doc-db:build-index`）は呼ばない**。検索バックエンドの選択（doc-db / doc-advisor）は **forge 側の `/forge:query-db-*` 抽象 SKILL** が `plugins/forge/scripts/backend_selection/select_backend.py` で行う責務であり、doc-advisor 側で二重判定しない（forge:DES-001_forge_query_abstraction_design §1.2 / §2.3）。
