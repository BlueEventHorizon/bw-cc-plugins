...
```json
{
  ...
  "index_error": null,
  "warnings": [
    "doc_type=adr matched no directory",
    "missing paths: 2 entries skipped"
  ],
  "missing_paths": [
...
品質テスト（NFR-001 達成判定 / `--doc-type` 指定クエリでの false negative ゼロ再測定 / Embedding / ToC / 統合の 3 方式測定）は**本 feature のスコープ外**であり、別 feature `quality-test-doc-type-adr`（仮称）として独立した plan で扱う。

**スコープ外の根拠**:

- forge `plan_principles_spec.md` のタスク粒度原則（1 Agent 実行で完結 / ビルド・テスト合格で完結性を担保）に収まらない（測定 → 評価 → 設計再検討の波及ループを含むため）
- 測定対象の `meta/test_docs/` は git 管理外のローカル運用（CLAUDE.md 規定）であり、plan ドリブンの実装作業と運用境界が異なる
- 測定の合否判定軸（false negative 件数 / コスト削減率）は本 feature の実装タスク完了判定（ビルド・テスト合格）と独立

**測定要件**: 測定対象 / 測定条件 / 合格判定は FNC-004 §性能・コスト要件で確定済み。別 feature はこれを引用して plan を立てる。

**別 feature 起票手順**: 本設計書の改定履歴 v1.11 を参照。
...
| 2026-05-04 | 1.11       | Issue #34 起因の文言明確化（add-adr plan レビュー id=1 false positive 起因）。§11.4 を「品質テストは本 feature スコープ外、別 feature `quality-test-doc-type-adr`（仮称）として独立した plan で扱う」と書き換え、曖昧表現「後続フェーズ」を排除。スコープ外の根拠（forge `plan_principles_spec.md` のタスク粒度原則、`meta/test_docs/` git 管理外運用、合否判定軸の独立）を明示。**別 feature 起票手順**: (1) `feature/quality-test-doc-type-adr` ブランチを `develop` から分岐、(2) `docs/specs/doc-advisor/requirements/FNC-XXX_quality_test_doc_type_adr_spec.md` で要件定義（測定対象・条件・合格判定は FNC-004 §性能・コスト要件を引用、本 feature merge 後に確定済み）、(3) `docs/specs/doc-advisor/design/DES-XXX_quality_test_doc_type_adr_design.md` で測定手順を設計（Embedding / ToC / 統合 3 方式の独立測定、`meta/test_docs/` ゴールデンセット拡張内容を含む）、(4) `docs/specs/doc-advisor/plan/quality-test-doc-type-adr_plan.yaml` で plan 化。plan TASK には `meta/test_docs/` 実体更新は含めず（git 管理外）、ゴールデンセット拡張定義 / 測定スクリプト整備 / 結果サマリ markdown 作成のみを TASK 化する。(5) 評価結果が DES-026 / FNC-004 の設計影響を及ぼす場合のみ本書および FNC-004 に改定履歴で追記。背景は Issue #34 / `add-adr_plan.yaml` id=1 skip_reason 参照                                                                                                                                  |
