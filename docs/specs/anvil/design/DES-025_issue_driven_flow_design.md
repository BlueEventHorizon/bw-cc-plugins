...
- forge スクリプト（`resolve_doc_structure.py`, `next-spec-id` 等）は anvil から直接 import / subprocess 呼び出ししない。常に forge skill を Skill tool 経由で起動し、結果は Skill ツールの返り値として受け取る（§6.3）。
...
**push 責務**: push の実行は forge skill の責務（forge skill 内部で `/anvil:commit` を呼び push まで完了）。impl-issue は forge skill 完了後に `verify_pushed` で origin との一致を機械的に確認するに留める。push 失敗時は impl-issue が §8.2 表「forge skill が非ゼロ終了」の経路で扱い、`pending_push` 状態（commit 成功後の push 段階失敗等）からの復帰は §7 状態管理 / §8.2 の `--resume-push <phase>` で再開する。**本項の forge 側前提は現在の実装と一致していない**（§6.3 の「既知の乖離」を参照）。
...
### 6.3 forge skill との連携契約

- **forge skill 成果物パス取得の公式経路**: forge skill は **ファイルベースの出力契約を持たない**。impl-issue は Skill ツールで forge skill を起動し、その **返り値（完了報告）に記載された成果物パス**を取得する。forge skill の内部スクリプト・内部状態への直接アクセスは NFR-01 / I-01 違反となるため禁止する。
- **返り値が唯一の受け渡し面である帰結**: 成果物パスは機械的に検証された値ではなく forge skill の自己申告である。impl-issue は受領したパスの実在を `Read` / `Glob` で確認してから工程を `completed` とし、実在しない場合は当該工程を失敗として扱う（§8.2）。
- impl-issue は工程進捗（対象 Issue 番号 / IssueKind / RuntimeKind / 各工程ステータス / 成果物パス）を保持する。保持先は Issue 本文の機械追記セクション（§6.2）および Issue コメントのマーカーブロックであり、ローカルの一時ファイルを正とはしない（セッションを跨いだ再開時に Issue から復元できることを優先する）。
- **保持してよい情報の不変条件**: 書き込んでよいのは「Issue 番号 / 種別 / 工程ステータス / 成果物 path」のみ。以下は保持しない:
...
- 書き戻しの直前には、その時点の HEAD が `git ls-remote origin <branch>` の sha と一致することを §7 の不変条件に従って機械検証する（push 済みであることが工程完了の条件であるため）。検証に使う sha は forge skill から受け取らず、impl-issue 自身がその場で取得する。

> **既知の乖離（2026-07-26 時点）**: 本節は forge 側のセッションプロトコル（`session_dir` / `output.yaml` / `session_manager.py`）が削除されたことを受けて連携契約を再定義したものであり、**forge 側の記述のみ実態と一致している**。anvil 側は以下が未実装であり、本設計が先行している状態にある:
>
> - `verify_pushed.py`（§3.2 / §11 に記載）は存在しない。push 済みの機械検証は行われていない
> - impl-issue はローカル session ディレクトリを作らない。工程状態の実際の保持先は Issue コメントの `<!-- anvil:triage-result:v1 -->` マーカーブロックである
> - §7 / §8.2 が前提とする「forge skill が非ゼロ終了で失敗を返す」は、Skill ツール起動には終了コードが無いため成立しない。失敗は返り値の内容から判断する必要がある
> - 「push は forge skill の責務」（UC-02）も成立していない。forge の `start-implement` は `anvil:commit` があれば委譲するのみで、`anvil:commit` は push を既定で行わない
...
- NFR-01 の「anvil → forge」一方向依存を **物理的に強制** するため。Python import 経由だと anvil 側のテストで forge 内部実装に踏み込みやすく、依存の越境が発生しやすい。Skill 起動経由なら Skill ツールの返り値越しの結合に限定できる。
- forge の内部実装変更（スクリプトの追加・削除・関数シグネチャの変更）が anvil に伝播しない。実際に forge のセッションプロトコルは削除されたが、anvil 側のスキルは影響を受けなかった（§6.3）。
