...
flowchart TD
    ...
    P6b -->|踏襲可| P7c["Phase 7条件3: AIの個別判断量（実測）"]
...
| 高リスク（早期確定） | Phase 7 条件1〜2（実害可能性／サイレント破壊性、Phase 1 直後にほぼ直行可能）に明らかに該当する（`RoutePlan1`、Phase 4〜5 は未実行）                                 | ユーザーに `start-plan` の起動を提案する。該当条件を提示する。Phase 5 は未実行のため、条件3の実測結果は添付しない                                               |
| 高リスク（後段確定） | Phase 7 条件3（AI の個別判断量、Phase 5 の実測結果に依存）に該当すると判定される（`RouteFinal_Risk`、Phase 5 実行済み）                                             | ユーザーに `start-plan` の起動を提案する。該当条件・個別判断が必要なファイルの実測結果を提示に添付する                                                          |
...
Phase 6 判定2（設計判断の残余）・Phase 7 条件3（AI の個別判断量）は Phase 4〜5 の調査結果を判定の直接根拠として要求する（既存の依存関係、§3.2 参照）。この依存は変更せず、**Phase 4〜5 はこれらの判定より前に実行する**（判定確定後に後ろ倒しにしない）。
