...
> [!WARNING]
> 本文書が定める 3 ルート判定（FNC-01 / FNC-02 / FNC-05）、実装工程の入口一本化（FNC-03）、調査結果の引き継ぎ（FNC-04）は [ADR-082](../design/ADR-082_triage_task_based_routing.md) により覆された。現在の仕様は `anvil` の `triage-issue` / `impl-issue` / `impl-ui` 各 SKILL.md が正であり、本文書は改訂されるまで旧仕様として据え置く。
