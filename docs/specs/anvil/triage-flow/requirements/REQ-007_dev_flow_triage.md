...
| UI Issue 判定の分岐点への統合                                     | 別途検討（Issue #188 補足に明記済み）                                                                                                                              |
...
- 要件の固まり具合に応じて、3 つのエントリポイントのいずれから入るべきかが判定され、根拠とともに**提案**される:
  - 要件自体が固まっていない → 要件定義から
  - 要件は明確だが設計判断が未決 → 設計から
  - 要件・設計とも確定済み → 計画から
...
2. 判定調査が行われ、「軽量実装」と判定される（根拠つき）
3. 判定結果と根拠が Issue に記録される
4. 実装工程へ進み、引き継がれた調査結果をもとに実装が行われる（重複調査なし）

**S2. SDD ルート**

1. ユーザーが Issue を指定してトリアージを開始する
2. 判定調査が行われ、「SDD」と判定される（根拠つき）
3. エントリポイント（要件定義 / 設計 / 計画のいずれか）が根拠とともに提案される
4. 判定結果と根拠が Issue に記録される
5. ユーザーが提案されたエントリポイントから既存の SDD フローを開始する（以降は既存フローに従う）
