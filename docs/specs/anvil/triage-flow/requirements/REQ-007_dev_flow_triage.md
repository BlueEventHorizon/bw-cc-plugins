...
本 feature は、Issue を入口とするすべての開発を**単一の分岐点（トリアージ）**で制御し、判定結果に応じてワンショット実装ルートまたは SDD ルートへ振り分ける仕組みを確立する。
...
- Issue の内容に基づく進め方判定（ワンショット実装 / SDD）
...
| 用語             | 定義                                                                                                                      |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| トリアージ       | Issue の内容を調査し、ワンショット実装ルートか SDD ルートかを判定して振り分ける操作。開発フロー全体の唯一の分岐点                 |
| ワンショット実装ルート   | 要件定義書・設計書を起こさず、実装工程（impl-issue 相当）へ直接進む経路                                                   |
...
| リスクゲート     | 要件・設計とも確定した Issue を、リスク（後戻り困難な実害等）を理由にワンショット実装から計画経由へ昇格させる単軸 Yes/No の条件群 |
...
**目的**: Issue の内容から、ワンショット実装ルートと SDD ルートのどちらで進めるべきかを判定する。

**ユーザー視点の振る舞い**:

- ユーザーが Issue を指定すると、判定調査（関連仕様書・ルール文書・類似実装・既存コード）が行われ、ワンショット実装 / SDD のいずれかの判定が根拠とともに提示される
