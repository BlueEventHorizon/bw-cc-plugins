...
```
/forge:review <種別> [--diff | --branch | --files a.md,b.py,... | --dirs d1/,d2/,...] [--interactive | --auto-critical | --auto] [--focus "<重点観点>"] [--scope "<到達目標>"] [--project-rules a.md,b.md] [--project-specs c.md] [--backend <name>]
```
...
| 指定                                            | 動作                                                                                 |
| ----------------------------------------------- | ------------------------------------------------------------------------------------ |
| 指定なし                                        | 候補を順に可用性検査し、最初に使えたものを採用する（現在の候補は `msg-review` のみ） |
...
| `.claude/.forge.yaml` の `review.backend_order` | 候補順を上書きする                                                                   |
...
既定のレビューバックエンド `msg-review` は msg-sys（Claude ⇔ Codex の非同期メッセージング基盤）の上で動作する。以下が必要:
...
**前提が満たされていない場合、依頼は 1 通も送信されない**。バックエンドを確定する前に可用性を検査し、満たせなければ**何が欠けているか（起床手段 / 相手の常駐 / msg-sys の設定）と対処**を並べて報告して終了する（フォールバックしない）。最大 10 分待たされてからタイムアウトで失敗する、ということは起きない。
...
Codex の返信を受け取る処理（Stop フック起点の受信・応答の解釈）は**レビューバックエンド側**に閉じている。本スキルは判定（承認 / 指摘あり / 失敗）と所見の配列を受け取る側であり、通信の詳細を持たない。
...
本スキルは所見の状態をファイルに永続化しない。1 ターン内で受領・評価・修正・返信まで完結させ、往復の履歴はバックエンドへ要求して都度復元する（`msg-review` バックエンドでは msg-sys のメッセージ DB が出所）。

文脈が失われたターン（セッション再開・compaction 後等）でも、差し戻されたメッセージ本文のプロトコルヘッダをトリガーにバックエンドが往復へ復帰し、当該レビューの往復のみを履歴から絞り込んで再開できる。
