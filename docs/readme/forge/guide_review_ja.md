...
- **検出値は本文に載りません**。位置・種別・長さ・先頭数文字だけがマスクして渡されます。依頼は別プロセス・別 AI のレビュアーへ渡り、バックエンドによっては永続化されるため、実値を載せると検出行為そのものが複製経路になるからです
...
| 指定                                      | 動作                                                                                   |
| ----------------------------------------- | -------------------------------------------------------------------------------------- |
| 指定なし                                  | 候補を順に可用性検査し、最初に使えたものを採用する（現在の候補は `agent-review` のみ） |
...
現在のバックエンドは外部依存を持たない `agent-review` だけである。複数の候補があるときに備えて指定と解決の機構は残してあり、**採用したバックエンドと、その決まり方（引数 / 設定 / 候補順）は毎回引数解釈結果に出力される**。ラウンドの実行が失敗した場合も別バックエンドへ自動で切り替えない。

前提条件はバックエンドごとに異なり、満たしていない場合は依頼を1通も送らず不足内容を報告する（fail closed）。候補順の設計根拠・各バックエンドの前提条件の詳細は `review/SKILL.md`、`REQ-013` FNC-1318 を参照。
...
**再開モードは、往復履歴を永続化するバックエンドでしか使えない**。履歴復元は任意拡張であり、現在唯一の `agent-review` は非永続のため対応しない。**したがって再開モードは現時点では動作しない**（往復履歴を永続化する方式の再設計は別途進める）。非対応のバックエンドで中断したレビューは、同一 `review_id` では再開せず、新しいレビューとして実行し直す。
...
往復の履歴が必要な場合はバックエンドへ要求するが、これは**履歴復元に対応するバックエンドに限る**（現在唯一の `agent-review` は非対応であり、この要求は成立しない）。復元は利用者または本体が明示して要求したときだけ行われ、自動で往復へ復帰することはない。
