...
対応しなかった所見を残したまま再レビューを依頼すると、レビュアーが同じ所見を指摘し続けて往復が終わらない。そのため**今回実施する修正が 1 件も無い場合は再レビューを依頼せず完了する**。
...
後者では、修正しなかった所見すべてを理由付きで一覧する（利用者が採用しないと判断したもの / 重大度によるモード範囲外 / 位置が確定せず対象外としたもの / 評価でドロップしたもの / 安全検証で取り消したもの）。人間が見落とさないための必須の区別である。
...
**確認なしに直してよいかを決めるのは、修正内容への確信度である。重大度ではない。**

| 確信度 | 意味                           | `--auto` での扱い  |
| ------ | ------------------------------ | ------------------ |
| ☑️      | 確認済み。実物を見た・実行した | 確認なしに直す     |
| 🤔     | 推論。根拠はあるが未確認       | 提示して採否を聞く |
| (無印) | 未検証。記憶・推測のみ         | 提示して採否を聞く |
