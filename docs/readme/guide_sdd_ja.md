...
既存プロジェクトに新しい feature を追加する、あるいは既存仕様を差分開発として変更するケース。

feature を使うかどうかは **変更の実質** で判定する — 実装の修正に伴い既存の要件定義書・設計書のいずれかに変更が必要になる場合、追加・修正・削除いずれの方向でも feature を使うのが原則である。「バグ修正だから」という名目だけを理由に不要と判定してはならない。「既存要件定義書へ追記したか／新しい要件定義書を作ったか」という文書操作の形式も判定条件ではない。feature を使わずに済むのは、変更規模が小さく **かつ** 旧仕様との一時的矛盾・ID 連番競合・実装完了後の統合が必要な規模のいずれにも該当しない場合に限る（判定基準の詳細は [`additive_development_spec.md`](../../plugins/forge/docs/additive_development_spec.md) §1）。
...
書き換えないのは旧仕様の**文書**（要件定義書・設計書・計画書）である。新機能は**新規ファイル / 新規ディレクトリとして切り出す**が、実装は追加開発の要件定義書に従って修正してよい。既存関数のシグネチャ変更・既存定数の変更も、旧実装を残す義務は無い（6-4 参照）。

#### (3) 実装完了後に旧仕様との齟齬を解消する（merge）

実装が完了して品質が確定したら、旧仕様書との齟齬を解消する。**merge は意味の統合であり、文書の物理的な結合ではない**。新しい仕様を正として旧仕様の記述を正す作業であり、feature の文書を消すことが目的ではない。

旧仕様と**同一スコープ**の内容は旧仕様側へ移し、移し終えて独立して存在する意味を失った文書だけを削除する。**スコープが異なる内容（別 backend・サブシステム相当）は分離したまま維持する**。無理に一体化すると、1 つの文書が複数の関心事を抱えることになる。なお削除した場合も Git 履歴には残るため、過去の feature 開発の経緯は `git log` / `git show` で追える。
...
[実装完了]       → レビュー + 自動修正が完了
   ↓
[merge]          → 旧仕様書の齟齬・不足・誤りを新しい仕様に合わせて正す
   ↓
[仕分け]         → 同一スコープの内容は旧仕様側へ移す / 別スコープの内容は分離を維持
   ↓
[後始末]         → 移し終えて役目を終えた文書のみ削除。残す文書は frontmatter を外して
                    恒久仕様に切り替える。いずれの場合も検索インデックスを再生成
```
...
追加開発の期間中に手を触れないのは、**旧仕様の文書**（要件定義書・設計書・計画書）である。既存設計書・計画書と矛盾が生じても、実装期間中は既存文書を変更しない。

**この制約は実装を縛らない。** 新機能のコードは新規ファイル・新規ディレクトリに作るが、既存コードの修正が必要なら通常どおり修正する。既存関数のシグネチャ変更も、既存定数・既存分岐の変更も、旧実装を残す義務は無い（6-5 のとおり、追加開発の要件定義書に従って実装する）。

旧実装を残すのは、本 feature の外が旧実装の動作に依存していて同時に壊せない場合だけである。その場合も、削除は依存が解消された時点で行う（merge は文書の齟齬を解消する作業であり、コードの後始末は含まない）。

### 6-5. 実装期間中、どちらが現在の仕様か

追加開発の要件定義書・設計書は、その対象範囲における現在の仕様である（[additive_development_spec.md](../../plugins/forge/docs/additive_development_spec.md) §2）。既存の要件定義書・設計書・計画書・ソースコードの記述は、その範囲について現在の仕様ではなく、内容としては追加開発側へ置き換わっている。新要件「ユーザーはゲスト状態でも閲覧可能」と旧要件「全機能は認証必須」があれば、**現在の仕様は新要件**である。旧記述の更新は merge 時に一括で行う。

この関係を「優先」と考えてはならない。優先とすると、矛盾を見つけたときだけ新しい方を取ればよいことになり、矛盾を見落とせば旧仕様のまま実装される。

どの文書が追加開発側かは、実装者が探すのではなく `/forge:start-implement` が機械的に判定してタスクごとに渡す。
...
1. 追加開発の内容を、旧仕様と同一スコープか別スコープかで仕分ける（ファイル単位ではなく内容単位で判断する）
2. 同一スコープの内容を既存の要件定義書・設計書へ反映し、旧記述を正す
3. 追加開発の計画は破棄する（実装済みのため）
