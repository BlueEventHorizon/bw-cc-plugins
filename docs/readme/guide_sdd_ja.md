...
追加開発の要件定義書は、既存の要件定義書・設計書・計画書・ソースコードのすべてに優先する（正本は [additive_development_spec.md](../../plugins/forge/docs/additive_development_spec.md) §2）。新要件「ユーザーはゲスト状態でも閲覧可能」が、旧要件「全機能は認証必須」と衝突したとしても、**新要件を採用して実装を進める**。不整合は merge 時に一括で解消する。
