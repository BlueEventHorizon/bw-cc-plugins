...
- slash 表記: `/anvil:create-pr` 等 (先頭スラッシュ付きの slash command 表記)
- ファイルパス: `plugins/forge/skills/review/SKILL.md` 等

> **注意**: prompt 内に slash command 表記 (`/forge:review` 等) を書くこと自体は、Agent ツールに渡すタスク指示文の中の表現としては **誤読源** であり推奨されない (`subagent_type` の値とは別物であるため)。
