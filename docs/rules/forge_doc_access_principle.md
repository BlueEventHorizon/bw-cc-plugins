...
| 経路            | 対象                                                                                                         | 方式                                                        |
| --------------- | ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------- |
| **A. 直接参照** | forge 内蔵 docs (`plugins/forge/docs/*`、または参照元スキル自身の `plugins/forge/skills/{同一skill}/docs/*`) | パスを直接書く: `${CLAUDE_PLUGIN_ROOT}/docs/xxx.md` を Read |
| **B. クエリ**   | プロジェクト固有 spec / rules (`docs/specs/`, `docs/rules/`)                                                 | `/forge:query-db-specs` / `/forge:query-db-rules` を呼ぶ    |

**Route A の範囲に注意**: `plugins/forge/skills/{skill}/docs/*` を直接参照してよいのは、**その skill 自身** から `${CLAUDE_SKILL_DIR}/docs/xxx.md` で参照する場合のみ。**別の skill から他 skill の `docs/*` を直接参照してはならない**（`docs/rules/implementation_guidelines.md`「スクリプト・文書の配置」参照）。複数 skill が必要とする文書は `plugins/forge/docs/*`（プラグイン共有）へ昇格する。
