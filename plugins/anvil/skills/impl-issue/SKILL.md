...
- **非 UI Issue** → Issue に記載した実装計画の TODO に沿って順番に実装を進める。**実装完了後は Phase 13 → Phase 14 へ進む**
...
- 指摘なし: そのまま Phase 14 へ進む
```

レビュー結果（指摘件数・対応有無）は Phase 14 の commit メッセージ・PR 本文に簡潔に反映する。
...
- [Phase 13: UI 実装レビュールール（UI Issue 経路のみ）](references/phase-13-ui-review.md)
