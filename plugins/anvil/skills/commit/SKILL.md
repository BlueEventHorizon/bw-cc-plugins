...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/check_hook.sh" prepare-commit-msg && echo "hook_exists"
```
...
```bash
ISSUE_REF=$("${CLAUDE_PLUGIN_ROOT}/scripts/extract_issue_ref.sh")
```
...
| `issue981`                 | `981`（最初の数字列）                             |
| `feature/fix-login/detail` | `fix-login`（数字なし → 第2セグメント）           |
| `feature/no-slash`         | `no-slash`（数字なし → 第2セグメント）            |
| `main`                     | `main`（数字なし・セグメント1つ → 第1セグメント） |

detached HEAD の場合は空文字を返し、issue 参照なしで続行する。
