...
## プロジェクトの Figma ファイル構成

| ファイル名                  | fileKey                  | 用途                                      |
| --------------------------- | ------------------------ | ----------------------------------------- |
| DaytonaPark_APP_Design_共有 | `Yg2pMkry4klPFTMHxcM63F` | デザイン（メイン）— UI 実装はこちらを参照 |
| アプリワイヤー              | `AgUnNptCnqo85esjicl2ne` | 仕様・WF 参照                             |

**デフォルト fileKey**: `Yg2pMkry4klPFTMHxcM63F`（デザインファイル）
...
3. fileKey が不明な場合:
   - デフォルト: デザインファイル `Yg2pMkry4klPFTMHxcM63F` を使用
   - ワイヤーフレーム参照が必要な場合: `AgUnNptCnqo85esjicl2ne` を使用
...
```yaml
nodeId: "6467:232879"
figmaUrl: "https://www.figma.com/design/Yg2pMkry4klPFTMHxcM63F/DaytonaPark_APP_Design_共有?node-id=6467-232879"
frameName: "ei-gum_ホームタブ画面_LIMITED ITEMSタブ"
frameSize: "390x812"
fileKey: "Yg2pMkry4klPFTMHxcM63F"
```
