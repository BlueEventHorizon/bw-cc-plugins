...
# that tells Claude to run /forge:setup-doc-structure first.
...
[[ ! -f "$CONFIG" ]] && {
    echo "[ACTION REQUIRED] Doc Advisor: .doc_structure.yaml not found. Run /forge:setup-doc-structure skill to create document structure configuration. This must be completed before document search or ToC generation will work. If in plan mode, run /forge:setup-doc-structure after exiting plan mode."
    exit 0
}
...
# Not configured → warn
echo "[ACTION REQUIRED] Doc Advisor: Document directories are not configured${CATEGORY:+ for '$CATEGORY'}. Run /forge:setup-doc-structure skill to auto-detect and configure document directories. This must be completed before document search or ToC generation will work. If in plan mode, run /forge:setup-doc-structure after exiting plan mode."
