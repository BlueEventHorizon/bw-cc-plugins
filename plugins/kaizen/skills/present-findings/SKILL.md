...
3. ユーザーの選択に応じて:
   - **修正を選択**（A案/B案）→ `/kaizen:fix-findings --single` を呼び出し、修正を委譲（後述「/kaizen:fix-findings 呼び出し時の責務」参照）
   - **このまま（対応しない）** → Step 5 へ
   - **一覧に戻る** → Step 2 へ
4. fix-findings の修正サマリーをユーザーに報告
5. 次の項目へ進む
...
1. ✅付き項目を収集し、`/kaizen:fix-findings --batch` を呼び出し、修正を委譲（後述「/kaizen:fix-findings 呼び出し時の責務」参照）
2. fix-findings の修正サマリーをユーザーに報告（項目ごとに何をしたか1行ずつ）
...
## /kaizen:fix-findings 呼び出し時の責務 [MANDATORY]

fix-findings に修正を委譲する際、以下を**漏れなく**渡すこと：
...
> fix-findings の入力仕様の詳細は `${CLAUDE_PLUGIN_ROOT}/skills/fix-findings/SKILL.md` を参照。
...
| fix-findings がエラーを返した | エラー内容をユーザーに報告し、手動対応するか確認（AskUserQuestion） |
