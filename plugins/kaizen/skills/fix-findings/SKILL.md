...
description: |
  ...
  /kaizen:present-findings からの対話的修正、/kaizen:review --auto-fix からの自動修正で使用。
...
| `$ARGUMENTS` | モード | 用途 |
|--------------|--------|------|
| `--single` | 1件修正 | /kaizen:present-findings の「段階的に解決」で 1 件ずつ修正 |
| `--batch` | 一括修正 | /kaizen:present-findings の「✅を一括修正」、/kaizen:review の「--auto-fix」 |

### 呼び出し元から受け取る情報 [MANDATORY]

呼び出し元（/kaizen:present-findings または /kaizen:review）は、以下を**漏れなく**提供する責務を持つ：
...
> 呼び出し元（`/kaizen:review` または `/kaizen:present-findings`）が事前に存在確認している前提で動作する。
...
subagent の修正サマリーを呼び出し元に返す。呼び出し元（/kaizen:present-findings または /kaizen:review）がユーザーへの報告を担当する。
