...
- `status: "error"` → 通常は Phase 1 step 2 で解決済み。タイミング問題等で発生した場合は `/kaizen:init-kaizen` を起動し `.doc_structure.yaml` の作成を促す。作成後にスクリプトを再実行
...
`/kaizen:fix-findings --batch` を呼び出し、🔴問題を修正:
...
`/kaizen:present-findings` Skill にレビュー結果ファイルを渡して呼び出す:

```
/kaizen:present-findings .claude/.temp/review-result-{timestamp}.md
```

`/kaizen:present-findings` が段階的・対話的に結果を提示し、修正アクションを処理する。

#### --auto-fix モード

`/kaizen:present-findings` は呼び出さない。サマリーを報告する:
