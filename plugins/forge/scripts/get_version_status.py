    ...
    while i < len(lines):
        ...
        if in_targets:
            if indent == 2 and content.startswith("- name:"):
                current_target = {
                    "name": _strip_quotes(content.split(":", 1)[1]),
                    "sync_files": [],
                    "scope": [],
                    "exclude": [],
                }
                ...
                if sublist_mode == "sync_files" and content.startswith("- path:"):
                    ...
                    current_target[sublist_mode].append(_strip_quotes(content[2:]))
            elif indent == 8 and current_sync is not None and sublist_mode == "sync_files":
                key, _, val = content.partition(":")
                current_sync[key.strip()] = _strip_quotes(val)

        elif in_changelog:
            key, _, val = content.partition(":")
            changelog[key.strip()] = _strip_quotes(val)

        elif in_git:
            key, _, val = content.partition(":")
            git_config[key.strip()] = _strip_quotes(val)
