...
artifact 書き込みと session meta 更新の順序を集約する。
"""
...
class SessionStore:
    ...
    def write_text(self, relative_path, content, meta=None, atomic=True):
        """セッション配下へテキストを書き込む。

        書き込み成功後に meta が指定されていれば session.yaml を更新する。
        meta 更新失敗は artifact 書き込みを失敗させない。
        """
    ...
    def update_meta(self, updates):
        ...
        return update_session_meta_warning(str(self.session_dir), updates)
