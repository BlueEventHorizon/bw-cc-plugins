...
**タスクグループ化**（複数ファイルの同期変更が技術的に必須の場合のみ）:

「タスクN完了時点でビルドが通る状態を作れるか？」→ No の場合のみグループ化。グループ化は最終手段。先に以下の回避策を検討すること:
- 互換レイヤー・Adapter 導入（旧インターフェースを残して段階的に移行）
- Feature Flag / 条件分岐（Optional プロパティとして追加し段階的に必須化）
- 段階的移行 Deprecation（typealias 等で旧名を残して段階的に移行）
