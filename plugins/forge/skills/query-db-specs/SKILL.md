...
available-skills から `doc-db:query` / `doc-advisor:query-specs` の有無を判定し、利用可能なバックエンドをカンマ区切りで `--available` に渡す（両方なら `doc-db,doc-advisor`、なければ空文字列）。
...
- `error` が null の場合 → `skill` フィールドの SKILL を `Skill` ツールで 1 回呼ぶ:
  - `/doc-db:query` → `/doc-db:query --category specs --query "$ARGUMENTS" --mode rerank`
  - `/doc-advisor:query-specs` → `/doc-advisor:query-specs "$ARGUMENTS"`
