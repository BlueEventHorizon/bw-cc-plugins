...
| レビュー実行も汎用 Agent      | Codex (Bash subprocess) または Claude (汎用 Agent) にレビューを委譲する                                                 |
...
汎用 Agent (subagent_type: general-purpose) を起動し、レビュー結果を `{session_dir}/{output_path}` に Write するよう指示する。
...
(Codex はリダイレクト、Claude は汎用 Agent が Write)。
