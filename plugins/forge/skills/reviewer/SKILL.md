...
## 関連コード（実装の問題を指摘する場合は必ず読むこと）
- <related_code のパス>

## 実装確認ルール [MANDATORY]
- 実装の問題を指摘する場合、または設計書が実装を正しく反映しているかを検証する場合は、**必ず related_code の該当ファイルを Read して実装の実態を確認すること**
- 実装を確認せずに推測で指摘してはならない。確認できない場合は指摘しない
- 設計書に「実装にない仕様」を追加する指摘は、実装を読んで本当に存在しないことを確認してから行うこと
