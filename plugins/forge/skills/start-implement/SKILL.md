...
`scope_missing_task_ids[]` が空でない場合、該当タスクのスコープ境界が 4.2 で導出されていない（＝レビュアーは対象を最終形として評価する）。**この事実を人間に提示してから 5.2 へ進む** — 段階分割されたタスクでこの状態のままレビューすると、後続タスクの未実装が欠陥として報告される（Issue #4）。
...
**Phase 3 で収集した文書と 4.2 で導出したスコープ境界をレビュー依頼へ渡す [MANDATORY]**。executor には渡してレビュアーには渡さない状態を作らない（実装者は範囲と規範を知っていて、検証者は知らないという非対称が往復を生む。Issue #4）。渡すもの・渡さないものは以下のとおり:
