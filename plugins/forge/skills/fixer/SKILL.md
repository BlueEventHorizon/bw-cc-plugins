...
参考文書を `refs.yaml` から読み込み、汎用 Agent (general-purpose) に実際の Edit/Write を委譲する。
...
| 修正の実行は汎用 Agent                       | メインコンテキストの消費を抑え、修正の Read/Edit を汎用 Agent 側で完結させる                                        |
...
| **やりかた** | plan.yaml を Read し `recommendation: fix` AND `status ∈ {pending, in_progress}` のみフィルタ → 介入軸フラグの severity 制約を AND 適用 → `review_<種別>.md` (最終系) から該当項目を抜粋 → refs.yaml の参考文書・関連コードと合わせて汎用 Agent (general-purpose) に修正を委譲 |
| **Agent**    | plan.yaml / refs.yaml / `review_<種別>.md` Read: メインコンテキスト / 修正実行: 汎用 Agent (general-purpose)                                                                                                                                                                   |
...
4. **`{session_dir}/review_<種別>.md` (最終系 = evaluator 整形済み) を Read** して、フィルタ後の id に対応する項目の詳細 (箇所・該当コード・なぜ問題か・修正案) を抜粋する [MANDATORY]:
   ...
   - 抜粋時には `priority` (P1/P2/P3) と `severity` (critical/major/minor) を **両方** 取得する (汎用 Agent 提示時に直交表示するため)

### Step 3: 修正対象一覧の表示 [MANDATORY]

汎用 Agent 起動前に、修正対象を **priority と severity を直交させて表示** する。フォーマット例:
...
### Step 4: 汎用 Agent 起動 [MANDATORY]

Fixer プロンプトテンプレート (下記) に従い、Agent ツールで汎用 Agent を起動する。
...
汎用 Agent が全参考文書を Read し、修正を実行し、サマリーを報告する。

### Step 5: 結果を呼び出し元に返す

汎用 Agent の修正サマリーを priority + severity 併記のまま呼び出し元に返す。呼び出し元 (`/forge:present-findings` または `/forge:review`) がユーザーへの報告を担当する。
...
| 汎用 Agent 起動失敗                                                            | エラー内容を呼び出し元に返す                                                               |
| 汎用 Agent が修正失敗を報告                                                    | エラー内容を呼び出し元に返す。呼び出し元がユーザーに報告                                   |
| 汎用 Agent が指摘事項と無関係な変更を報告                                      | 呼び出し元がサマリーを確認し、ユーザーに報告して判断を仰ぐ                                 |
