...
4. 各項目の perspective に応じて `{session_dir}/review_{perspective}.md`（最終系 = evaluator 整形済み）を Read して該当項目の詳細を取得
...
2. **`review_{perspective}.md`（最終系）から各項目の詳細を取得**
...
4. `has_severity` なら重大度順に並べ替え。ない場合は番号順を維持
5. 項目数をカウントし、提示準備

> **定常フローで target_files / reference_docs を再 Read しない**。
> evaluator が書き換えで必要情報を `review_{perspective}.md` に含めているため再取得は不要。
...
2. **一括修正後の単独修正レビュー [MANDATORY]** — fixer --batch 完了直後に必ず実行する。サマリー報告や次フローへの移行より先に実施する。スキップ禁止。
...
1. **`review_{perspective}.md`（最終系）から回答可能か判定**
   - 答えが `review_{perspective}.md` に含まれる → 該当箇所を引用して回答し、再度 AskUserQuestion で判断を仰ぐ
   - 含まれない → ステップ 2 へ

2. **親 Claude が直接 Read して回答**
   - `refs.yaml` から target_files / reference_docs のパスを確認
   - 質問に関連するファイルのみを最小限 Read する（**全件 Read しない**）
   - subagent には委譲しない（直接親 Claude が Read する）
   - 読み取り結果をもとに回答し、再度 AskUserQuestion で判断を仰ぐ

3. **情報不足のシグナル**
   - `review_{perspective}.md`（最終系）に本来含まれるべき情報が不足していた場合、
...
Claude は `write_interpretation.py` 経由で `review_{perspective}.md` を更新する。
...
# evaluator 評価（perspective: {perspective_name}） — ユーザー対話反映版
...
Fixer は `review_{perspective}.md`（最終系）のみを読む。対話後に更新しないと、
...
| ------------------ | ------------------------------------------------------- | ------------------------------------------------------------- | ---------------------------------- |
| 対象ファイル表示   | 問題・修正の対象ファイルパスを `path:line` 形式で表示   | `review_{perspective}.md`（最終系）の「箇所」欄               | **全ての項目（必須）**             |
| コード表示         | 該当箇所のコードを表示                                  | `review_{perspective}.md`（最終系）の「該当コード」欄         | コードに関する項目                 |
| 比較表             | 修正前/修正後、オプションA/Bを表で対比                  | `review_{perspective}.md`（最終系）の「修正案」欄             | 比較・選択がある場合               |
| 影響範囲の説明     | 影響・リスク・メリットを説明                            | `review_{perspective}.md`（最終系）の「なぜ問題か」欄         | 全ての項目                         |
| ルール・根拠の引用 | 規約・設計意図からの根拠を引用                          | `review_{perspective}.md`（最終系）の「なぜ問題か」欄の引用   | 根拠が必要な場合                   |
| 正しいパターン     | 正しい実装例・修正後コードを提示                        | `review_{perspective}.md`（最終系）の「修正案」欄             | コードレビュー時                   |
| AI判定表示         | 推奨 / 自動修正可能 / 判定根拠を表示                    | plan.yaml の recommendation / auto_fixable / reason           | 全ての項目（末尾に表示）           |
| 重大度マーク       | 🔴🟡🟢 / ❌（却下）/ ✅（auto_fixable）                  | plan.yaml の severity / recommendation / auto_fixable         | 全ての項目                         |

> **定常フローでは target_files / reference_docs を再 Read しない**。
> コード・ルールの引用は `review_{perspective}.md`（最終系）に含まれている内容を使用する。
...
> **情報の取得元**: 以下の例の「該当コード」「なぜ問題か」「修正案」は全て `review_{perspective}.md`（最終系）から引用する。
> 定常フローで target_files / reference_docs を再 Read しない。
...
> **AI判定セクションは提示の末尾に必ず表示する**。plan.yaml の recommendation / auto_fixable / reason を
> そのまま転記する（日本語訳: `fix` → 修正 / `skip` → 却下 / `needs_review` → 要確認）。
