...
| **fixer は Agent ツール経由 (id 単位 1 起動)** | 修正実行時は `forge:fixer` カスタム Agent を Agent ツールで finding_id 単位に起動する (REQ-006 / DES-032 §3.1 / §3.5.1 で fork 型 SKILL から Agent 化済み)。`--batch` は廃止され、orchestrator 側の id ループに変換 |
...
3. ユーザーの選択に応じて:
   ...
   - **修正を選択**（B 案 / Other 独自方針）→ `python3 ${CLAUDE_SKILL_DIR}/scripts/mark_in_progress.py {session_dir} {id}` で更新 → 修正方針の解釈が必要なため軽量経路に入らず、Agent ツールで `forge:fixer` カスタム Agent を起動し、修正を委譲（後述「forge:fixer Agent 呼び出し時の責務」参照）
...
4. plan.yaml を `fixed` に更新: `mark_fixed.py` を呼ぶ
...
2. **軽量経路判定 [REQ-004 FNC-413] [MANDATORY]**: ✅付き項目数を見て経路を分岐する
   ...
   | ✅付き項目数 **4 件以上** | **fixer 経路** (id 単位ループ) | orchestrator が ✅付き id を集めた `fix_ids` をループし、各 id について Agent ツールで `forge:fixer` カスタム Agent を `finding_id={id}` / `allowed_files=[当該 finding の target_files]` で起動 (DES-032 §3.5.1 単一 finding 起動原則。`--batch` は廃止)。後述「forge:fixer Agent 呼び出し時の責務」参照 |
...
3. **一括修正後の単独修正レビュー [MANDATORY]** — 軽量経路 / fixer 経路いずれも完了直後に必ず実行する。サマリー報告や次フローへの移行より先に実施する。スキップ禁止。
   ...
   3. 問題なし → 次へ
   4. plan.yaml を `fixed` に更新（単独修正レビュー完了後に `mark_fixed.py` を呼ぶ）
      - 軽量経路の場合: 修正した finding の id と修正ファイルパス一覧を `mark_fixed.py` に渡す
      - fixer 経路の場合: `patch_result.json` の `patched_ids` と `files_modified` を参照して `mark_fixed.py` に渡す
...
fixer に修正を委譲する際は、Agent ツールで `forge:fixer` カスタム Agent を **1 finding につき 1 起動** で呼び出す (REQ-006 / DES-032 §3.1 / §3.5.1 で fork 型 SKILL から Agent 化済み、`--batch` は廃止)。prompt には以下の構造化引数だけを渡すこと。親タスク本文、指摘詳細、対象ファイル本文、参考文書本文は貼り付けない。

| 項目          | 必須 | 渡し方                                                                                                              |
| ------------- | ---- | ------------------------------------------------------------------------------------------------------------------- |
| session_dir   | 必須 | `{session_dir}`                                                                                                     |
| kind          | 必須 | `{review_type}` (`code` / `design` / `requirement` / `plan` / `uxui` / `generic`)                                   |
| finding_id    | 必須 | **整数値 1 個** (単一 finding 起動原則 / DES-032 §3.5.1)                                                            |
| allowed_files | 必須 | 編集を許可するファイルパスの配列 (DES-032 §3.5.2 allowlist)。当該 finding の `target` ファイル + 必要な関連ファイル |
...
prompt:
  ...
  agents/fixer.md の手順 (Step 1〜7) に従い、DES-032 §3.5 の 4 制約
