...
| **fixer は Skill ツール (fork) 経由** | 修正実行時は `/forge:fixer` を Skill ツール (fork) で起動する                                          |
...
3. ユーザーの選択に応じて:
   ...
   - **修正を選択**（B 案 / Other 独自方針）→ `python3 ${CLAUDE_SKILL_DIR}/scripts/mark_in_progress.py {session_dir} {id}` で更新 → 修正方針の解釈が必要なため軽量経路に入らず、Skill ツール (fork) で `/forge:fixer` を呼び出し、修正を委譲（後述「/forge:fixer 呼び出し時の責務」参照）
...
| `auto_fixable: false` | **fixer 経路** | Skill ツール (fork) で `/forge:fixer` を `args: "{session_dir} {review_type} --single {id}"` で呼び出し、修正を委譲（後述「/forge:fixer 呼び出し時の責務」参照）                     |

軽量経路は fixer (Skill ツール) を起動せず、Skill ツール起動オーバーヘッドを回避する。判定段階で `review_<種別>.md` 全文を読まず、軽量経路に入ったときのみ該当 finding の修正案セクションを抜粋 Read する。
...
1. Skill ツール (fork) で `/forge:reviewer` を `--diff-only {修正されたファイル}` 付きで呼び出し、修正差分のみをレビュー
   - `args: "{session_dir} {review_type} {engine} --diff-only {files_modified}"`
2. 修正起因の問題が見つかった場合 → Skill ツール (fork) で `/forge:fixer` を `--diff-only` 付きで呼び出して修正 → 再レビュー（上限: 3回）。**`--diff-only` サイクルでの再修正は常に fixer 経路** (FNC-413 除外規定)
...
2. **軽量経路判定 [REQ-004 FNC-413] [MANDATORY]**: ✅付き項目数を見て経路を分岐する
   ...
   | ✅付き項目数 **4 件以上** | **fixer 経路** | Skill ツール (fork) で `/forge:fixer` を `args: "{session_dir} {review_type} --batch"` で呼び出し、修正を委譲（後述「/forge:fixer 呼び出し時の責務」参照）                                                           |
...
3. **一括修正後の単独修正レビュー [MANDATORY]** — 軽量経路 / fixer 経路いずれも完了直後に必ず実行する。サマリー報告や次フローへの移行より先に実施する。スキップ禁止。
   1. Skill ツール (fork) で `/forge:reviewer` を `--diff-only {修正されたファイル一覧}` 付きで呼び出し、修正差分のみをレビュー
      - `args: "{session_dir} {review_type} {engine} --diff-only {files_modified}"`
   2. 修正起因の問題が見つかった場合 → Skill ツール (fork) で `/forge:fixer` を `--diff-only` 付きで再度呼び出して修正 → 再レビュー（上限: 3回）。**`--diff-only` サイクルでの再修正は常に fixer 経路** (FNC-413 除外規定)
      - `args: "{session_dir} {review_type} --diff-only {files_modified}"`
   3. 問題なし → 次へ
...
fixer に修正を委譲する際は、Skill ツール (fork) で `/forge:fixer` を呼び出し、`$ARGUMENTS` には以下の構造化引数だけを渡すこと。親タスク本文、指摘詳細、対象ファイル本文、参考文書本文は貼り付けない。

| 項目                       | 必須                                 | 渡し方                                                                            |
| -------------------------- | ------------------------------------ | --------------------------------------------------------------------------------- |
| session_dir                | 必須                                 | `{session_dir}`                                                                   |
| レビュー種別               | 必須                                 | `{review_type}` (`code` / `design` / `requirement` / `plan` / `uxui` / `generic`) |
| モード                     | 必須                                 | `--single` / `--batch` / `--diff-only`                                            |
| 対象項目の id              | 必須 (`--single`) / 任意 (`--batch`) | `--single {id}`。`--batch` で絞り込む場合は `--ids 1 2 3` を追加する              |
| 介入軸フラグ               | 任意 (`--batch` のみ)                | `--auto-critical` / `--auto`。指定なしは severity フィルタなし                    |
| ユーザーが選択した修正方針 | 任意                                 | AskUserQuestion の回答 (A 案 / B 案等)。必要な場合のみ短い方針として追加する      |

呼び出し例:

```text
Skill ツール (fork): /forge:fixer
...
Skill ツール (fork): /forge:fixer
args: "{session_dir} {review_type} --batch --ids 1 2 3"

Skill ツール (fork): /forge:fixer
args: "{session_dir} {review_type} --diff-only {files_modified}"
```

> fixer は session_dir を受け取り、plan.yaml / refs.yaml / `review_<種別>.md` から指摘詳細、target_files、reference_docs、related_code を自前で読み込む。個別に渡してはならない。
