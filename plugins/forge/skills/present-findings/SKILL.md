...
3. ユーザーの選択に応じて:
   - **修正を選択**（A案/B案）→ `/forge:fix-findings --single` を呼び出し、修正を委譲（後述「/forge:fix-findings 呼び出し時の責務」参照）
   - **このまま（対応しない）** → Step 5 へ
   - **一覧に戻る** → Step 2 へ
...
1. ✅付き項目を収集し、`/forge:fix-findings --batch` を呼び出し、修正を委譲（後述「/forge:fix-findings 呼び出し時の責務」参照）
...
## /forge:fix-findings 呼び出し時の責務 [MANDATORY]
