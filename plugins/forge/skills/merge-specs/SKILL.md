...
Skill ツールで `/forge:fix-doc-refs` を起動する。**grep で自分で抽出しない。**

本 skill は文書の削除・改名・節の移動を伴うため、参照切れが最も生じやすい。置換されなかった所見について、**このセッションで自分が動かした節・見出し・パス**があればその記憶で直す。心当たりが無いものは報告に留め、フローは止めない。
...
Phase 6.2 で実施済み。Phase 7.1 で ID を修正した場合は、Skill ツールでもう一度 `/forge:fix-doc-refs` を起動する。
...
### 8.5 参照の実在性検査 [MANDATORY]

Skill ツールで `/forge:fix-doc-refs` を起動する。

**削除した文書を指す参照が他文書に残る。** 8.3 の `git rm` はこの skill で最後に行う書き込みであり、ここを飛ばすと、消した文書への参照が残ったまま commit される。Phase 6.2 の検査は削除より前なので、この形は捕まえられない。

置換されなかった所見について、**このセッションで自分が動かした・改名した・削除した文書**があればその記憶で直す。削除した文書への参照は、内容を基本側へ移していればその移動先へ差し替える。内容ごと消したのであれば、参照を残すかどうかは利用者の判断なので報告に留める。フローは止めない。
...
{additional_dir} を {base_dir} に統合しました:
  ...
  基本側の修正 (Phase 4): N 件
  追加側からの新規作成 (Phase 5): N 件
  追加側の削除 (Phase 8): N 件

検証結果:
  ID 重複: {Phase 7.1 の結果}
  参照切れ: {Phase 8.5 の結果。置換 N 件 / 残 N 件（残があれば位置を列挙）}
