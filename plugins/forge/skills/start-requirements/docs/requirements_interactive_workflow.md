...
> **注**: 「曖昧表現」行の判定はレビュー時に `${CLAUDE_PLUGIN_ROOT}/skills/review/docs/review_criteria_requirement.md` の verifiability perspective「判定原則: 設計書委譲の判定」表 3（カテゴリ × 委譲明示段階）で機械化されている。本表は書き手向けの簡易版であり、reviewer / evaluator は criteria 表 3 を一次情報源として使用する。
