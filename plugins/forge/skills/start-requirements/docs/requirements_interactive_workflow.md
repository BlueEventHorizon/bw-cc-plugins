...
### 9. 改訂履歴は確定段階でまとめる [MANDATORY]

検討中の暫定案を改訂履歴に書き込まない。1 議論 1 エントリでもキリがなく、後で見たときに「どれが正本か」が分からなくなる。

- 議論段階: 履歴に書かず、TBD として未確定を明示する
- 確定段階: 関連する変更を 1 エントリにまとめて履歴へ
- 大規模 rewrite の途中で履歴エントリを増やしていくのは禁止
...
| **改訂履歴の早書き**                | 検討中の暫定案が履歴に残り、後で正本不明                   | 議論中は TBD のみ。確定後に 1 エントリにまとめる                           |
...
> **注**: 「曖昧表現」行の判定はレビュー時に `${CLAUDE_PLUGIN_ROOT}/skills/review/docs/review_criteria_requirement.md` の verifiability perspective「判定原則: 設計書委譲の判定」の判定マトリクスで機械化されている。本表は書き手向けの簡易版であり、reviewer / evaluator は criteria 側を一次情報源として使用する。
