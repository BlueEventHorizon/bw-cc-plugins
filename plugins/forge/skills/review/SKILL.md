...
## Phase 0: セッション準備 [MANDATORY]

中断判定と再開分岐を **動詞 1 つ** で行う。schema や session.yaml の status 値を SKILL は触らない。

```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/review_session.py probe
```

返却の `state` で分岐:

- **`state: "none"`** → 通常フロー (Phase 1 → 2 → 3 → 4 → 5)
- **`state: "resumable"`** → AskUserQuestion で「前回の未完了セッション (`{session_dir}`) を再開しますか？」

  - **再開する**:
    ```bash
    python3 ${CLAUDE_SKILL_DIR}/scripts/review_session.py resume {session_dir}
    ```
  ...
  - **破棄する**:
    ```bash
    python3 ${CLAUDE_SKILL_DIR}/scripts/review_session.py finish {session_dir}
    ```
    削除完了後、通常フローへ進む。

> **設計**: `probe` は内部で `status: completed` の残骸を全スキル横断で自動回収する。SKILL 側で個別に cleanup する必要はない。詳細は `${CLAUDE_PLUGIN_ROOT}/docs/session_format.md` 参照。
...
### Step 4: セッション開始 (refs.yaml 書き込みを含む)

`review_session.py start` を 1 度呼ぶだけで、session ディレクトリ作成と refs.yaml 書き込みが原子的に完了する。schema 詳細は `${CLAUDE_PLUGIN_ROOT}/docs/session_format.md` §7 (review 固有ファイル) を参照。

```bash
echo '<refs_json>' | python3 ${CLAUDE_SKILL_DIR}/scripts/review_session.py start \
    --review-type {review_type} --engine {engine} --interaction {interaction}
```
...
`review_session.py status` で残件を集計する:

```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/review_session.py status {session_dir}
```

返却の `unprocessed_total` で分岐:

- **`0`** → `review_session.py finish {session_dir}` で完了処理 (complete + cleanup を 1 動詞に統合)
- **`> 0`** → AskUserQuestion で決着方法を確認 (`個別に判定する` / `全件「対応しない」として終了`)。「残す」選択肢は提示しない

```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/review_session.py finish {session_dir}
```

> **なぜ「残す」を選択肢に入れないか**: 終了条件を満たさないまま意図的に session を残すと、「終わったのか終わっていないのか」が曖昧になる。クラッシュ等で未完のまま残った session は、次回 `/forge:review` 起動時の Phase 0 で resumable として検出される。
