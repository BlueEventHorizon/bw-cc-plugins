...
**Agent ツール**で `subagent_type: "forge:reviewer"` を **1 体のみ** 起動する。これは REQ-006 / DES-032 §3.1 の決定 (fork 型 SKILL から Agent 化) に基づく。engine (`codex` / `claude`) に関係なく経路は同一で、orchestrator はレビュー実行エンジンを直接起動しない。
...
evaluator も **1 起動**で動作する。**Agent ツール**で `subagent_type: "forge:evaluator"` を **1 体のみ** 起動する (REQ-006 / DES-032 §3.1 / TASK-006 で fork 型 SKILL → カスタム Agent 化)。
