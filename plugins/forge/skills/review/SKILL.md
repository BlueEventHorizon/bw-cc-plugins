...
##### 4-c: forge ルール検索による追加 perspectives（generic 以外）

generic 以外の種別では必ず以下を実行する:

- `/forge:query-db-rules` Skill にレビュー種別に関連するルール文書を問い合わせる
...
`/forge:query-db-rules` / `/forge:query-db-specs` は**使用しない**。参考文書は perspectives に含まれる criteria ファイルのみ。

##### generic 以外の種別

必ず以下を実行する:

- `/forge:query-db-specs` Skill → 関連する要件定義書・設計書を特定
...
各 evaluator は以下を**必ず**実行する:

1. 自分の担当 `review_{perspective}.md` を読み、指摘を個別吟味
2. `{session_dir}/eval_{perspective_name}.json` に判定メタ情報（recommendation / auto_fixable / reason）を Write
3. **`write_interpretation.py` 経由で `review_{perspective}.md` を常に全面書き換え**（判定に同意する場合でも必須）
   - 原文は `review_{perspective}.raw.md` に自動バックアップされる（初回のみ作成・冪等）
   - 書き換え内容は present-findings が段階的提示に必要とする情報粒度（該当コード抜粋・ルール引用・修正案）に整形したもの

- **部分失敗**: 失敗した perspective の吟味結果は欠損として扱い、成功分のみで続行する
- **全 perspective 失敗（成功 0 件）**: hard fail

##### Step 1.5: 統合 review.md の再生成 [MANDATORY]

全 evaluator 完了後、統合 review.md を evaluator 整形済み内容で再生成する:

```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/extract_review_findings.py \
  {session_dir} --review-only
```
...
全 evaluator 完了後、orchestrator が `merge_evals.py` を1回だけ呼び出して plan.yaml を更新する:

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/session/merge_evals.py {session_dir}
```

このスクリプトは `eval_*.json` を収集し、**perspective ごとのローカル ID を plan.yaml のグローバル ID に変換**した上で一括更新を行う。戻り値 JSON から以下を取得する:

- `should_continue`: `recommendation: fix` が1件以上なら `true`
...
各 evaluator は以下を**必ず**実行する（対話モードの Step 1 と同じ手順）:

1. 自分の担当 `review_{perspective}.md` を読み、指摘を個別吟味
2. `{session_dir}/eval_{perspective_name}.json` に判定メタ情報を Write
3. **`write_interpretation.py` 経由で `review_{perspective}.md` を常に全面書き換え**（判定に同意する場合でも必須）
   - 原文は `review_{perspective}.raw.md` に自動バックアップされる

全 evaluator 完了後、orchestrator が以下を順に実行する（対話モードの Step 1.5 / 1.6 と同じ手順）:

```bash
# 1. 統合 review.md を evaluator 整形済み内容で再生成（plan.yaml は書き換えない）
python3 ${CLAUDE_SKILL_DIR}/scripts/extract_review_findings.py \
  {session_dir} --review-only
...
fixer は plan.yaml を自力 Read して `recommendation: fix` AND `status ∈ {pending, in_progress}` にフィルタし、
`review_{perspective}.md`（最終系 = evaluator 整形済み）から該当項目を抜粋して修正を実行する。
`recommendation: skip` / `needs_review` の項目は処理対象外。`.raw.md`（reviewer 原文）は読まない。
...
3. テストが失敗した場合 → 失敗内容を報告し、`AskUserQuestion` を使用して次アクション（`失敗を確認して終了` / `修正を継続する` / `テストを再実行する`）を確認する
...
3. 設計書を更新した場合 → `/forge:update-db-specs` が利用可能であれば ToC も更新する
...
`/forge:update-db-specs` Skill が利用可能であれば呼び出す。利用不可の場合はスキップ。
...
- 選択肢:
  ...
  - **「全件『対応しない』として終了」**
    → 未処理を一括で `skipped` に更新してから Step 2a の cleanup に進む:

    ```bash
    python3 ${CLAUDE_PLUGIN_ROOT}/skills/review/scripts/skip_all_unprocessed.py {session_dir}
    ```

    **exit 0 の場合のみ** 以下の cleanup を実行して Step 2a の終了メッセージを出力する:

    ```bash
    python3 ${CLAUDE_PLUGIN_ROOT}/scripts/session_manager.py cleanup {session_dir}
    ```

    **非 0 終了（`stage=summarize_plan` / `stage=json_build` / `stage=update_plan`）の場合**:

    - cleanup を **実行しない**（未処理指摘を残したまま session を削除しない）
    - stderr 先頭行の `stage=...` を含むエラー内容をユーザーに報告する
    - `AskUserQuestion` を使用して次アクション（`再実行する` / `個別に判定に戻る` / `中断する`）を確認する
    - session_dir は残すため、次回 `/forge:review` 起動時の残存セッション検出で再開対象となる
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/resolve_rules.py"
python3 "${CLAUDE_SKILL_DIR}/scripts/resolve_specs.py"
```
