...
★ ここで収集した情報を session_dir に保存する。以降の全 agent は session_dir パスのみ受け取る。
...
#### Step 7: セッションディレクトリの作成 [MANDATORY]
...
2. `{session_dir}/session.yaml` を Write する
   （フォーマット: `${CLAUDE_PLUGIN_ROOT}/docs/session_format.md` の「session.yaml」参照）

3. `{session_dir}/refs.yaml` を Write する
   （フォーマット: `${CLAUDE_PLUGIN_ROOT}/docs/session_format.md` の「refs.yaml」参照）

4. session_dir パスをコンテキストに保持（`{session_dir}` として以降で使用）
...
- session_dir
- レビュー種別
- 修正対象フラグ: `--interactive`（全件 AI が推奨判定を行い evaluation.yaml に記録。plan.yaml は変更しない）
...
##### Step 2: show-report でレポートを表示

`/forge:show-report {session_dir}` を呼び出して初期 HTML レポートを生成しブラウザで表示する。

##### Step 3: present-findings を呼び出す
...
（refs.yaml は既存のものを使用。reviewer が再度読み込む）
