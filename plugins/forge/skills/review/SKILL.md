...
- `{name}`: 英小文字の識別子（例: `correctness`, `resilience`）
...
generic 以外の種別で DocAdvisor（`.claude/skills/query-rules/SKILL.md`）が利用可能な場合:

- `/query-rules` Skill にレビュー種別に関連するルール文書を問い合わせる
...
`/query-rules` / `/query-specs` は**使用しない**。参考文書は perspectives に含まれる criteria ファイルのみ。
...
2. refs.yaml をスクリプトで生成する:
   ...
   ```json
   {
     ...
     "perspectives": [
       {"name": "correctness", "criteria_path": "review/docs/review_criteria_code.md", "section": "正確性 (Logic)", "output_path": "review_correctness.md"},
...
for perspective in perspectives; do
    ${CLAUDE_PLUGIN_ROOT}/skills/reviewer/scripts/run_review_engine.sh \
...
- `output_path`（例: `review_correctness.md`）
