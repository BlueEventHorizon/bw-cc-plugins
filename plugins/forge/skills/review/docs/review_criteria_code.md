...
| -------- | --------------------------------------------------- | ---------- | -------------------------------------------------------------------------------- |
| P1       | `docs/rules/implementation_guidelines.md`           | rules      | プロジェクト固有実装ルール (規範本体 + 重大度カタログ)                           |
| P1       | `docs/rules/cli_output_formatting.md`               | rules      | CLI 出力フォーマット規約 (コーディング規約として参照、規範本体 + 重大度カタログ) |
| P1       | `docs/specs/<feature>/design/*.md`                  | specs      | target に対応する関連設計書 (規範本体 + 重大度カタログ)                          |
| P2       | 関連設計書と実装の整合性チェック対象                | specs      | 矛盾検出 (P1 で参照した設計書 + target_files 間の相反記述を突合)                 |
| P3       | `${CLAUDE_PLUGIN_ROOT}/docs/forge_anti_patterns.md` | principles | 不要な複雑化判定 (Yes/No 判定原則、FNC-411 拡充後はアンチパターン重大度カタログ) |
...
1. **P1 ルール合致**: `docs/rules/implementation_guidelines.md` → `docs/rules/cli_output_formatting.md` → 関連 `docs/specs/<feature>/design/*.md` の順で対象コードと照合する
2. **P2 矛盾・齟齬**: P1 で参照した設計書と target_files 間で、同一対象への相反記述 (インターフェース定義 / データモデル / 処理フロー等) を突き合わせる (不足・欠落は P2 対象外)
3. **P3 不要な複雑化**: `${CLAUDE_PLUGIN_ROOT}/docs/forge_anti_patterns.md` を参照し、より少ない要素で同じ目的を達成できる代替案の有無を Yes/No で判定する
