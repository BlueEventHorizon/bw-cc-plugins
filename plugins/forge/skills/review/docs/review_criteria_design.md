...
1. **P1 ルール合致**: `${CLAUDE_PLUGIN_ROOT}/docs/spec_design_boundary_spec.md` (What/How 境界 / §4 カテゴリ別ガイド) → `${CLAUDE_PLUGIN_ROOT}/docs/design_principles_spec.md` (設計原則 / 失敗パターン) → プロジェクト固有アーキテクチャ規約 (`docs/rules/*.md` のうち存在するもの) の順で対象設計書と照合する
