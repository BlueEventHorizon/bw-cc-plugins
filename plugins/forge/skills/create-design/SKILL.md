...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/session_manager.py find --skill create-design
```
...
設計書の作成・更新後、`.claude/skills/create-specs-toc/SKILL.md` が存在する場合のみ `/create-specs-toc` を実行すること（存在しない場合はスキップ）。
