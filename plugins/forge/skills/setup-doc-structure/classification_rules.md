...
4. **不明なら聞く**: 判断に迷う場合は推測せず、AskUserQuestion を使用してユーザーに確認する。
...
- 複数のコンポーネントが category で矛盾する場合（例: `specs/rules/`）→ AskUserQuestion を使用してユーザーに確認
...
確信度が低いため、AskUserQuestion を使用してユーザーに確認を推奨。
