...
> 3. プロジェクトの `.doc_structure.yaml` がある場合、Phase 1 で feature 候補を `forge:doc-structure` 経由で列挙する
...
`.doc_structure.yaml` がある場合は forge:doc-structure 経由で feature を列挙する:

```bash
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
DOC_STRUCTURE="${CLAUDE_PLUGIN_ROOT}/skills/doc-structure/scripts/resolve_doc_structure.py"

if [ -f "$PROJECT_ROOT/.doc_structure.yaml" ] && [ -f "$DOC_STRUCTURE" ]; then
  python3 "$DOC_STRUCTURE" --features --project-root "$PROJECT_ROOT" 2>/dev/null
else
  # フォールバック: 一般的な docs/specs 配下から自動推測
  # 1 階層 (docs/specs/{feature}/) と 2 階層 (docs/specs/{plugin}/{feature}/) の両方を候補に
  find docs/specs -mindepth 1 -maxdepth 2 -type d 2>/dev/null \
    | grep -vE '/(requirements|requirement|design|plan)$'
fi
```
...
doc-advisor がインストールされていないプロジェクトでは `.claude/doc-advisor/` ディレクトリ自体が存在しないため、追加除外は不要 (`grep -v` は安全に no-match で素通りする)。
...
完了後、以下を表示する。検出した依存ツールに応じて「次のステップ」を出し分ける:
...
次のステップは以下を **検出時のみ** 案内する:

```bash
# doc-advisor ToC 再生成 (.claude/doc-advisor/ がある場合)
[ -d ".claude/doc-advisor" ] && echo "  - /update-forge-toc または /doc-advisor:create-specs-toc"

# anvil による commit (anvil プラグインが入っている場合)
if [ -d "${CLAUDE_PLUGIN_ROOT}/../anvil" ] || command -v gh >/dev/null 2>&1; then
  echo "  - /anvil:commit または git commit"
fi
```

検出ツールがいずれも無い場合は汎用案内に切り替える:

```
次のステップ:
  - 変更を確認して commit してください (git add / git commit)
  - 必要なら ToC や検索インデックスを再生成してください
```
...
- **依存ツールは「あれば実行・なければスキップ」**: dprint / doc-advisor / anvil の有無で動作を分岐する。完了案内も検出時のみ表示する
