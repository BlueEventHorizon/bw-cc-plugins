...
**この検査は read-only 性の担保ではありません [MANDATORY]**。Agent 定義が意図どおりに書かれていることを確認するだけです。レビュアーが成果物を変更していないことの担保は、本体がラウンドごとに変更集合を確認することで与えられます。本バックエンドはこの確認を代行しません（逸脱した側の申告を担保にしないため）。
