...
**条件 2 で「外部通信ツールを含まない」と断定しません**。`Bash` は外部通信も成果物の変更も可能にするため、断定すると検査条文が事実と食い違います。`Bash` を許可するのはレビュアーが差分・ブランチ対象を自分で確定するために必要だからです（これを外すと `--diff` / `--branch` のレビューが成立しません）。

不足ごとに `axis` / `detail` / `remedy` を持つ要素を `missing` へ追加します。全条件を満たす場合だけ `{"available": true, "missing": [], "retains_context": false}` を返します。

**`retains_context` は常に `false` です**。本バックエンドはラウンドごとに新しい Agent を起動し、前ラウンドの transcript を渡しません。本体はこの申告に従って 2 ラウンド目以降に完全な依頼本文を送ります。申告を誤ると、レビュアーは対象も観点も持たないまま応答することになります。
