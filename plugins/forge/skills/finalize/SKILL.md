...
name: finalize
description: |
  文書作成後の品質確定ステップ。レビュー+自動修正・ToC 更新・commit & push を一括実行する。
  create-requirements / create-design / create-plan の後続処理として使用。
user-invocable: true
...
`create-requirements` / `create-design` / `create-plan` で作成した文書の品質を確定する。
...
# レビューのみ（修正なし）
/forge:finalize plan specs/login/plan/login_plan.md --refactor 0
```
...
`/anvil:commit` Skill が利用可能か確認する（利用可能なスキル一覧に `anvil:commit` が存在するか）。

- 利用可能 → `/anvil:commit` を呼び出す（コミットメッセージは自動生成）
- 利用不可 → スキップ（エラーにしない）

> **Note**: `--refactor 0`（レビューのみ）の場合でも呼び出す。変更がなければ `/anvil:commit` 側がスキップする。
