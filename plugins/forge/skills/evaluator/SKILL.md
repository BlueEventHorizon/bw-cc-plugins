...
| 全モード共通ロジック         | auto / interactive を問わず、`apply_eval.py` 経由で plan.yaml を直接更新・should_continue 判定を実行                                                                       |
...
1. `apply_eval.py` 経由で plan.yaml を直接更新 (中間ファイル `eval_<種別>.json` は生成しない — Issue #103)
2. `review_<種別>.md` を整形済み Markdown で全面書き換え
...
- **id = plan.yaml グローバル ID**。reviewer が採番した出現順 1-based id がそのまま plan.yaml の id と対応する (reviewer 1 起動原則の前提)
