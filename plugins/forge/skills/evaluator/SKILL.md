...
4. `refs.yaml` の `perspectives` 配列から自分の担当 perspective（`name == perspective_name`）の `criteria_path` を取得し Read する（後段の判定基準ソース・オブ・トゥルース）。レビュー種別が `requirement` で、かつ `criteria_path` がプラグイン標準と異なる（DocAdvisor 追加 perspective 等）の場合のみ、プラグイン標準 `${CLAUDE_PLUGIN_ROOT}/skills/review/docs/review_criteria_requirement.md` を追加 Read する（後段「設計フェーズ委譲」判定の判定マトリクス参照のため）

（収集・探索は行わない。`refs.yaml` に記載されたパスおよび上記 criteria のみ使用する）
...
判定基準は criteria の **「判定原則: 設計書委譲の判定」** セクションの判定マトリクスに従う（重複定義を避けるためここでは再掲しない）。判定アクションの要約:
...
| 指摘内容                         | 要件側の状態                             | 判定                                                              |
| -------------------------------- | ---------------------------------------- | ----------------------------------------------------------------- |
| 「Embedding モデル名が未記載」   | 散文で「設計書で確定」と明示（部分明示） | `fix` / `auto_fixable: false`（TBD-xxx 形式に整形 + 期限追加）    |
| 「LLM Rerank の top-N が未定義」 | TBD-007 で完全明示                       | 通常指摘対象外。誤検出なら `skip` / `false_positive`              |
| 「chunk 分割上限が曖昧」         | 委譲明示なし（実装手段）                 | `fix` / `auto_fixable: false`（TBD-xxx として未確定事項表に追記） |
| 「使いやすい UI／基準なし」      | 受け入れ基準が主観表現のみ               | `fix` / `auto_fixable: false`（定量的・客観的基準に置換）         |
