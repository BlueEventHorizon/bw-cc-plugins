...
- **スキップ**: 対象ファイルを読んで問題が存在しない・設計意図がある・既に対処済み・設計フェーズ委譲対象 → `recommendation: skip`（理由を記録）
...
カテゴリ分類（表1）・委譲明示段階（表2）・判定マトリクス（表3）の詳細は、Step 1 で Read した `criteria_path`（= `review_criteria_requirement.md`）の **verifiability perspective「判定原則: 設計書委譲の判定」** セクションに集約されている。重複定義を避けるため evaluator では再掲しない。判定時は同セクションの 3 表を参照する。
