...
1. `{session_dir}/refs.yaml` を Read して `reference_docs` / `related_code` / `target_files` / `perspectives` を取得
...
4. `refs.yaml` の `perspectives` 配列から自分の担当 perspective（`name == perspective_name`）の `criteria_path` を取得し Read する（後段の判定基準ソース・オブ・トゥルース）
5. **レビュー種別が `requirement` の場合は、自分の `criteria_path` がプラグイン標準と一致しているか否かに関わらず、プラグイン標準 `${CLAUDE_PLUGIN_ROOT}/skills/review/docs/review_criteria_requirement.md` を必ず追加で Read する**（後段「要件定義書レビュー固有の判定: 設計フェーズ委譲」で必要となる表 1〜3 の参照元。DocAdvisor 追加 perspective でプロジェクト固有ルール文書が `criteria_path` に来た場合でも表 1〜3 が読めなくなる事態を防ぐため、無条件で併読する）

（収集・探索は行わない。`refs.yaml` に記載されたパスおよび上記プラグイン標準 criteria のみ使用する）
...
> **吟味対象外の指摘の扱い**: `--auto` では 🟢 が、`--auto-critical` では 🟡🟢 が吟味対象外となる。吟味対象外の指摘は plan.yaml に `recommendation: skip` / `skip_reason: "out_of_scope"` / `reason: "吟味対象外（モードによるフィルタ）"` として記録し、`status` を `skipped` に更新する（`skip_reason` enum カタログは「`skip_reason` の値カタログ」セクション参照）。
...
各指摘について以下の6つの観点で評価し、`修正する / スキップ / 要確認` を判定する:
...
| **設計フェーズ委譲**   | 要件定義書レビュー時、指摘が設計書で確定すべき実装手段（How）を要件側に要求していないか（要件定義書レビュー時に限り適用 — 詳細は次項「要件定義書レビュー固有の判定: 設計フェーズ委譲」参照） |

**判定結果:**

- **修正する**: 対象ファイルを読んで問題の実在を確認でき、副作用リスクが低い → `recommendation: fix`
- **スキップ**: 対象ファイルを読んで問題が存在しない・設計意図がある・既に対処済み・完全明示済み TBD-xxx の誤検出（要件定義書レビュー時のみ — 詳細は次項「要件定義書レビュー固有の判定: 設計フェーズ委譲」参照） → `recommendation: skip`（理由を記録）
- **要確認**: 対象ファイルを読んでも判断が難しい → `recommendation: needs_review`

#### 要件定義書レビュー固有の判定: 設計フェーズ委譲 [MANDATORY — レビュー種別が requirement の場合のみ適用]

**背景**: 要件定義書は What レイヤ、設計書は How レイヤ。実装手段（How）は要件定義書から委譲して良い。ただし**委譲意思があっても要件定義書本体に TBD-xxx 形式（ID・内容・期限）で記載されていない限り、設計フェーズで拾い上げる仕組みが機能しない**。よって委譲対象の指摘は skip で消失させず、fix で要件定義書本体への TBD-xxx 化を促す。

**判定基準のソース・オブ・トゥルース**:

カテゴリ分類（表1）・委譲明示段階（表2）・判定マトリクス（表3）の詳細は、プラグイン標準 `${CLAUDE_PLUGIN_ROOT}/skills/review/docs/review_criteria_requirement.md` の **verifiability perspective「判定原則: 設計書委譲の判定」** セクションに集約されている。重複定義を避けるため evaluator では再掲しない。判定時は同セクションの 3 表を参照する。

> **注**: 表 1〜3 はプラグイン標準 criteria 側にしか存在しない。DocAdvisor 追加 perspective でプロジェクト固有ルール文書が `criteria_path` に来た場合に表 1〜3 を読めなくなる事態を防ぐため、Step 1 の手順 5 でレビュー種別が `requirement` の場合のプラグイン標準 criteria の無条件併読を必須化している。

**evaluator の判定アクション**:

criteria の表3 に従う:

- **完全明示**（要件定義書本体の `## 未確定事項` 表に TBD-xxx として ID・内容・期限が記載）→ 本来 reviewer は指摘しないが、誤って指摘された場合は `recommendation: skip` / `skip_reason: "false_positive"` / `reason: "TBD-xxx で完全明示済み（要件定義書として正しい状態）"`
- **部分明示**（散文での委譲記述のみ）→ `recommendation: fix` / `auto_fixable: false` / `reason: "TBD-xxx 形式への整形と期限明示を要件定義書に反映する必要がある（criteria 表3）"`
- **委譲明示なし** → カテゴリに応じて修正方向を提示（実装手段: TBD-xxx 化 / ユーザー価値定量目標: 値記載 or TBD-xxx 化 / その他: 定量化）。`recommendation: fix` / `auto_fixable: false`

**判定例:**

| 指摘内容                                     | 要件側の状態                                                 | 判定                                                                |
| -------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------- |
| 「Embedding モデル名が未記載」               | 散文で「モデルは設計書で確定」と明示（部分明示）             | `fix` / `auto_fixable: false`（TBD-xxx 形式に整形 + 期限追加）      |
| 「score 統合方式が曖昧」                     | 散文で「設計時に確定」と明示（部分明示）                     | `fix` / `auto_fixable: false`（TBD-xxx 形式に整形 + 期限追加）      |
| 「LLM Rerank の top-N が未定義」             | TBD-007 として「設計開始前」と未確定事項表に記載（完全明示） | reviewer は通常指摘しないはず。誤検出なら `skip` / `false_positive` |
| 「chunk 分割上限が曖昧」                     | 委譲明示なし                                                 | `fix` / `auto_fixable: false`（TBD-xxx として未確定事項表に追記）   |
| 「recall 合格ラインが未定義」                | TBD-009 として「評価実施前」と未確定事項表に記載（完全明示） | reviewer は通常指摘しないはず。誤検出なら `skip` / `false_positive` |
| 「recall 合格ラインが未定義」                | 委譲明示なし                                                 | `fix` / `auto_fixable: false`（具体値記載 or TBD-xxx 化）           |
| 「応答時間目標が未定義」                     | 委譲明示なし                                                 | `fix` / `auto_fixable: false`（具体値記載 or TBD-xxx 化）           |
| 「使いやすい UI と書かれているが基準がない」 | 受け入れ基準が主観表現のみ                                   | `fix` / `auto_fixable: false`（定量的・客観的基準に置換）           |
...
| -------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `out_of_scope`       | モードによる吟味対象外      | `--auto` で 🟢 / `--auto-critical` で 🟡🟢                                                                                            |
| `false_positive`     | reviewer の誤認識・過剰指摘 | 対象ファイルを Read して問題不在を確認。requirement レビューでは「TBD-xxx で完全明示済み」のケースもこの値で扱う（criteria 表3 参照） |
...
>
> **設計判断**: 当初検討した `deferred_to_design` は採用しない。設計フェーズ委譲が要件定義書本体に TBD-xxx として完全明示されている状態は要件定義書として正しい記述であり、reviewer が指摘した場合は `false_positive` で扱う。委譲意思のみの部分明示や明示なしは fix で TBD-xxx 化を促す（skip による消失を防ぐため）。詳細は criteria の verifiability「判定原則: 設計書委譲の判定」参照。
