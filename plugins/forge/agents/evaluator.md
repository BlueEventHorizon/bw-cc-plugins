...
- session_dir 配下の `refs.yaml` / `review_<種別>.md` / 参考文書 (reference_docs / related_code / ssot_refs[].path) / target_files の Read
...
5. `review_packet.ssot_refs[]` の全 `path` を Read → 規範本体 + 重大度カタログ + グレーゾーン許容範囲を把握 (FNC-411)
...
| `review_packet.ssot_refs[]` の path が読めない      | severity 委譲経路が壊れているためエラー return                             |
