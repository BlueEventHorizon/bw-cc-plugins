...
- refs.yaml / review_criteria_*.md / ssot_refs[].path / reference_docs / target_files の Read
...
| `ssot_refs[]`     | criteria の §1 SSOT参照表から抽出した規範本体 + 重大度カタログのパス配列。各要素は `{ path, priority: "P1"\|"P2"\|"P3", doc_type: "rules"\|"principles"\|"format" }` 形式 (ADR-032 で旧 `doc_path` から `path` に統一) |
| `check_order`     | criteria の §2 チェック順 (`["P1", "P2", "P3"]`)。上から実行する                                                                                                                                                       |
| `severity_source` | severity の SoT (固定値 `"principles"`)。reviewer は ssot_refs[].path 側の重大度カタログから severity を取得する (FNC-411)                                                                                             |
| `output_filename` | レビュー結果の出力先ファイル名 (session_dir 内の sandbox ファイル名)。**種別ベース命名** `review_<種別>.md` (例: `review_code.md` / `review_design.md`)。ADR-032 で旧 `output_path` から改名                           |
...
- 出力: `{session_dir}/{output_filename}` に**新規 finding を append する** (既存 finding は上書き・削除しない)。append フォーマットは通常モードと同一の severity セクション (🔴/🟡/🟢) を使用するため、orchestrator が `extract_review_findings.py` を再実行することで新 finding が連番 id で plan.yaml に統合される。findings がなければ `output_filename` の中身を変更しない (空 append しない)
...
2. `refs.yaml` から以下を取得する:
   ...
   - `review_packet`: 入力契約セクション (`criteria_path` / `ssot_refs[]` / `check_order` / `severity_source` / `output_filename`)
3. `review_packet.criteria_path` を Read する (種別ベース criteria。例: `review_criteria_code.md`)
4. `review_packet.ssot_refs[]` の全 `path` を Read する (規範本体 + 重大度カタログを把握)
...
1. finding に該当する規範を `ssot_refs[].path` 側で特定する
2. その path の **重大度カタログ** (FNC-411 拡充節) から該当規範の severity (`🔴 critical` / `🟡 major` / `🟢 minor`) を取得する
3. finding の `severity` フィールドにその値を **転記** する
4. finding の `severity_source` フィールドに「取得元 path + 該当節」を記載する
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/review/scripts/run_review_engine.sh {session_dir}/{output_filename} <project_dir> "<prompt>"
```

| 終了コード | 意味                                        | 次のアクション          |
| ---------- | ------------------------------------------- | ----------------------- |
| 0          | 成功 (output_filename に結果が書き出された) | Phase 4 完了            |
| 2          | Codex が見つからない                        | Claude フォールバックへ |
| 1          | Codex 実行エラー                            | エラー報告              |

スクリプトは `codex exec -o` の出力先を中間ファイル (`{output}.codex_lastmsg.txt`) に分離し、stdout 経由で受け取った会話全体から `extract_codex_output.py` が Markdown 本文を抽出して `{session_dir}/{output_filename}` に書き出す。
...
Agent 自身がレビューを実行し、結果を `{session_dir}/{output_filename}` に直接 Write する (別 Agent を起動しない)。Codex 不在 (`run_review_engine.sh` exit code=2) の場合も同じ経路で完結させる。orchestrator は fallback 用に 2 体目の reviewer を起動しない (FNC-412)。
...
- ssot_refs[]:
  - { path: <path>, priority: P1, doc_type: <rules|principles|format> } の全要素を列挙
...
- 各 finding の severity_source に「取得元 path + 該当節」を必須で記載
...
| `severity_source` | severity を取得した委譲先 path + 該当節。判断の追跡可能性を保証                                                   |
...
レビュー完了後、結果は `{session_dir}/{output_filename}` (= `review_<種別>.md`) に保存済みの状態となる (Codex はリダイレクト、Claude/Agent 自身は Write)。
