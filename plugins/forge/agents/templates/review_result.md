...
8. **severity_source 必須**: severity は委譲先 principles 側カタログから転記すること (reviewer は自ら判定しない)。`severity_source` フィールドに取得元 path + 該当節を必ず記載する (FNC-411。ADR-032 で `doc_path` から `path` に統一)
