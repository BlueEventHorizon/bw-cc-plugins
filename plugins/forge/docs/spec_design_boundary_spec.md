...
**最初の判断基準で Yes となったものには適用しない**（決定済みの事項は、以下の質問で設計書へ振り分けられてはならない）。
...
機能の存在理由・ユーザーにとっての価値は、要件定義書にのみ現れる構成要素であり、本節の判断基準には掛けない。「ユーザーマニュアルに書くか」では拾えないが、設計書に書く対象でもないためである（§4.7）。
...
```mermaid
flowchart TD
    A[この記述はどこに書くべき？] --> G{既に決まっていて<br/>選択の余地が無いか？}
    G -->|Yes| C[要件定義書<br/>What]
    G -->|No| B{ユーザーマニュアルに<br/>書く内容か？}
    B -->|Yes| C
...
そもそも数値化すべきでない。
→ [spec_priorities_spec.md](spec_priorities_spec.md) §3 参照。
...
- 設計書 ❌:「本設計により利用者の入力負担が下がる」（§4.6 の保証表現と同類）
...
- ユーザーが理解できる内容
- 実装方法に依存しない。ただし既に決まっていて選択の余地が無い事項は、具体的に書く

**設計書は「実装ガイド」**

- 開発者が実装できる詳細
- 技術的な詳細を含む。要件で決まっていない事項を、どう作るかとして定める

### 7.2 迷ったら

1. 「既に決まっていて選択の余地が無いか？」で判断
2. 決まっていなければ「ユーザーマニュアルに書くか？」で判断
3. それでも迷ったら、人間に確認
4. グレーゾーン（型定義等）は実用上許容
...
| 機能の存在理由・ユーザーにとっての価値が要件定義書に記載されていない | §4.7 | 🟡 major       | 何のために作るかが要件から辿れず、設計・実装の判断根拠が失われる         |
| 要件定義書に手段を含む数値目標 (Redis を使って 200ms 切る等)         | §4.6 | 🔴 critical    | 実装手段を要件に混入 (技術依存)                                          |

### 8.2 §6 グレーゾーン (型 / 図表)

§6 は「許容する」と明示しているが、許容外を見落とすケースがあるため重大度カタログで補強する。
