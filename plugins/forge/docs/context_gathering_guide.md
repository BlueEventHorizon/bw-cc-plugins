...
```
session_dir: {session_dir}           # セッションディレクトリのパス
guide: plugins/forge/docs/context_gathering_guide.md  # このガイドのパス
steps: [1, 2, 3]                     # 実行する Step 番号のリスト
feature: "login"                     # 対象 Feature 名（Step 2, 5 の検索キーワード）
skill_type: "設計書作成"              # 作業種別（Step 3 の検索キーワード）
```
...
- `feature` と `skill_type` は検索精度を上げるためのヒントとして活用する
