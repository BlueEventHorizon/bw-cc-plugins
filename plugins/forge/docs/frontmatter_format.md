...
feature_note:
  - この文書が、この文書の対象範囲における現在の仕様である。旧仕様（ソースコード・設計書・計画書）の記述は現在の仕様ではなく、矛盾の有無に関わらずこの文書へ置き換わっている。
...
feature_type: temporary-feature
feature_note:
  - 本設計書が、本設計書の対象範囲における現在の設計である。旧設計書の記述は現在の設計ではなく、矛盾の有無に関わらず本設計書へ置き換わっている。本設計書と対応する追加 feature 要件定義書（REQ-xxx）が食い違う場合は要件定義書に従う。
  - 旧仕様ファイルは本 feature 実装完了まで書き換えない。新規ファイル / 新規ディレクトリとして切り出すこと。
...
`feature_type` とは意味論が異なる独立したキーである。差分 feature 運用（当該範囲の現在の仕様であること・merge 予定）を示す `feature_type` に対し、`doc_status` は「文書がまだ完全な状態ではない理由」を機械可読に示す。
...
`feature_type`/`feature_note` と併記する場合は同一 frontmatter ブロック内に両方のキーを列挙してよい（意味論が独立した別軸のキーであるため、片方の有無はもう片方の判定に影響しない）。**キーの順序は問わない。** 順序に意味を持たせないため、順序によって値の解釈が変わる読み取り実装を作ってはならない。
