...
| `files`         | object[] | レビュー対象パスエントリ一覧（`--files` 指定。未指定時 `[]`）。各要素は `path` フィールドを持つ dict (ADR-032) |
...
> **session.yaml のフィールド区分** (ADR-032): `files[]` のみ path entry 配列 (`[{path}]`)。他のメタフィールド (`skill` / `started_at` / `last_updated` / `status` / `review_type` / `engine` / `auto_count` / `current_cycle` / `feature` / `mode` / `output_dir` / `task_id`) は flat scalar として保持する。
...
| `plan.yaml`（更新）        | `evaluator` → `present-findings` → `review` / `fixer` | evaluator は `apply_eval.py` 経由で直接更新（中間ファイル廃止 Issue #103）         |
...
> **パス系フィールドの標準形式** (ADR-032): refs.yaml の全パス系フィールドは「パスリスト形式 (`pathN`)」または「単一スカラー形式 (`path1`)」のいずれかで表現する。パスリストは常に dict 配列 (`[{path, ...optional metadata}]`)、文字列配列は不可。詳細は ADR-032 §「決定」を参照。

| フィールド       | 型       | 必須 | 説明                                                                                                         |
| ---------------- | -------- | ---- | ------------------------------------------------------------------------------------------------------------ |
| `target_files`   | object[] | 必須 | レビュー対象パスエントリ一覧。各要素は `path` を持つ dict (ADR-032)                                          |
| `reference_docs` | object[] | 必須 | 参考文書リスト（各要素は `path` フィールドを持つ）                                                           |
...
| `ssot_refs`       | object[] | 必須 | SSOT 参照リスト（非空）。各要素は `path`, `priority` (`P1`/`P2`/`P3`), `doc_type` (`rules`/`principles`/`format`) を持つ (ADR-032 で旧 `doc_path` から `path` に改名)                                           |
| `check_order`     | string[] | 必須 | reviewer がチェックする観点の順序（非空、例: `["P1", "P2", "P3"]`）                                                                                                                                             |
| `severity_source` | string   | 必須 | severity 判定根拠となる principles ファイルパス (例: `plugins/forge/docs/review_priorities_spec.md`)                                                                                                            |
| `output_filename` | string   | 必須 | reviewer 出力ファイル名（種別固定、`^review_[a-z0-9_-]+\.md$` 形式）。session_dir 内の出力先 sandbox ファイル名であり外部参照 `path` とは別概念 (ADR-032 で旧 `output_path` から改名)。`../` および絶対パス不可 |
...
target_files:
  ...
  - path: plugins/forge/skills/present-findings/SKILL.md
...
related_code:
  - path: plugins/forge/agents/reviewer.md
    reason: 同種 AI 専用 Agent の frontmatter 参考
    lines: "1-30"
  - path: plugins/forge/agents/evaluator.md
    reason: 同種 AI 専用 Agent の frontmatter 参考
```
...
| `evaluator`                  | Bash → `apply_eval.py`（検証・priority ソート・plan.yaml 一括更新を 1 ステップで完結）                       | 判定完了後      |
...
| フィールド       | 型       | 必須 | 説明                                                                                                                            |
| ---------------- | -------- | ---- | ------------------------------------------------------------------------------------------------------------------------------- |
| `target_files`   | object[] | ○    | レビュー対象パスエントリ (各要素は `path` フィールドを持つ dict、ADR-032)                                                       |
| `reference_docs` | object[] | ○    | 参考文書（空配列可、各要素は `path` フィールドを持つ）                                                                          |
| `review_packet`  | object   | ○    | reviewer に渡すレビュー基準パッケージ（`criteria_path`, `ssot_refs`, `check_order`, `severity_source`, `output_filename` 必須） |
| `related_code`   | object[] | -    | 関連コード（各要素は `path` / `reason` 必須、`lines` 任意）                                                                     |

**バリデーション**:

- 旧 `perspectives[]` スキーマは存在検出時に拒否（DES-028 §2.3 / FNC-412 で完全撤廃済み・回帰防止）
- `target_files[]` の各要素は dict、`path` フィールド必須 (ADR-032 で旧 string array から改修)
- `review_packet.ssot_refs[].path` は必須（ADR-032 で Issue #99 の `doc_path` 改名を覆し `path` に統一）
- `review_packet.ssot_refs[].priority` は `P1` / `P2` / `P3` のいずれか
- `review_packet.ssot_refs[].doc_type` は `rules` / `principles` / `format` のいずれか
- `review_packet.output_filename` は `^review_[a-z0-9_-]+\.md$` 形式、`../` および絶対パスは拒否 (ADR-032 で旧 `output_path` から改名)
- 旧キー名 (`doc_path` / `output_path`、`target_files` の string 配列) を渡した場合は明示的 ValueError で reject
...
`write_plan(session_dir, plan_data)` は `SessionStore` 経由で `plan.yaml` を atomic に保存する。`apply_eval.py` も同じ関数で書き戻す。
