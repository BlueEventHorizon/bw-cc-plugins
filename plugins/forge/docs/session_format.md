...
`session.yaml` は成果物本文やレビュー項目の正規データを持たない。再開時の概況把握のため、浅い進行状態だけを flat YAML として保持する。

| フィールド        | 型     | 必須 | 既定値        | 説明                                                                           |
| ----------------- | ------ | ---- | ------------- | ------------------------------------------------------------------------------ |
| `phase`           | string | ○    | `created`     | 粗い進行段階。標準値は下表参照                                                 |
| `phase_status`    | enum   | ○    | `in_progress` | `pending` / `in_progress` / `completed` / `failed`                             |
| `focus`           | string | ○    | `""`          | 進行状況表示用の短い焦点。改行は空白へ正規化する                               |
...
実装上は `SessionStore.write_nested_yaml()` を使い、`refs.yaml` の atomic write と meta 更新を同じ経路で実行する。
