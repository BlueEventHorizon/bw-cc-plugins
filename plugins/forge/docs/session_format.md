...
flowchart TD
    ...
    Found -->|なし| Create
    Found -->|あり| Ask[AskUserQuestion:<br/>削除 or 残す?<br/>review は再開判断も許容]
    Ask -->|再開| Reuse[既存 session_dir を使用]
    Ask -->|削除| Cleanup[session_manager.py cleanup]
    Ask -->|残す| Create
    Cleanup --> Create
...
`status: "found"` の場合は AskUserQuestion で次の扱いを確認する:
...
`extract_review_findings.py {session_dir}` は `review_*.md` を統合して `review.md` / `plan.yaml` を生成する。`--review-only` は `plan.yaml` を保護する再生成モードであり、`review.md` のみ書き出す。
