...
| 判定ルール (fix / create_issue / skip / needs_review の切替) | **持つ** (運用フロー) | criteria                             |
...
| 3 | `## 3. 判定ルール` | `recommendation: fix` / `create_issue` / `skip` / `needs_review` の切替条件 (運用フロー、DES-028 §4.3)              |
...
| `needs_review` | 対象を読んでも判断が難しく人間判断が必要 (上記いずれにも該当しないとき。観点 2 設計意図整合または観点 3 副作用限定が不成立、DES-028 §4.3) |
```
