...
| P1       | [additive_development_spec.md](../additive_development_spec.md)                          | principles | 追加開発ワークフロー (計画書は frontmatter を持たず、`requirements_traceability` 先の要件定義書の `feature_type: temporary-feature` で追加 feature かを判定する §6-3 / 旧仕様優先度 §2 / P2 矛盾除外規定の前提を提供) |
...
1. **P1 ルール合致**: [plan_format.md](../plan_format.md) → [plan_principles_spec.md](../plan_principles_spec.md) → 関連 `docs/specs/<feature>/design/*.md` / `docs/specs/<feature>/requirements/*.md` → `(query-db-rules: "計画書レビューに関するプロジェクト固有の依存関係・ワークフロー規約")` (プロジェクト固有の依存関係・ワークフロー規約) の順で対象 plan.yaml と照合する (YAML 構造 → タスク内容の原則 → 要件・設計との対応 → プロジェクト固有 rules)
   - **追加 feature 判定**: target plan.yaml が追加 feature のものかは、計画書自身の frontmatter ではなく `requirements_traceability` が参照する要件定義書の `feature_type: temporary-feature` frontmatter ([additive_development_spec.md](../additive_development_spec.md) §6-3) で判定する。計画書には frontmatter を付与しないため、その欠如を指摘しない
