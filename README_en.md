...
For the details of switching — the `.claude/.forge.yaml` syntax, the `--backend` forcing argument that only the index-update skills (`/forge:update-db-rules` / `update-db-specs`) accept, and the separate review-executor axis (`agent-review`) — see [switch-forge-backend.md](docs/readme/switch-forge-backend.md).
...
| Plan           | start-plan         | Design docs                  | Plan (JSON)                  |
...
| -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| [**review**](docs/readme/forge/guide_review.md)                                        | Whatever runs the skill drives request assembly, finding evaluation, fixing, re-request, and completion; the round-trip with the reviewer is delegated to a review backend SKILL. Pick one with `--backend`, or let forge probe the candidates in order (the only current candidate is agent-review, which has no external dependencies). Request/resume modes (resume only on backends that restore history; no such backend exists at present). `--secrets` scans for leaked secrets; `--scope` states the intended goal and deliberate omissions | `"review"`                            |
| _consult_                                                                              | Present review findings one at a time and drive the accept/reject discussion                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ※ Called by review                    |
...
| [**start-plan**](docs/readme/forge/guide_create_docs.md#start-plan)                    | Extract tasks from design docs into a JSON plan                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `"start plan"`                        |
...
| **create-feature-from-markdown-plan**                                                  | Turn a Claude plan-mode Markdown plan into requirements and design docs in one pass (forge's own `{feature}_plan.json` is out of scope)                                                                                                                                                                                                                                                                                                                                                                                                             | `"create feature from markdown plan"` |
