...
| プラグイン | バージョン | 説明 |
|-----------|-----------|------|
| **kaizen** | 0.0.4 | AI によるコード・文書レビュー。段階的な対話提示と自動修正に対応。文書構造管理を統合 |
...
AI によるコード・文書レビュー。段階的な対話提示と自動修正に対応。`.doc_structure.yaml` によるプロジェクト文書構成の管理機能を統合。
...
| `init-kaizen` | 可能 | プロジェクトのディレクトリをスキャン・分類し、`.doc_structure.yaml` を生成 |
...
`init-kaizen` スキルがプロジェクトのディレクトリをスキャンして Markdown ファイルを検出し、対話的に分類して `.doc_structure.yaml` を生成します。kaizen はこのファイルを直接読み込んで、レビュー・修正時の参考文書を収集します。

完全なスキーマ仕様は [docs/doc_structure_format.md](docs/doc_structure_format.md) を参照してください。
