...
8. plugin.json のバージョンが 0.3.0 以上 (Issue #54 で導入された最小バージョン)
...
# Issue #54 の auto モード再定義が導入された最小バージョン。
# 固定一致だと以降のバンプで必ず壊れるため、下限比較で「バンプ済み」を検証する。
MIN_VERSION = (0, 3, 0)
...
class TestPluginVersionBumped(unittest.TestCase):
    """plugin.json / marketplace.json のバージョンが 0.3.0 以上であること (受け入れ #8)"""
    ...
    def test_plugin_json_version(self):
        ...
        self.assertIsNotNone(
            version,
            "plugins/doc-advisor/.claude-plugin/plugin.json に version がない",
        )
        self.assertGreaterEqual(
            _parse_version(version),
            MIN_VERSION,
            f"plugins/doc-advisor/.claude-plugin/plugin.json の version {version} "
            f"が下限 {'.'.join(map(str, MIN_VERSION))} 未満",
        )
    ...
    def test_marketplace_json_version(self):
        ...
        version = doc_advisor_entry.get('version')
        self.assertGreaterEqual(
            _parse_version(version),
            MIN_VERSION,
            f"marketplace.json の doc-advisor.version {version} "
            f"が下限 {'.'.join(map(str, MIN_VERSION))} 未満",
        )
