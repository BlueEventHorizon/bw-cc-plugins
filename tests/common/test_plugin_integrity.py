...
class TestReferencedFilesExist(unittest.TestCase):
    ...
    def test_review_criteria_default_exists(self):
        ...
        criteria_dir = PLUGINS_DIR / 'forge' / 'docs' / 'criteria'
        expected_files = [
    ...
    def test_review_target_resolution_script_exists(self):
        """review Skill が参照する対象解決スクリプトが存在"""
        path = PLUGINS_DIR / 'forge' / 'skills' / 'review' / 'scripts' / 'resolve_targets.py'
        self.assertTrue(path.is_file(), f'{path} が存在しない')
...
class TestSchemaConsistency(unittest.TestCase):
    ...
    def test_parser_supports_all_documented_fields(self):
        ...
        import sys
        sys.path.insert(0, str(PLUGINS_DIR / 'forge' / 'scripts' / 'doc_structure'))
