#!/usr/bin/env python3
"""
start-implement/scripts/init_session.py のテスト

session_manager.py init を透過的に呼び出すラッパーを検証する。
位置引数 {feature} {task_id} → --feature / --task-id へのマッピング、
ハードコード --skill、exit code 透過を確認。
"""
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'start-implement' / 'scripts' / 'init_session.py')
...
def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_init_session_start_implement", WRAPPER_PATH
    )
...
class TestInitSessionWrapper(unittest.TestCase):
    ...
    def test_positional_args_mapped_to_flags(self):
        """位置引数 {feature} {task_id} が --feature / --task-id に変換される"""
        with mock.patch.object(self.wrapper.subprocess, "run") as mock_run:
            mock_run.return_value = subprocess.CompletedProcess(args=[], returncode=0)
            argv = ["init_session.py", "my_feature", "TASK-042"]
        ...
        self.assertEqual(cmd[cmd.index("--task-id") + 1], "TASK-042")
        ...
        for code in (0, 1, 2, 42):
            with self.subTest(code=code):
                with mock.patch.object(self.wrapper.subprocess, "run") as mock_run:
                    ...
                    argv = ["init_session.py", "f", "TASK-001"]
                    with mock.patch.object(sys, "argv", argv):
                        rc = self.wrapper.main()
