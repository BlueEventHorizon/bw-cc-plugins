...
が **まだ存在する** ことも合わせて確認し、TASK-032〜036 完了後に削除されれば
そのテストは自然と falsify される (= merge 完了の検出器を兼ねる)。
...
# Phase 5 (addendum merge) 完了までは reviewer が SSOT として参照する 4 ファイルが
# `docs/specs/forge-review/principles/` 側に存在する。merge 完了 (TASK-032〜036) で
# これらは削除される。本テストはそれまでの「生きている回帰検出」として機能する。
ADDENDUM_FILES = {
...
class TestReviewIntegration(unittest.TestCase):
    ...
    # ------------------------------------------------------------------
    # 10. addendum 一時フィクスチャの整合性
    #     (Phase 5 addendum merge 完了時にこの test が落ちれば merge 検出器になる)
    # ------------------------------------------------------------------

    def test_addendum_fixture_present_until_merge(self):
        """addendum 4 ファイルが `docs/specs/forge-review/principles/` に存在する。

        DES-028 §5.1: addendum merge は TASK-032〜036 で実施され、merge 完了後に
        addendum ファイルは削除される。本テストは merge 未完了時点での「reviewer
        SSOT 参照の補助フィクスチャが揃っている」ことを保証し、merge 完了時には
        意図的に falsify されることで merge の機械的検出を兼ねる。

        merge 完了後は本テストを削除 (または書き換え) すること (DES-028 §5.1 注記)。
        """
        for name in ADDENDUM_FILES:
            with self.subTest(addendum=name):
                ...
                self.assertTrue(
                    p.is_file(),
                    f"addendum {name} が見つからない: "
                    "Phase 5 merge 完了済みなら本テストを削除すること "
                    "(DES-028 §5.1)",
                )
