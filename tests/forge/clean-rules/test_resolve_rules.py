#!/usr/bin/env python3
"""
clean-rules/scripts/resolve_rules.py のテスト

resolve_doc_structure.py --type rules を透過的に呼び出すラッパーを検証する。

実行:
  python3 -m unittest tests.forge.clean-rules.test_resolve_rules -v
"""
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'clean-rules' / 'scripts' / 'resolve_rules.py')
EXPECTED_TYPE = "rules"
EXPECTED_LOW_LEVEL = (REPO_ROOT / 'plugins' / 'forge' / 'scripts'
                      / 'doc_structure' / 'resolve_doc_structure.py')


def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_resolve_rules_clean_rules", WRAPPER_PATH
    )
...
class TestResolveRulesWrapper(unittest.TestCase):
    """resolve_rules.py が resolve_doc_structure.py --type rules を透過的に呼ぶことを検証"""
    ...
    def test_subprocess_called_with_type(self):
        """subprocess.run が --type rules で呼ばれる"""
