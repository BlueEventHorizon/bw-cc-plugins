#!/usr/bin/env python3
"""
start-design/scripts/resolve_doc.py のテスト

resolve_doc_structure.py --doc-type design を透過的に呼び出すラッパーを検証する。

実行:
  python3 -m unittest tests.forge.start-design.test_resolve_doc -v
"""
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'start-design' / 'scripts' / 'resolve_doc.py')
EXPECTED_DOC_TYPE = "design"
...
def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_resolve_doc_start_design", WRAPPER_PATH
    )
...
class TestResolveDocWrapper(unittest.TestCase):
    """resolve_doc.py が resolve_doc_structure.py --doc-type design を透過的に呼ぶことを検証"""

    def setUp(self):
        self.wrapper = _load_wrapper()

    def test_doc_type_hardcoded(self):
        """DOC_TYPE 定数が design にハードコードされている"""
    ...
    def test_subprocess_called_with_doc_type(self):
        """subprocess.run が --doc-type design で呼ばれる"""
