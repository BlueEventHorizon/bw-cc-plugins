...
from update_version_files import (
    FilterNotFoundError,
    VersionDriftError,
...
class TestVersionPath(unittest.TestCase):
    ...
    def test_three_level_path_with_duplicate_intermediate_key_updates_correct_block(self):
        ...
        content = (
            '{\n'
            '  "a": {"package": {"version": "999.88.7"}},\n'
            '  "b": {"package": {"version": "999.88.7"}}\n'
            '}'
        )
        result = update_version_in_text(
            content, "999.88.7", "999.88.8", version_path="b.package.version"
        )
        # b 配下のみ更新され、a 配下は無傷であること
        self.assertIn('"a": {"package": {"version": "999.88.7"}}', result)
    ...
    def test_nested_path_parent_value_not_object_raises(self):
        ...
        content = (
            ...
            '  "other": {"version": "999.88.7"}\n'
            '}'
        )
        with self.assertRaises(ValueError):
            update_version_in_text(
                content, "999.88.7", "999.88.8", version_path="package.version"
            )
    ...
    def test_nested_path_brace_in_string_does_not_break_scope(self):
        ...
        content = (
            '{\n'
            '  "package": {\n'
            '    "description": "uses {braces} and \\"quotes\\"",\n'
            '    "version": "999.88.7"\n'
            '  }\n'
            '}'
        )
    ...
    def test_path_prefix_version_not_corrupted(self):
        """境界: old=0.6.1 が "0.6.10" を前方一致で破壊しない
        （見つからないが、フィールド自体は存在するため VersionDriftError）"""
        content = '{\n  "version": "0.6.10"\n}'
        with self.assertRaises(VersionDriftError):
...
class TestVersionPathDrift(unittest.TestCase):
    ...
    def test_nested_field_exists_but_value_mismatch_raises_drift_error(self):
        """ネストフィールドは存在するが値が old_version と不一致 → VersionDriftError"""
        content = '{\n  "package": {\n    "version": "0.2.2"\n  }\n}'
        with self.assertRaises(VersionDriftError):
            update_version_in_text(content, "999.88.7", "999.88.8", version_path="package.version")

    def test_field_truly_absent_still_raises_plain_value_error(self):
        """フィールド自体が存在しない場合は従来通り ValueError（VersionDriftError ではない）"""
        content = '{"name": "test"}'
        with self.assertRaises(ValueError) as ctx:
            update_version_in_text(content, "999.88.7", "999.88.8", version_path="version")
        self.assertNotIsInstance(ctx.exception, VersionDriftError)

    def test_similarly_named_field_does_not_cause_false_drift(self):
        """単独修正レビューで検出: "other_version" のような無関係なフィールドが
        "version" 検索にマッチして誤って VersionDriftError にならないこと"""
        content = '{"other_version": "1.2.3"}'
        with self.assertRaises(ValueError) as ctx:
            update_version_in_text(content, "999.88.7", "999.88.8", version_path="version")
        self.assertNotIsInstance(ctx.exception, VersionDriftError)
...
class TestCLI(unittest.TestCase):
    ...
    def test_optional_drift_cli(self):
        ...
        content = '| **forge** | 0.2.2 | desc |'
        result = self._run(
            content, "999.88.7", "999.88.8", "--filter", "**forge**", "--optional"
        )
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout, "")
        status = json.loads(result.stderr)
    ...
    def test_drift_without_optional_is_error_cli(self):
        """--optional なしで drift の場合は従来通り exit 1"""
        content = '| **forge** | 0.2.2 | desc |'
        result = self._run(
            content, "999.88.7", "999.88.8", "--filter", "**forge**"
        )
        self.assertEqual(result.returncode, 1)
    ...
    def test_optional_path_drift_cli(self):
        ...
        result = self._run(
            content, "999.88.7", "999.88.8", "--version-path", "version", "--optional"
        )
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout, "")
        status = json.loads(result.stderr)
    ...
    def test_path_drift_without_optional_is_error_cli(self):
        """--optional なしで path モードの drift の場合は従来通り exit 1"""
        content = '{"version": "0.2.2"}'
        result = self._run(
            content, "999.88.7", "999.88.8", "--version-path", "version"
        )
        self.assertEqual(result.returncode, 1)
    ...
    def test_file_not_found_with_optional_is_skipped_cli(self):
        ...
        result = subprocess.run(
            [sys.executable, str(SCRIPTS_DIR / 'update_version_files.py'),
             '/nonexistent.json', '999.88.7', '999.88.8', '--optional'],
            capture_output=True, text=True, timeout=10,
        )
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout, "")
