#!/usr/bin/env python3
"""
start-plan/scripts/init_session.py のテスト
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'start-plan' / 'scripts' / 'init_session.py')
...
def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_init_session_start_plan", WRAPPER_PATH
    )
...
class TestInitSessionWrapper(unittest.TestCase):
    """init_session.py が session_manager.py init を透過的に呼ぶことを検証"""
    ...
    def test_positional_args_mapped_to_flags(self):
        """位置引数 {feature} {mode} {output_dir} が正しく flag に変換される"""
        with mock.patch.object(self.wrapper.subprocess, "run") as mock_run:
            mock_run.return_value = subprocess.CompletedProcess(args=[], returncode=0)
            argv = ["init_session.py", "my_feature", "new", "/tmp/out"]
        ...
        self.assertEqual(cmd[cmd.index("--mode") + 1], "new")
        ...
        for code in (0, 1, 2, 42):
            with self.subTest(code=code):
                with mock.patch.object(self.wrapper.subprocess, "run") as mock_run:
                    ...
                    argv = ["init_session.py", "f", "new", "/tmp/out"]
                    with mock.patch.object(sys, "argv", argv):
                        rc = self.wrapper.main()
