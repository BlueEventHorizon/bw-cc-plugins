#!/usr/bin/env python3
"""
start-plan/scripts/resolve_doc.py のテスト

resolve_doc_structure.py --doc-type plan を透過的に呼び出すラッパーを検証する。

実行:
  python3 -m unittest tests.forge.start-plan.test_resolve_doc -v
"""
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'start-plan' / 'scripts' / 'resolve_doc.py')
...
def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_resolve_doc_start_plan", WRAPPER_PATH
    )
