#!/usr/bin/env python3
"""
query-db-specs/scripts/sync_documents.py のテスト

sync_docdb.py を category=specs 固定で透過呼び出しするラッパーを検証する
（DES-057 §9.2: category 固定値・引数透過・stdout/stderr/exit code 透過・
--start / --status <job_id> の両操作の透過）。
...
helpers = _load(
    ...
    "_wrapper_helpers_query_db_specs_sync",
)
...
class TestSyncDocumentsWrapper(unittest.TestCase):
    """sync_documents.py が sync_docdb.py を category=specs で透過的に呼ぶ"""

    def setUp(self):
        self.wrapper = helpers.load_wrapper(
            helpers.wrapper_path(SKILL, "sync_documents.py"),
            "_sync_documents_query_db_specs",
        )
