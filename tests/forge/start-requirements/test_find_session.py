#!/usr/bin/env python3
"""
start-requirements/scripts/find_session.py のテスト

session_manager.py find --skill start-requirements を透過的に呼び出すラッパーを検証する。
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'start-requirements' / 'scripts' / 'find_session.py')
...
def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_find_session_start_requirements", WRAPPER_PATH
    )
