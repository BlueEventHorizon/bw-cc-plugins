#!/usr/bin/env python3
"""
start-requirements/scripts/resolve_doc.py のテスト

resolve_doc_structure.py --doc-type requirement を透過的に呼び出すラッパーを検証する。

実行:
  python3 -m unittest tests.forge.start-requirements.test_resolve_doc -v
"""
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'start-requirements' / 'scripts' / 'resolve_doc.py')
...
def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_resolve_doc_start_requirements", WRAPPER_PATH
    )
...
class TestResolveDocWrapper(unittest.TestCase):
    """resolve_doc.py が resolve_doc_structure.py --doc-type requirement を透過的に呼ぶことを検証"""

    def setUp(self):
        self.wrapper = _load_wrapper()

    def test_doc_type_hardcoded(self):
        """DOC_TYPE 定数が requirement にハードコードされている"""
    ...
    def test_subprocess_called_with_doc_type(self):
        """subprocess.run が --doc-type requirement で呼ばれる"""
        ...
        self.assertIn("--doc-type", cmd)
        idx = cmd.index("--doc-type")
        self.assertEqual(cmd[idx + 1], EXPECTED_DOC_TYPE)
