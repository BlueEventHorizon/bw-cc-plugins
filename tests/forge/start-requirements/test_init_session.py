#!/usr/bin/env python3
"""
start-requirements/scripts/init_session.py のテスト
...
WRAPPER_PATH = (REPO_ROOT / 'plugins' / 'forge' / 'skills'
                / 'start-requirements' / 'scripts' / 'init_session.py')
...
def _load_wrapper():
    spec = importlib.util.spec_from_file_location(
        "_init_session_start_requirements", WRAPPER_PATH
    )
...
class TestInitSessionWrapper(unittest.TestCase):
    ...
    def test_positional_args_mapped_to_flags(self):
        with mock.patch.object(self.wrapper.subprocess, "run") as mock_run:
            ...
            argv = ["init_session.py", "my_feature", "interactive", "/tmp/out"]
        ...
        self.assertEqual(cmd[cmd.index("--mode") + 1], "interactive")
        ...
        for code in (0, 1, 2, 42):
            with self.subTest(code=code):
                with mock.patch.object(self.wrapper.subprocess, "run") as mock_run:
                    ...
                    argv = ["init_session.py", "f", "interactive", "/tmp/out"]
                    with mock.patch.object(sys, "argv", argv):
                        rc = self.wrapper.main()
