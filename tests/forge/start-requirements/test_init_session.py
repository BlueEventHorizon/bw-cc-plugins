#!/usr/bin/env python3
"""start-requirements/scripts/init_session.py の薄い wrapper テスト。"""
...
EXPECTED_SKILL = 'start-requirements'
...
class TestInitSessionWrapper(unittest.TestCase):
    ...
    def setUp(self):
        self.wrapper = load_wrapper(WRAPPER_PATH, '_init_session_start_requirements')
    ...
    def test_positional_args_mapped_to_flags(self):
        rc, mock_run = invoke_with_mocked_run(self.wrapper, argv=['init_session.py', 'my_feature', 'interactive', '/tmp/out'])
        self.assertEqual(rc, 0)
        assert_init_session_command(
            self,
            command_from_mock(mock_run),
            EXPECTED_SKILL,
            {'--feature': 'my_feature', '--mode': 'interactive', '--output-dir': '/tmp/out'},
        )
        assert_transparent_subprocess_kwargs(self, mock_run)

    def test_exit_code_transparent(self):
        assert_exit_code_transparent(self, self.wrapper, lambda: ['init_session.py', 'f', 'interactive', '/tmp/out'])
