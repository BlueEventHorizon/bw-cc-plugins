#!/usr/bin/env python3
"""
query-db-rules/scripts/query_documents.py のテスト

query_docdb.py を category=rules 固定で透過呼び出しするラッパーを検証する
...
helpers = _load(
    ...
    "_wrapper_helpers_query_db_rules_query",
)
...
class TestQueryDocumentsWrapper(unittest.TestCase):
    """query_documents.py が query_docdb.py を category=rules で透過的に呼ぶ"""

    def setUp(self):
        self.wrapper = helpers.load_wrapper(
            helpers.wrapper_path(SKILL, "query_documents.py"),
            "_query_documents_query_db_rules",
        )
