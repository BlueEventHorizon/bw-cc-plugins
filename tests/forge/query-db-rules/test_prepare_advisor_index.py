#!/usr/bin/env python3
"""
query-db-rules/scripts/prepare_advisor_index.py のテスト

低レベル CLI prepare_advisor_index.py を category=rules 固定で透過呼び出しする
ラッパーを検証する（DES-057 §9.2: category 固定値・引数透過・
stdout/stderr/exit code 透過）。
...
helpers = _load(
    ...
    "_wrapper_helpers_query_db_rules_prepare",
)

SKILL = "query-db-rules"
...
class TestPrepareAdvisorIndexWrapper(unittest.TestCase):
    """prepare_advisor_index.py が同名の低レベル CLI を category=rules で透過的に呼ぶ"""

    def setUp(self):
        self.wrapper = helpers.load_wrapper(
            helpers.wrapper_path(SKILL, "prepare_advisor_index.py"),
            "_prepare_advisor_index_query_db_rules",
        )
    ...
    def test_bare_invocation_passes_category_only(self):
        """追加引数なしでは category だけを低レベル CLI へ渡す"""
