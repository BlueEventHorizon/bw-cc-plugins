...
| forge 実装規範         | `plugins/forge/docs/` の additive_development_spec §1–§3 / scope_proportionality_spec、`plugins/forge/skills/start-implement/docs/task_execution_spec.md`、`plugins/anvil/skills/impl-issue/references/`（既存資産優先・推測禁止・共用部品・検証の裏取り）。ただし残存確認の条項（改名・語彙改訂後に別名・形式の呼び名も検索し、検索語と範囲を残す）は COPY 範囲が正本 |
